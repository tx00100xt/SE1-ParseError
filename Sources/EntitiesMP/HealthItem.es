...
procedures:
  ItemCollected(EPass epass) : CItem::ItemCollected {
    ...
    if (IsOfClass(epass.penOther, "Player")) {
			//CPrintF("GetHealth: %.3f\n", ((CPlayer &)*epass.penOther).GetHealth());
			if(m_bOverTopHealth) {
				if (GetSP()->sp_bUsePEHealth) {
					if (((CPlayer &)*epass.penOther).GetHealth()==300) {						
						return;
					} 
				} else {
					if (((CPlayer &)*epass.penOther).GetHealth()==200) {						
						return;
					} 
				}
			} else {
				if (GetSP()->sp_bUsePEHealth) {
					...
					if (((CPlayer &)*epass.penOther).GetHealth()>=100) {						
						return;
					} 
				}
			}
  ...
  Main() {
		...
		if (strLevelName=="15_TheGreatPyramid" && m_strName=="Health +25 Copy") {
      m_bCanBeenPicked = FALSE;
		}
