...
functions:
   ...
   void AdjustDifficulty(void)
  {
    ...
    fMoveSpeed *= 0.8f+FRnd()*0.4f;
    fAttackSpeed *= 0.8f+FRnd()*0.4f;
 ...
 ************************************************************/
  Main(EVoid) {
    ...
    if (m_EecChar==EYC_SERGEANT) {
      ...
      GetModelObject()->StretchModel(FLOAT3D(1.3f*fWhatever, 1.3f*fWhatever, 1.3f*fWhatever));
      ModelChangeNotify();
      m_iScore = 1000;
    } else {
      m_iScore = 500;
      ...
      GetModelObject()->StretchModel(FLOAT3D(1.0f*fWhatever, 1.0f*fWhatever, 1.0f*fWhatever));
      ModelChangeNotify();
    }
    if (m_bInvisible && !GetSP()->sp_bMental) {
      GetModelObject()->mo_colBlendColor = C_WHITE|100;
      m_iScore*=2;
    }
    // setup moving speed
    /*m_fWalkSpeed = FRnd() + 1.5f*fWhatever;
    m_aWalkRotateSpeed = FRnd()*10.0f + 50.0f*fWhatever;
    if (m_EecChar==EYC_SERGEANT) {
      m_fAttackRunSpeed = FRnd()*2.0f + 10.0f*fWhatever;
      m_aAttackRotateSpeed = AngleDeg(FRnd()*50 + 60.0f)*fWhatever;
      m_fCloseRunSpeed = FRnd()*2.0f + 10.0f*fWhatever;
      m_aCloseRotateSpeed = AngleDeg(FRnd()*50 + 60.0f)*fWhatever;
    } else {
      m_fAttackRunSpeed = FRnd()*2.0f + 9.0f*fWhatever;
      m_aAttackRotateSpeed = AngleDeg(FRnd()*50 + 60.0f)*fWhatever;
      m_fCloseRunSpeed = FRnd()*2.0f + 9.0f*fWhatever;
      m_aCloseRotateSpeed = AngleDeg(FRnd()*50 + 60.0f)*fWhatever;
    }*/
    ...
    /*m_fFlyWalkSpeed = FRnd()*2.0f + 3.0f*fWhatever;
    m_aFlyWalkRotateSpeed = FRnd()*20.0f + 600.0f*fWhatever;
    if (m_EecChar==EYC_SERGEANT) {
      m_fFlyAttackRunSpeed = FRnd()*2.0f + 20.0f*fWhatever;
      m_aFlyAttackRotateSpeed = FRnd()*25 + 350.0f*fWhatever;
      m_fFlyCloseRunSpeed = FRnd()*10.0f +20.0f*fWhatever;
      m_aFlyCloseRotateSpeed = FRnd()*50 + 100.0f*fWhatever;
    } else {
      m_fFlyAttackRunSpeed = FRnd()*5.0f + 15.0f*fWhatever;
      m_aFlyAttackRotateSpeed = FRnd()*10.0f + 100.0f*fWhatever;
      m_fFlyCloseRunSpeed = FRnd()*10.0f + 15.0f*fWhatever;
      m_aFlyCloseRotateSpeed = FRnd()*50.0f + 100.0f*fWhatever;
    }*/
    ...
    m_fGroundToAirSpeed = 5.5f;
    m_fAirToGroundSpeed = 5.5f;
    m_fAirToGroundMin = 0.1f;
    m_fAirToGroundMax = 0.1f;
    m_fFlyHeight = 1.0f;
    // attack properties - CAN BE SET
    m_fFlyAttackDistance = 200.0f;
    ...
    m_soMumble.Set3DParameters(25.0f, 0.0f, 1.0f*fWhatever, 1.0f);
		m_fStepHeight = 10.0f;
		m_fLagger = 1.5f; 
    m_bUseTactics = FALSE;
