...
#include "Models/TeleporterGun/GunAnim.h"
...
void CPlayerAnimator_Precache(ULONG ulAvailable)
{
  ...
  if ( ulAvailable&(1<<(WEAPON_DOUBLESHOTGUN-1)) ) {
    pdec->PrecacheModel(MODEL_TP_ANIM        ); 
...
// ************** DOUBLE SHOTGUN **************
 51 model   MODEL_TP_ANIM               "Models\\TeleporterGun\\GunAnim.mdl",
...
// ************** TOMMYGUN **************
 ...
 /*
 70 model   MODEL_TOMMYGUN              "Models\\Weapons\\Mini1\\Mini1GunItem.mdl",
  ...
  void SyncWeapon(void)
  {
    ...
    for (INDEX iWeapon = BODY_ATTACHMENT_COLT_RIGHT; iWeapon<=BODY_ATTACHMENT_ITEM; iWeapon++) {
      ...
      // if exists in both
        ...
        // add it to rendering
        //pamoWeapRen = pmoBodyRen->AddAttachmentModel(iWeapon);
        //pamoWeapRen->amo_plRelative = pamoWeapRen->amo_plRelative;
        //pamoWeapRen->amo_moModelObject.Copy(pamoWeapRen->amo_moModelObject);
        //pamoWeapRen->amo_plRelative = pamoWeapDef->amo_plRelative;
        //pamoWeapRen->amo_plRelative.pl_OrientationAngle += ANGLE3D(0, 90, 0);
        //pamoWeapRen->amo_moModelObject.Synchronize(pamoWeapDef->amo_moModelObject);
      }
    }
  }
  ...
  void SetWeapon(void) {
    ...
    // *********** DOUBLE COLT ***********
      case WEAPON_DOUBLECOLT:
        ...
        AddWeaponAttachment(COLTITEM_ATTACHMENT_BULLETS, MODEL_COLTBULLETS, TEXTURE_COLTBULLETS, 0, 0, 0);
        AddWeaponAttachment(COLTITEM_ATTACHMENT_COCK, MODEL_COLTCOCK, TEXTURE_COLTCOCK, 0, 0, 0);
        AddWeaponAttachment(COLTITEM_ATTACHMENT_BODY, MODEL_COLTMAIN, TEXTURE_COLTMAIN, 0, TEX_SPEC_MEDIUM, 0);
    ...
    // *********** DOUBLE SHOTGUN ***********
      case WEAPON_DOUBLESHOTGUN:
        AddWeaponAttachment(BODY_ATTACHMENT_COLT_RIGHT, MODEL_TP_ANIM, TEXTURE_TP, 0, 0, 0);
        SetAttachment(BODY_ATTACHMENT_COLT_RIGHT);
        AddWeaponAttachment(GUNANIM_ATTACHMENT_GUN, MODEL_TP_GUN, TEXTURE_TP, 0, 0, 0);
    ...
    // *********** TOMMYGUN ***********
      case WEAPON_TOMMYGUN:
        AddWeaponAttachment(BODY_ATTACHMENT_KNIFE, MODEL_BOMB_SMALL, TEXTURE_BOMB, TEX_REFL_DARKMETAL, TEX_SPEC_MEDIUM, 0);
        break;

/*
      case WEAPON_TOMMYGUN:
        AddWeaponAttachment(BODY_ATTACHMENT_MINIGUN, MODEL_TOMMYGUN, TEXTURE_TG_BODY, 0, 0, 0);
        SetAttachment(BODY_ATTACHMENT_MINIGUN);
        AddWeaponAttachment(MINI1GUNITEM_ATTACHMENT_BODY, MODEL_TG_BODY, TEXTURE_TG_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        AddWeaponAttachment(MINI1GUNITEM_ATTACHMENT_BARRELS, MODEL_TG_BARRELS, TEXTURE_TG_BARRELS, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        AddWeaponAttachment(MINI1GUNITEM_ATTACHMENT_ENGINE, MODEL_TG_ENGINE, TEXTURE_TG_BARRELS, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        SetAttachment(MINI1GUNITEM_ATTACHMENT_BODY);
 ...
 ************************************************************/
  ...
  void BodyAndHeadOrientation(CPlacement3D &plView) {
    ...
    pamoHead->amo_plRelative.pl_OrientationAngle(3) *= 4.0f;
		...
		/*if (m_penPlayer!=NULL) {
			if (((CPlayer&)*m_penPlayer).m_penStar!=NULL) {
				CEntity *pen = ((CPlayer&)*m_penPlayer).m_penStar;
				CStar *penStar = ((CStar*)pen);
				penStar->ReOrient();
			}
		}*/
    ...
    // stand
      ...
      if (m_bReference) {
        ...
        // not in jump anim and in stand mode change
          ...
          if (!m_bCrouch) {
            // running anim
            if (vDesiredTranslation.Length()>12.0f && vCurrentTranslation.Length()>12.0f) {
              ...
              m_fLastActionTime = _pTimer->CurrentTick();
            // walking anim
            } else if (vDesiredTranslation.Length()>5.0f && vCurrentTranslation.Length()>5.0f) {
              if (vCurrentTranslation(3)<0) {
  ...
  void FireAnimation(INDEX iAnim, ULONG ulFlags) {
    if (m_bSwim) {
      ...
      switch (iWeapon) {
        case WEAPON_NONE:
          break;
        case WEAPON_KNIFE: case WEAPON_COLT: case WEAPON_DOUBLECOLT: case WEAPON_TOMMYGUN: case WEAPON_DOUBLESHOTGUN:
 ...
 ************************************************************/
    ...
    if (plw.m_iCurrentWeapon==WEAPON_MINIGUN) {
      ...
      if (pamo1!=NULL && pamo2!=NULL) {
        ...
        pamo2->amo_plRelative.pl_OrientationAngle(3) = aAngle;
			...
			/*} else if (plw.m_iCurrentWeapon==WEAPON_TOMMYGUN) {
				ANGLE aAngle = Lerp(plw.m_aMiniGunLast, plw.m_aMiniGun, _pTimer->GetLerpFactor());
				// rotate minigun barrels
				CPlayer &pl = (CPlayer&)*m_penPlayer;
				CAttachmentModelObject *pamo = pl.GetModelObject()->GetAttachmentModelList(
					PLAYER_ATTACHMENT_TORSO, BODY_ATTACHMENT_MINIGUN, MINI1GUNITEM_ATTACHMENT_BARRELS, -1);
				if (pamo!=NULL) {
  ...
  // show flare
  void ShowFlare(INDEX iAttachWeapon, INDEX iAttachObject, INDEX iAttachFlare, FLOAT fSize) {
    CPlayer &pl = (CPlayer&)*m_penPlayer;
    CAttachmentModelObject *pamo = pl.GetModelObject()->GetAttachmentModelList(
    ...
    if (pamo!=NULL) {
      ...
      /*if (iAttachWeapon==BODY_ATTACHMENT_SINGLE_SHOTGUN) {
        pamo->amo_plRelative.pl_OrientationAngle(1) = 180.0f;
      }*/
      CModelObject &mo = pamo->amo_moModelObject;
      mo.StretchModel(FLOAT3D(fSize, fSize, fSize));
    }
  };
...
/*    if(!IsPredictionHead()) {
    ...
    */
    ...
    if (pen->m_iFlare==FLARE_ADD) {
      ...
      switch(iWeapon) {
        case WEAPON_COLT:
          ShowFlare(BODY_ATTACHMENT_COLT_RIGHT, COLTITEM_ATTACHMENT_BODY, COLTMAIN_ATTACHMENT_FLARE, 0.5f);
          break;
        case WEAPON_DOUBLECOLT: 
          ShowFlare(BODY_ATTACHMENT_COLT_RIGHT, COLTITEM_ATTACHMENT_BODY, COLTMAIN_ATTACHMENT_FLARE, 0.5f);
          ShowFlare(BODY_ATTACHMENT_COLT_LEFT, COLTITEM_ATTACHMENT_BODY, COLTMAIN_ATTACHMENT_FLARE, 0.5f);
          break;
        case WEAPON_SINGLESHOTGUN:
          ShowFlare(BODY_ATTACHMENT_SINGLE_SHOTGUN, SINGLESHOTGUNITEM_ATTACHMENT_BARRELS, BARRELS_ATTACHMENT_FLARE, 0.7f);
          break;
/*        case WEAPON_DOUBLESHOTGUN:
          ShowFlare(BODY_ATTACHMENT_DOUBLE_SHOTGUN, DOUBLESHOTGUNITEM_ATTACHMENT_BARRELS, DSHOTGUNBARRELS_ATTACHMENT_FLARE);
          break;
        case WEAPON_TOMMYGUN:
          ShowFlare(BODY_ATTACHMENT_MINIGUN, MINI1GUNITEM_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE);
//          ShowFlare(BODY_ATTACHMENT_TOMMYGUN, TOMMYGUNITEM_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE);
          break;*/
        case WEAPON_SNIPER:
          ShowFlare(BODY_ATTACHMENT_FLAMER, SNIPERITEM_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE, 0.6f);
          break;
        case WEAPON_MINIGUN:
          ShowFlare(BODY_ATTACHMENT_MINIGUN, MINI2GUNITEM_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE01, 1.0f);
          ShowFlare(BODY_ATTACHMENT_MINIGUN, MINI2GUNITEM_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE02, 1.0f);
          break;
      }
    // remove
...
/*        case WEAPON_DOUBLESHOTGUN:
          ...
          break;
        case WEAPON_TOMMYGUN:
          HideFlare(BODY_ATTACHMENT_MINIGUN, MINI1GUNITEM_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE);
