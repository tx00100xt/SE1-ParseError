...
event EKeyFE {
  enum KeyItemTypeFE kitType,
};

%{

const char *GetKeyNameFE(enum KeyItemTypeFE kit)
{
  switch(kit) {
...
class CKeyItemFE : CItem {
name      "KeyItemFE";
thumbnail "Thumbnails\\KeyItem.tbn";
features "IsImportant";

properties:
  1 enum KeyItemTypeFE m_kitTypeFE    "Type" 'Y' = KIT_ANKHWOOD,     // key type
...
functions:
  ...
  {
    //pes->es_strName = GetKeyName(m_kitTypeFE);
    pes->es_ctCount = 1;
    pes->es_ctAmmount = 1;
    pes->es_fValue = 1;
  ...
  void SetProperties(void) {
    ...
    //m_strDescription = GetKeyName(m_kitTypeFE);
