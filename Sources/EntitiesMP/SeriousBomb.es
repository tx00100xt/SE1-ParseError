  ...
  void ExplodeBomb(void)
  {
    ...
    {FOREACHINDYNAMICCONTAINER(this->GetWorld()->wo_cenEntities, CEntity, iten) {
      ...
      if (IsDerivedFromClass(pen, "Enemy Base")) {
        ...
        if (penEnemy->m_bBoss==TRUE || DistanceTo(this, penEnemy)>150.0f) {
          continue;
        }
				FLOAT tmSpawnInvuln = 0.1f;
				if (tmSpawnInvuln>0 && _pTimer->CurrentTick()-penEnemy->m_tmSpawned<tmSpawnInvuln
					|| !penEnemy->m_bHasBeenSpawned ) {
					// ignore damage
          continue;
        }
				if (m_penOwner!=NULL) {
					this->InflictDirectDamage(pen, m_penOwner, DMT_EXPLOSION, penEnemy->GetHealth()+1000.0f, pen->GetPlacement().pl_PositionVector, FLOAT3D(0,1,0));
				} else {
					this->InflictDirectDamage(pen, this, DMT_EXPLOSION, penEnemy->GetHealth()+1000.0f, pen->GetPlacement().pl_PositionVector, FLOAT3D(0,1,0));
				}
      }
    }}
  }
...
procedures:
  ...
  Main(ESeriousBomb esb)
  {
    ...
    if (esb.penOwner) {
      ...
      //Glare(1.0f, 2.0f, 0.8f, 0.8f);
      
      ShakeItBaby(_pTimer->CurrentTick(), 0.5f, 0.5f, TRUE);
      autowait(1.5f);

      // fading shake     
      ShakeItBaby(_pTimer->CurrentTick(), 1.0f, 0.5f, FALSE);
