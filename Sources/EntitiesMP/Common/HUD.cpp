...
static CTextureObject _iconEnemy_Ughzan_1;
...
//##################################################################################################################
static void HUD_DrawIconENM( FLOAT fCenterX, FLOAT fCenterY, CTextureObject &toIcon, COLOR colDefault, FLOAT fNormValue, BOOL bBlink)
{
  // determine color
  COLOR col = colDefault;
  if( col==NONE) col = GetCurrentColor( fNormValue);
  ...
  CTextureData *ptd = (CTextureData*)toIcon.GetData();
	...
	const FLOAT fHalfSizeI = _fResolutionScaling * _fCustomScalingAdjustment *_fCustomScaling * ptd->GetPixWidth()  * iconscale;
	const FLOAT fHalfSizeJ = _fResolutionScaling * _fCustomScalingAdjustment *_fCustomScaling * ptd->GetPixHeight() * iconscale;
  // done
  _pDP->InitTexture( &toIcon);
...
//FLOAT fCol2 = pixLeftBound+220;
  ...
  fCol = pixRightBound -fHalfUnitS - 200.0f;
...
#define TXT_WIDTH_KILLS		4
  ...
  COLOR colMana, colFrags, colDeaths, colHealth, colArmor;//, colScore;
  INDEX iScoreSum = 0;
...
//####
  ...
  if (hud_bShowPEInfo) {
   ...
   // draw 'the ugh zan lives
  if (gam_bUZActive == TRUE) {
    int rpos=0;
    int iRowSize=20;
    CTextureObject* cenemy = NULL;
    cenemy = &_iconEnemy_Ughzan_1;
    fRow = pixTopBound  +20;
    int fRowIcon=(pixTopBound  +25);
    fCol = pixLeftBound + fHalfUnit;
    fAdv = 0;//fAdvUnit+ fChrUnit*fWidthAdj/2 -fHalfUnit
    int pname1_len;
    pname1_len = 0;
    if (gam_iUZLives>0) {
       HUD_DrawIconENM(fCol+fAdv+pname1_len+20,fRowIcon+(rpos * iRowSize)-2, *cenemy, colScore, 1.0f, FALSE);
    }
    if (gam_iUZLives>2) {
       HUD_DrawIconENM(fCol+fAdv+pname1_len+50,fRowIcon+(rpos * iRowSize)-2, *cenemy, colScore, 1.0f, FALSE);
    }
    if (gam_iUZLives>4) {
       HUD_DrawIconENM(fCol+fAdv+pname1_len+80,fRowIcon+(rpos * iRowSize)-2, *cenemy, colScore, 1.0f, FALSE);
    }
  }
  // end of draw 'the ugh zan lives
...
extern void InitHUD(void)
{
  ...
  try {
    ...
    _iconEnemy_Ughzan_1.SetData_t(  CTFILENAME("Textures\\EnemyIcons\\Ughzan_1.tex"));
    ...
    ((CTextureData*)_iconEnemy_Ughzan_1.GetData())->Force(TEX_CONSTANT);
