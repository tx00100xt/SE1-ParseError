...
// NEW 
static CTextureObject _toKnifeTrail01;
static CTextureObject _toShockWave;
static CTextureObject _toRailTrail;
static CTextureObject _toFlakTrail;
static CTextureObject _toTracerTrail;
static CTextureObject _toColtShells;
static CTextureObject _toFireworks01;
static CTextureObject _toFireworks02;
static CTextureObject _toSnowFlakes;
static CTextureObject _toPlasmaBall;
static CTextureObject _toPlasmaSmoke;
static CTextureObject _toGlow;
static CTextureObject _toGlow2;
static CTextureObject _toGlow3;
...
// NEW
    _toKnifeTrail01.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\KnifeTrail01.tex"));
    _toShockWave.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\ShockWave.tex"));
    _toRailTrail.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\RailTrail.tex"));
    _toFlakTrail.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\FlakTrail.tex"));
    _toTracerTrail.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\TracerTrail.tex"));
    _toColtShells.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\ColtShells.tex"));
		_toFireworks01.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\Fireworks01.tex"));
		_toFireworks02.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\Fireworks02.tex"));
		_toSnowFlakes.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\SnowFlakes.tex"));
		_toPlasmaBall.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\PlasmaBall.tex"));
		_toPlasmaSmoke.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\PlasmaSmoke.tex"));
		_toGlow.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\Glow.tex"));
		_toGlow2.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\Glow2.tex"));
		_toGlow3.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\Glow3.tex"));
...
// NEW
  _toKnifeTrail01.SetData(NULL);
  _toShockWave.SetData(NULL);
  _toRailTrail.SetData(NULL);
  _toFlakTrail.SetData(NULL);
  _toTracerTrail.SetData(NULL);
	_toColtShells.SetData(NULL);
	_toFireworks01.SetData(NULL);
	_toFireworks02.SetData(NULL);
	_toSnowFlakes.SetData(NULL);
	_toPlasmaBall.SetData(NULL);
	_toPlasmaSmoke.SetData(NULL);
	_toGlow.SetData(NULL);
	_toGlow2.SetData(NULL);
	_toGlow3.SetData(NULL);
}
...
void SetupParticleTexture(enum ParticleTexture ptTexture)
{
  ...
  case PT_LAVAFLOW:         Particle_PrepareTexture(&_toLava,      PBT_BLEND); break;
	case PT_GLOW:             Particle_PrepareTexture(&_toGlow,			 PBT_ADD);   break;
	case PT_GLOW2:            Particle_PrepareTexture(&_toGlow2,		 PBT_ADD);   break;
	case PT_GLOW3:            Particle_PrepareTexture(&_toGlow3,		 PBT_ADD);   break;
...
void SetupParticleTextureWithAddAlpha(enum ParticleTexture ptTexture)
{
  ...
  case PT_WATERFLOW:        Particle_PrepareTexture(&_toWater,     PBT_BLEND); break;
  case PT_LAVAFLOW:         Particle_PrepareTexture(&_toLava,      PBT_BLEND); break;
	case PT_GLOW:             Particle_PrepareTexture(&_toGlow,			 PBT_ADDALPHA);   break;
	case PT_GLOW2:            Particle_PrepareTexture(&_toGlow2,		 PBT_ADDALPHA);   break;
	case PT_GLOW3:            Particle_PrepareTexture(&_toGlow3,		 PBT_ADDALPHA);   break;
  default:    ASSERT(FALSE);
  }
  Particle_SetTexturePart( 512, 512, 0, 0);
}
...
#define BOMB_TRAIL_POSITIONS 5
#define BOMB_TRAIL_INTERPOSITIONS 2
...
#define FIRECRACKER_TRAIL_POSITIONS 8
...
#define TP_TRAIL_POSITIONS 10
//#define TP_TRAIL_INTERPOSITIONS 4
//#define TP_TRAIL_PARTICLES (TP_TRAIL_INTERPOSITIONS*TP_TRAIL_POSITIONS)
void Particles_TPTrail_Prepare(CEntity *pen)
{
  pen->GetLastPositions(TP_TRAIL_POSITIONS);
}
void Particles_TPTrail(CEntity *pen)
{
  /*CLastPositions *plp = pen->GetLastPositions(TP_TRAIL_POSITIONS);
  Particle_PrepareTexture(&_toRailTrail,  PBT_ADD);
  Particle_SetTexturePart( 512, 512, 0, 0);

  if( plp->lp_ctUsed<2) return;
  const FLOAT3D *pvPos1;
  const FLOAT3D *pvPos2 = &plp->GetPosition(plp->lp_ctUsed-1);
  INDEX iParticle = plp->lp_ctUsed*TP_TRAIL_INTERPOSITIONS;
  for(INDEX iPos = plp->lp_ctUsed-2; iPos>=0; iPos--)
  {
    pvPos1 = pvPos2;
    pvPos2 = &plp->GetPosition(iPos);
    for (INDEX iInter=0; iInter<TP_TRAIL_INTERPOSITIONS; iInter++)
    {
      FLOAT3D vPos = Lerp(*pvPos1, *pvPos2, iInter*1.0f/TP_TRAIL_INTERPOSITIONS);
      FLOAT fSize = (TP_TRAIL_PARTICLES-iParticle)*0.25f/TP_TRAIL_PARTICLES;
      Particle_RenderSquare( vPos, fSize/3.0f, 0, C_WHITE|255);
      //Particle_RenderSquare( vPos, fSize/3.0f, 0, RGBToColor(44,0,70)|255);
      iParticle--;
    }
  }
  // all done
  ...
  for( INDEX iPos=3; iPos<plp->lp_ctUsed+1; iPos++)
  {
    FLOAT3D vPos = plp->GetPosition(iPos);
    ...
    FLOAT fSize = 0.1f-iPos*0.015f;
    //FLOAT fSize = 0.1f;
    Particle_RenderLine( vPos, vOldPos, fSize, C_WHITE|255);
    //Particle_RenderLine( vPos, vOldPos, fSize, RGBToColor(44,0,70)|255);
    vOldPos=vPos;
  }
  // all done
  Particle_Flush();
...
#define SW_TRAIL_POSITIONS 16
#define SW_TRAIL_INTERPOSITIONS 1
#define SW_TRAIL_PARTICLES (SW_TRAIL_INTERPOSITIONS*SW_TRAIL_POSITIONS)
void Particles_SWTrail_Prepare(CEntity *pen)
{
  pen->GetLastPositions(TP_TRAIL_POSITIONS);
}
void Particles_SWTrail(CEntity *pen)
{
  CLastPositions *plp = pen->GetLastPositions(SW_TRAIL_POSITIONS);
  Particle_PrepareTexture(&_toShockWave,  PBT_BLEND); // PBT_ADD
  Particle_SetTexturePart( 512, 512, 0, 0);

  if( plp->lp_ctUsed<2) return;
  const FLOAT3D *pvPos1;
  const FLOAT3D *pvPos2 = &plp->GetPosition(plp->lp_ctUsed-1);
  INDEX iParticle = plp->lp_ctUsed*SW_TRAIL_INTERPOSITIONS;
  for(INDEX iPos = plp->lp_ctUsed-2; iPos>=0; iPos--)
  {
    pvPos1 = pvPos2;
    pvPos2 = &plp->GetPosition(iPos);
    for (INDEX iInter=0; iInter<SW_TRAIL_INTERPOSITIONS; iInter++)
    {
      FLOAT3D vPos = Lerp(*pvPos1, *pvPos2, iInter*1.0f/SW_TRAIL_INTERPOSITIONS);
      FLOAT fSize = 
        (SW_TRAIL_PARTICLES-iParticle)*0.25f/SW_TRAIL_PARTICLES;
      UBYTE ub = 255-iParticle*255/SW_TRAIL_PARTICLES;
      Particle_RenderSquare( vPos, fSize*1.1f, 0, C_WHITE|100);
      iParticle--;
    }
  }
  // all done
...
#define RAIL_TRAIL_POSITIONS 4
void Particles_Rail_Trail_Prepare(CEntity *pen)
{
  pen->GetLastPositions(RAIL_TRAIL_POSITIONS);
}
void Particles_Rail_Trail(CEntity *pen)
{
  CLastPositions *plp = pen->GetLastPositions(RAIL_TRAIL_POSITIONS);
  Particle_PrepareTexture(&_toVerticalGradient, PBT_ADD);
  //Particle_PrepareTexture(&_toTracerTrail, PBT_ADD);
  Particle_SetTexturePart( 512, 512, 0, 0);
  FLOAT3D vOldPos = plp->GetPosition(1);
  for( INDEX iPos=1; iPos<plp->lp_ctUsed; iPos++)
  {
    FLOAT3D vPos = plp->GetPosition(iPos);
    FLOAT fSize = iPos*0.01f+0.025f;
    //Particle_RenderLine( vPos, vOldPos, fSize, C_dRED|255);
    Particle_RenderLine( vPos, vOldPos, fSize, C_GREEN|255);
    vOldPos=vPos;
  }
  // all done
  Particle_Flush();
...
#define FLAK_TRAIL_POSITIONS 4
#define FLAK_TRAIL_INTERPOSITIONS 32
#define FLAK_TRAIL_PARTICLES (FLAK_TRAIL_INTERPOSITIONS*FLAK_TRAIL_POSITIONS)
void Particles_Flak_Trail_Prepare(CEntity *pen)
{
  pen->GetLastPositions(FLAK_TRAIL_POSITIONS);
}
void Particles_Flak_Trail(CEntity *pen)
{
  CLastPositions *plp = pen->GetLastPositions(FLAK_TRAIL_POSITIONS);
  Particle_PrepareTexture(&_toFlakTrail,  PBT_ADD);
  Particle_SetTexturePart( 512, 512, 0, 0);

  if( plp->lp_ctUsed<2) return;
  const FLOAT3D *pvPos1;
  const FLOAT3D *pvPos2 = &plp->GetPosition(plp->lp_ctUsed-1);
  INDEX iParticle = plp->lp_ctUsed*FLAK_TRAIL_INTERPOSITIONS;
  for(INDEX iPos = plp->lp_ctUsed-2; iPos>=0; iPos--)
  {
    pvPos1 = pvPos2;
    pvPos2 = &plp->GetPosition(iPos);
    for (INDEX iInter=0; iInter<FLAK_TRAIL_INTERPOSITIONS; iInter++)
    {
      FLOAT3D vPos = Lerp(*pvPos1, *pvPos2, iInter*1.0f/FLAK_TRAIL_INTERPOSITIONS);
      FLOAT fSize = 
        (FLAK_TRAIL_PARTICLES-iParticle)*0.25f/FLAK_TRAIL_PARTICLES;
      UBYTE ub = 255-iParticle*255/FLAK_TRAIL_PARTICLES;
      Particle_RenderSquare( vPos, fSize/4.0f, 0, C_vdYELLOW|255);
      iParticle--;
    }
  }
  // all done
...
#define TRACER_TRAIL_POSITIONS 3
void Particles_Tracer_Trail_Prepare(CEntity *pen)
{
  pen->GetLastPositions(TRACER_TRAIL_POSITIONS);
}

void Particles_Tracer_Trail(CEntity *pen)
{
  // render line
  CLastPositions *plp = pen->GetLastPositions(TRACER_TRAIL_POSITIONS);
  Particle_PrepareTexture(&_toTracerTrail, PBT_ADDALPHA);
	Particle_SetTexturePart( 512, 2048, 0, 0);
	Particle_RenderLine( plp->GetPosition(0), plp->GetPosition(2), 0.1f, C_WHITE|255);
  // all done
  Particle_Flush();
}

/*	//Get Offset position (around which camera is rotated) 
	CPlacement3D plVehicle = CPlacement3D(m_vCamOffset, ANGLE3D());//m_aCamRotation(1)-180,m_aCamRotation(2)*-1,m_aCamRotation(3)-180)); 
	plVehicle.RelativeToAbsolute(GetPlacement()); 
	//Get placement for ray (with direction from camera to offset) 
	FLOAT3D vDir = plVehicle.pl_PositionVector - ((CPlayer&)*m_penPlayer).m_penView->GetPlacement().pl_PositionVector; 
	DirectionVectorToAngles(vDir, plVehicle.pl_OrientationAngle); 
	CCastRay crRay(this, plVehicle);

void Particles_Tracer_Trail(CEntity *pen, FLOAT tmStart)
{
  // render line
  CLastPositions *plp = pen->GetLastPositions(TRACER_TRAIL_POSITIONS);
  //Particle_PrepareTexture(&_toTracerTrail, PBT_ADD);
  ...
  for( INDEX iPos=1; iPos<plp->lp_ctUsed; iPos++)
  {
    ...
    FLOAT fSize = 0.04f;
		...
		INDEX iRnd =(pen->en_ulID+iPos+INDEX(tmStart*317309.14521f))%CT_MAX_PARTICLES_TABLE;
    FLOAT fRatio=Clamp(fT/0.5f, 0.0f, 1.0f);
    UBYTE ubR = 255;
    ...
    //COLOR colAlpha = pTD->GetTexel(PIX(ClampUp(fRatio*1024.0f, 1023.0f)), 0);
		COLOR colAlpha = pTD->GetTexel( ClampUp(FloatToInt(fT*8192),8191L), 0);
    COLOR col= RGBToColor( ubR, ubG, ubB) | (colAlpha&0x000000FF);
    col=MulColors(col, 0xFFFFFFFF);
    //Particle_RenderLine( vPos, vOldPos, fSize, C_vlYELLOW|255);
		Particle_RenderLine( vPos, vOldPos, fSize, col);
    vOldPos=vPos;
  }
  // all done
  Particle_Flush();
...
#define SPIRAL_TRAIL_POSITIONS 48
void Particles_SpiralTrail(CEntity *pen)
{
  ...
  FLOAT fSeconds = _pTimer->GetLerpedCurrentTick();
  FLOAT fAngle = 0.0f;
  Particle_PrepareTexture(&_toStar01, PBT_ADD);
  Particle_SetTexturePart( 512, 512, 0, 0);
  ...
  for(INDEX iPos = 0; iPos<plp->lp_ctUsed; iPos++)
  {
    ...
    vPos(1) += fSin*iPos*1.0f/SPIRAL_TRAIL_POSITIONS*8;
    vPos(2) += fCos*iPos*1.0f/SPIRAL_TRAIL_POSITIONS*8;

    //UBYTE ub = iPos*SPIRAL_TRAIL_POSITIONS;
    Particle_RenderSquare( vPos, 0.2f, fAngle, C_RED|255);
  }
  // all done
  Particle_Flush();
}
...
#define FIREBALL01_TRAIL_POSITIONS 4
#define FIREBALL01_TRAIL_INTERPOSITIONS 2
...
#define GRENADE_TRAIL_POSITIONS 8
...
#define CANNON_TRAIL_POSITIONS 5
...
#define LAVA_BOMB_TRAIL_POSITIONS 2
...
#define BEAST_PROJECTILE_DEBRIS_TRAIL_POSITIONS 3
...
#define BEAST_PROJECTILE_TRAIL_POSITIONS 8
...
#define BEAST_BIG_PROJECTILE_TRAIL_POSITIONS 16
...
#define ROCKET_TRAIL_POSITIONS 6
#define ROCKET_TRAIL_INTERPOSITIONS 6
...
#define TM_EXPLOSIONDEBRISLIFE1 0.5f
#define CT_EXPLOSIONDEBRIS1 16
void Particles_ExplosionDebris1(CEntity *pen, FLOAT tmStart, FLOAT3D vStretch, COLOR colMultiply/*=C_WHITE|CT_OPAQUE*/)
{
  FLOAT tmNow = _pTimer->GetLerpedCurrentTick();
	FLOAT tmAge = tmNow-tmStart;
	if (tmAge>TM_EXPLOSIONDEBRISLIFE1) { return; }
  ...
  for( INDEX iDebris=0; iDebris<CT_EXPLOSIONDEBRIS1; iDebris++)
  {
    ...
    if (vPos(2)<vCenter(2)) vPos(2) = ( vPos(2) - vCenter(2) ) * -1.0f + vCenter(2); 
    UBYTE ubR = (UBYTE) (255);
...
void Particles_ExplosionDebris3(CEntity *pen, FLOAT tmStart, FLOAT3D vStretch, COLOR colMultiply/*=C_WHITE|CT_OPAQUE*/)
{
  ...
  for( INDEX iDebris=0; iDebris<CT_EXPLOSIONDEBRIS3; iDebris++)
  {
    ...
    FLOAT3D vOldPos=vCenter+vRelOld;
		if (vOldPos(2)<vCenter(2)) vOldPos(2) = ( vOldPos(2) - vCenter(2) ) * -1.0f + vCenter(2);
...
#define TM_EXPLOSIONDEBRISLIFE4 0.25f
#define CT_EXPLOSIONDEBRIS4 6
void Particles_ExplosionDebris4(CEntity *pen, FLOAT tmStart, FLOAT3D vStretch, COLOR colMultiply/*=C_WHITE|CT_OPAQUE*/)
{
  CTextureData *pTD = (CTextureData *) _toExplosionDebrisGradient.GetData();
  Particle_PrepareTexture( &_toLavaEruptingTexture, PBT_ADDALPHA);
  ...
  FLOAT tmNow = _pTimer->GetLerpedCurrentTick();
	FLOAT tmAge = tmNow-tmStart;
	if (tmAge>TM_EXPLOSIONDEBRISLIFE4) { return; }
  FLOAT fG = 0.0f;//5.0f;
  FLOAT fStretchSize=(vStretch(1)+vStretch(2)+vStretch(3))/3.0f;

  for( INDEX iDebris=0; iDebris<CT_EXPLOSIONDEBRIS4; iDebris++)
  {
    INDEX iRnd =(pen->en_ulID+iDebris+INDEX(tmStart*123456.23465f))%CT_MAX_PARTICLES_TABLE;
    INDEX iRnd2=(pen->en_ulID+iDebris+INDEX(tmStart*653542.129633))%CT_MAX_PARTICLES_TABLE;
    INDEX iTexture = iRnd%3;
    ...
    FLOAT3D vSpeed=FLOAT3D(afStarsPositions[iRnd][0],afStarsPositions[iRnd][1],afStarsPositions[iRnd][2]);
    FLOAT fT=tmNow-tmStart;
    FLOAT fRatio=Clamp(fT/TM_EXPLOSIONDEBRISLIFE4*(afStarsPositions[iRnd][2]+1.0f), 0.0f, 1.0f);
    FLOAT fTimeDeccelerator=Clamp(1.0f-(fT/2.0f)*(fT/2.0f), 0.5f, 1.0f);
    FLOAT fSpeed=(afStarsPositions[iRnd][0]+afStarsPositions[iRnd][1]+afStarsPositions[iRnd][2]+0.5f*3)/3.0f*60.0f;
    fSpeed*=fTimeDeccelerator;
    FLOAT3D vRel=vSpeed*fSpeed*fT-vY*fG*fT*fT;
    ...
    FLOAT fSize=(0.2f+afStarsPositions[iRnd2][0]*0.2f)*fStretchSize;
    FLOAT fAngle=afStarsPositions[iRnd2][1]*720.0f*fT;
    Particle_RenderSquare( vPos, fSize*10.0f, fAngle, col);
  }
  // all done
  Particle_Flush();
}

/*#define TM_ES_TOTAL_LIFE 2.0f
#define TM_ES_SMOKE_DELTA 1.4f
void Particles_ExplosionSmoke(CEntity *pen, FLOAT tmStart, FLOAT3D vStretch, COLOR colMultiply)
{
  ...
  INDEX ctSmokes = 1+INDEX((afStarsPositions[iCtRnd][0]+0.5f)*2);
  for(INDEX i=0; i<ctSmokes; i++)
  {
    ...
    if( fT>0)
    {
      ...
      FLOAT fRndBlend = 16.0f+(afStarsPositions[iRnd][2]+0.5f)*32.0f;
      UBYTE ubRndH=255;
      UBYTE ubRndS=0;
      UBYTE ubRndV = UBYTE( 180.0f+afStarsPositions[iRnd][0]*64.0f);
...
void Particles_ExplosionSmoke(CEntity *pen, FLOAT tmStart, FLOAT3D vStretch, COLOR colMultiply/*=C_WHITE|CT_OPAQUE*/)
{
  ...
  INDEX iCtRnd =(pen->en_ulID*2+INDEX(tmStart*5311234.12531))%CT_MAX_PARTICLES_TABLE;
  INDEX ctSmokes = 4+INDEX((afStarsPositions[iCtRnd][0]+0.5f)*2);
  for(INDEX i=0; i<ctSmokes; i++)
  {
		FLOAT fG = 2.0f;
    INDEX iRnd = (pen->en_ulID+i+INDEX(tmStart*432256.32423f))%CT_MAX_PARTICLES_TABLE;
    INDEX iRnd2 =( pen->en_ulID+i+INDEX(tmStart*631512.15464f))%CT_MAX_PARTICLES_TABLE;
    INDEX iRndTex = iRnd*324561+pen->en_ulID;
    Particle_SetTexturePart( 512, 512, iRndTex%3, 0);
    FLOAT3D vSpeed = FLOAT3D(afStarsPositions[iRnd][0],afStarsPositions[iRnd][1],afStarsPositions[iRnd][2]);
    FLOAT fT = fNow-tmStart;
    FLOAT fTimeDeccelerator = Clamp(1.0f-(fT*1.25f), 0.25f, 1.0f);
    FLOAT fSpeed = (afStarsPositions[iRnd][0]+afStarsPositions[iRnd][1]+afStarsPositions[iRnd][2]+0.5f*3)/3.0f*120.0f;
    fSpeed *= fTimeDeccelerator;
    FLOAT3D vRel = vSpeed*fSpeed*fT-vY*fG*fT*fT;
    vRel *= vStretch(1);   
    FLOAT3D vPos = vCenter+vRel;
    ...
    UBYTE ubRndV = UBYTE( 96.0f+afStarsPositions[iRnd][0]*64.0f);
		UBYTE ubAlpha = UBYTE( 70-fT*139);
    COLOR col = HSVToColor(200, 0, ubRndV)|ubAlpha;
    col = MulColors(col, colMultiply);
	
		//CPrintF("iRndfT: %.3f, fSpeed: %.3f, fTimeDeccelerator: %.3f\n", fT, fSpeed, fTimeDeccelerator);
		//CPrintF("iRnd: %d, : %.3f\n", iRnd, afStarsPositions[iRnd][1]);
		FLOAT fSize = fT*fStretchSize*(10.0f+afStarsPositions[iRnd][1]*2.0f);
    FLOAT fAngle = afStarsPositions[iRnd2][1]*20.0f*fT;
    Particle_RenderSquare( vPos, fSize, fAngle, col);
  }
  // all done
  Particle_Flush();
...
void Particles_ChimneySmoke(CEntity *pen, INDEX ctCount, FLOAT fStretchAll, FLOAT fMipDisappearDistance)
{
  ...
  INDEX ctSmokes=12+INDEX((afStarsPositions[iCtRnd%CT_MAX_PARTICLES_TABLE][0]+0.5f)*8);
  for(INDEX i=0; i<ctSmokes; i++)
  {
                                  ...
                                  FLOAT fParam1)
{
  ...
  for(INDEX i=0; i<ctCount; i++)
  {
    ...
    COLOR colA = pTD->GetTexel(PIX(ClampUp(fT*1024.0f, 1023.0f)), 0);
    UBYTE ubA=UBYTE((colA&0xFF)*0.75f*fMipBlender);
 //   COLOR colCombined=(col&0x99000000)|ubA;
    COLOR colCombined=(col&0xFFFFFF00)|ubA;
...
#define BLOOD01_TRAIL_POSITIONS 10
void Particles_BloodTrail(CEntity *pen)
{
  ...
  for( INDEX iPos=0; iPos<plp->lp_ctUsed; iPos++)
  {
    ...
    Particle_RenderSquare( vPos, fSize*2, fAngle, col);
  }
  // all done
  Particle_Flush();
}
...
#define ELECTRICITY_SPARKS_FADE_OUT_TIME 0.2f
#define ELECTRICITY_SPARKS_TOTAL_TIME 0.5f
void Particles_ElectricitySparks( CEntity *pen, FLOAT fTimeAppear, FLOAT fSize, FLOAT fHeight, INDEX ctParticles)
{
  ...
  for( INDEX iSpark=0; iSpark<ctParticles; iSpark++)
  {
    ...
    {
      fFade=-0.5f/(ELECTRICITY_SPARKS_TOTAL_TIME-ELECTRICITY_SPARKS_FADE_OUT_TIME)*(fT-ELECTRICITY_SPARKS_TOTAL_TIME);
    }
    else
    {
      fFade=0.5f;
...
void Particles_Raycast(const FLOAT3D &vSrc, const FLOAT3D &vDst, INDEX ctRays, FLOAT fSize, COLOR colLine)
{
  Particle_PrepareTexture(&_toVerticalGradient, PBT_ADD);
  Particle_SetTexturePart( 512, 512, 0, 0);
  // get direction vector
  ...
  for(INDEX iRay = 1; iRay<ctRays; iRay++)
  {
    FLOAT3D v0 = vSrc;
    FLOAT fT = FLOAT(iRay)/ctRays + _pTimer->GetLerpedCurrentTick()/1.5f;
    FLOAT fDT = fT-INDEX(fT);
    ...
    {
      //INDEX iOffset = ULONG(fPos*1234.5678f+iRay*103)%32;
      FLOAT3D v1 = vSrc+(vZ*fPos + vX*(0.5f*fSize) +
                                   vY*(0.5f*fSize));
      Particle_RenderLine( v0, v1, 0.05f*fSize, colLine|0xFF);
      v0 = v1;
    }
  }
  // all done
                    ...
                    CTextureData *ptdSnowMap, FLOATaabbox3D &boxSnowMap, FLOAT fSnowStart)
{
  ...
  //Particle_SetTexturePart( 512, 512, 0, 0);
	Particle_PrepareTexture(&_toSnowFlakes, PBT_BLEND);
  ...
  for( INDEX iZ=0; iZ<ctGrids; iZ++)
  {
    ...
    for( INDEX iX=0; iX<ctGrids; iX++)
    {
      ...
      for( INDEX iY=0; iY<(YGRIDS_VISIBLE_ABOVE+YGRIDS_VISIBLE_BELOW); iY++)
      {
        ...
        UBYTE ubR = 255;
        COLOR colDrop = RGBToColor(ubR, ubR, ubR)|(UBYTE(fFactor*255.0f));
        FLOAT fSize = 0.2f+afStarsPositions[(INDEX)fT0*CT_MAX_PARTICLES_TABLE][1]*0.05f;
        ...
        if( ptdSnowMap != NULL)
        {
          ...
          else
          {
            continue;
				...
				Particle_SetTexturePart( 512, 512, int((iRndX-308)/4)%4, int(iRndZ/511)%2);
        Particle_RenderSquare( vRender, fSize/2, fAngle, colDrop);
      }
    }
  }
  // all done
...
#define BULLET_SPRAY_FADEOUT_START 0.25f
#define BULLET_SPRAY_TOTAL_TIME 0.6f

#define BULLET_SPRAY_WATER_FADEOUT_START 0.25f
#define BULLET_SPRAY_WATER_TOTAL_TIME 0.7f

#define BULLET_SPARK_FADEOUT_START 0.025f
#define BULLET_SPARK_TOTAL_TIME 0.06f
#define BULLET_SPARK_FADEOUT_LEN (BULLET_SPARK_TOTAL_TIME-BULLET_SPARK_FADEOUT_START)

#define BULLET_SMOKE_FADEOUT_START 0.0f
#define BULLET_SMOKE_TOTAL_TIME 0.6f
...
void Particles_EmptyShells( CEntity *pen, ShellLaunchData *asldData, INDEX iTexture)
{
  if (iTexture==1) {
    Particle_PrepareTexture(&_toColtShells, PBT_BLEND);
  } else {
  ...
  for( INDEX iShell=0; iShell<MAX_FLYING_SHELLS; iShell++)
  {
    ...
    switch( sld.sld_estType)
    {
      ...
      case ESL_COLT_SHELLS:
      {
        FLOAT fLife = 1.5f;
        if( tmNow > (tmLaunch+fLife) ) continue;
        FLOAT fTRatio = fT/fLife;
        INDEX iFrame = INDEX( fTRatio*16*4)%16;
        iRow = iFrame/4;
        iColumn = iFrame%4;
        Particle_SetTexturePart( 256, 256, iColumn, iRow);
        FLOAT3D vPos = (sld.sld_vPos)+(sld.sld_vSpeed*fT)+(vGDir*(fT*fT*fGA/2.0f));
        Particle_RenderSquare( vPos, 0.02f, 0, C_WHITE|CT_OPAQUE);
        break;
      }
    }
  }
...
void Particles_ColtShells( CEntity *pen, ShellLaunchData *asldData)
{
  FLOAT tmNow = _pTimer->GetLerpedCurrentTick();
  FLOAT fGA = ((CMovableEntity *)pen)->en_fGravityA;
  FLOAT3D vGDir = ((CMovableEntity *)pen)->en_vGravityDir;
  ...
  Particle_Flush();
	...
	/*void Particles_Combo( CEntity *pen, INDEX iX )
{

  // calculate viewer position
  FLOAT3D vPosViewer = prPlayerProjection->pr_vViewerPosition;

  FLOAT tmNow = _pTimer->GetLerpedCurrentTick();
	FLOAT3D vPos;
  FLOAT3D vPlacement = ((CMovableEntity *)pen)->GetPlacement().pl_PositionVector;
	vPlacement(2) += 2.0f;
	vPlacement(1) -= 5.0f;
	...
	// C
	Particle_PrepareTexture(&_toFont, PBT_BLEND);
  Particle_SetTexturePart( 256, 256, 2, 2);
	vPlacement(1) -= 0.2f;
  vPos = vPlacement;
  Particle_RenderSquare( vPos, 0.2f, 0, C_WHITE|CT_OPAQUE);

	// O
	Particle_PrepareTexture(&_toFont, PBT_BLEND);
  Particle_SetTexturePart( 256, 256, 3, 2);
	vPlacement(1) += 0.2f;
  vPos = vPlacement;
  Particle_RenderSquare( vPos, 0.2f, 0, C_WHITE|CT_OPAQUE);
...
#define FADE_IN_LENGHT 0.01f
#define FADE_OUT_LENGHT 1.5f
#define FADE_IN_START 0.0f
#define SPIRIT_SPIRAL_START 0.01f
...
void Particles_Death(CEntity *pen, TIME tmStart)
{
  ...
  for( FLOAT fVtx=0.0f; fVtx<ctVtx; fVtx+=fStep)
  {
    ...
    vPos = 
      ...
      vCenter;
    Particle_RenderSquare( vPos, 0.15f*fColorFactor, 0, col);
  }
...
void Particles_Burning(CEntity *pen, FLOAT fPower, FLOAT fTimeRatio)
{
  ...
  INDEX ctVtx = avVertices.Count()/2;
...
#define CT_AFTERBURNER_SMOKES 10
#define CT_AFTERBURNER_HEAD_POSITIONS 2
#define CT_AFTERBURNER_HEAD_INTERPOSITIONS 2
...
#define SP_EXPLODE_PARTICLES 16
...
#define TM_TWISTER_TOTAL_LIFE 5.0f
...
#define CT_COLLECT_ENERGY_PARTICLES 64
#define CT_PROJECTILE_SPAWN_STARS 12
...
#define CT_IMPLOSION_ENERGY_PARTICLES 36

//#define IMPLOSION_FADE_IN 0.2f
//#define IMPLOSION_FADE_OUT 0.6f
//#define IMPLOSION_TOTAL_TIME 1.0f
//#define IMPLOSION_EXIST_TIME 0.5f

#define TM_IMPLOSIONLIFE 0.75f
//#define CT_IMPLOSION 32

void Particles_Implosion(CEntity *pen, FLOAT tmStart, FLOAT3D vStretch, COLOR colMultiply)//(CEntity *pen, FLOAT tmStart, FLOAT tmStop)
//void Particles_Implosion( CEntity *pen, FLOAT fSize, FLOAT fHeight, INDEX ctParticles, FLOAT fMipFactorDisappear)
{
  ...
  FLOAT fLife = 0.5f;
  INDEX ctRendered = 0;
  FLOAT tmDelta = fLife/CT_IMPLOSION_ENERGY_PARTICLES;
	FLOAT fStretchSize=(vStretch(1)+vStretch(2)+vStretch(3))/3.0f;
  for( INDEX iVtx=0; iVtx<62; iVtx++)
  {
    FLOAT tmFakeStart = tmStart+iVtx*tmDelta;
    FLOAT fPassedTime = fNow-tmFakeStart;
    if(fPassedTime>fLife) continue;
    // calculate fraction part
    FLOAT fT=fPassedTime/fLife;
    fT=fT-INDEX(fT);
    ...
    vRnd(3) *= 40.0f;
    FLOAT3D vSource = vCenter+vRnd;
    FLOAT3D vDestination = vCenter+vRnd*0.1f;
    //vDestination(2) += 40.0f+vRnd(2)/8.0f;
    FLOAT3D vPos, vPos2;
    ...
    FLOAT fSize=(0.1f+afStarsPositions[iRnd][0]*0.15f)*fStretchSize;
    Particle_RenderLine( vPos2, vPos, fSize, colLine);
    ctRendered++;
  }
  ...
  INDEX ctRendered = 0;
  FLOAT tmDelta = fLife/CT_IMPLOSION_ENERGY_PARTICLES;
	FLOAT fStretchSize=(vStretch(1)+vStretch(2)+vStretch(3))/8.0f;
  for( INDEX iVtx=0; iVtx<CT_IMPLOSION_ENERGY_PARTICLES; iVtx++)
  {
    //FLOAT tmFakeStart = tmStart+iVtx*tmDelta;
    FLOAT fPassedTime = fNow-tmStart;
		//CPrintF("fPassedTime = %.3f\n", fPassedTime);
    if(fPassedTime>fLife) continue;
    // calculate fraction part
    FLOAT fT=fPassedTime/fLife;
    fT=fT-INDEX(fT);
		FLOAT fRatio=Clamp(fT/TM_IMPLOSIONLIFE, 0.0f, 1.0f);

    INDEX iRnd = iVtx%CT_MAX_PARTICLES_TABLE;
    FLOAT3D vRnd= FLOAT3D(afStarsPositions[iRnd][0],afStarsPositions[iRnd][1],afStarsPositions[iRnd][2]);
    vRnd *= 50.0f*fStretchSize;
    //vRnd(2) *= 30.0f*fStretchSize;
		//CPrintF("vRnd(2): %.0f\n", vRnd(2));
		//abs(vRnd(2));
    //vRnd(3) *= 30.0f*fStretchSize;
    FLOAT3D vSource = vCenter+vRnd;
    FLOAT3D vDestination = vCenter+vRnd*0.1f;
    FLOAT3D vPos, vPos2;
    // lerp position
    ...
    vPos2 = Lerp( vSource, vDestination, fT2);
		if (vPos(2)<vCenter(2)) vPos(2) = ( vPos(2) - vCenter(2) ) * -1.0f + vCenter(2); 
		if (vPos2(2)<vCenter(2)) vPos2(2) = ( vPos2(2) - vCenter(2) ) * -1.0f + vCenter(2); 
  
    FLOAT fSize = 0.35f;
    //Particle_RenderLine( vPos2, vPos, fSize, colLine);

    UBYTE ubR = 75+afStarsPositions[iRnd][2]*32;
    UBYTE ubG = 100+afStarsPositions[iRnd][1]*32;
    UBYTE ubB = 255;
    COLOR colAlpha = pTD->GetTexel(PIX(ClampUp(fRatio*1024.0f, 1023.0f)), 0);
    COLOR col= RGBToColor( ubR, ubG, ubB) | (colAlpha&0x000000FF);
    col=MulColors(col, colMultiply);
    ...
    //FLOAT fSize=(0.1f+afStarsPositions[iRnd][0]*0.15f)*fStretchSize;
    Particle_RenderLine( vPos, vPos2, fSize, col);
    ctRendered++;
  }

	//CPrintF("Particle_Flush()\n");
  // all done
  Particle_Flush();
 ...
 /* Particle_PrepareTexture( &_toExplosionSpark, PBT_ADDALPHA);
  ...
  for( INDEX iDebris=0; iDebris<CT_IMPLOSION; iDebris++)
  {
    INDEX iRnd =(pen->en_ulID+iDebris+INDEX(tmStart*317309.14521f))%CT_MAX_PARTICLES_TABLE;
    INDEX iRnd2=(pen->en_ulID+iDebris+INDEX(tmStart*421852.46521f))%CT_MAX_PARTICLES_TABLE;
    FLOAT3D vSpeed=FLOAT3D(afStarsPositions[iRnd][0],afStarsPositions[iRnd][1],afStarsPositions[iRnd][2])*1.25f;
  ...
  for( INDEX iVtx=0; iVtx<CT_IMPLOSION_ENERGY_PARTICLES; iVtx++)
  {
    ...
    UBYTE ubR = 155;//+afStarsPositions[iRnd][1]*64;
  ...
  FLOAT fNow = _pTimer->GetLerpedCurrentTick();
  Particle_PrepareTexture( &_toElectricitySparks, PBT_ADDALPHA);
  Particle_SetTexturePart( 512, 1024, 0, 0);
  const FLOATmatrix3D &m = pen->GetRotationMatrix();
  FLOAT3D vY( m(1,2), m(2,2), m(3,2));
  FLOAT3D vCenter = pen->GetLerpedPlacement().pl_PositionVector+vY*fHeight;
  ...
  for( INDEX iStar=0; iStar<ctParticles; iStar++)
  {
    FLOAT fT = fNow+afTimeOffsets[iStar];
    // apply time strech
    fT *= 1/IMPLOSION_TOTAL_TIME;
    // get fraction part
    fT = fT-int(fT);
    FLOAT fF = fT*(IMPLOSION_TOTAL_TIME/IMPLOSION_EXIST_TIME);
    if( fF>1) continue;
    FLOAT fFade;
    if(fF<IMPLOSION_FADE_IN) fFade=fF*(1/IMPLOSION_FADE_IN);
    else if (fF>(1.0f-IMPLOSION_FADE_OUT)) fFade=(1-fF)*(1/IMPLOSION_FADE_OUT);
    else fFade=1.0f;
		//CPrintF("fT=%.3f, fF = %.3f\n", fT, fF);
    ...
    FLOAT3D vPos = vCenter+FLOAT3D(
      ...
      afStarsPositions[iStar][1],
      afStarsPositions[iStar][2])*fSize*(fF+0.4f);
    FLOAT3D vPos2 = vCenter+FLOAT3D(
      afStarsPositions[iStar][0], 
      afStarsPositions[iStar][1],
      afStarsPositions[iStar][2])*fSize*(fF+0.8f);
    
    UBYTE ub = NormFloatToByte( fFade*fDisappearRatio);
    COLOR colStar = RGBToColor( ub, ub, ub>>1);
    //Particle_RenderSquare( vPos, 1.0f, 0, colStar|0xFF);
    FLOAT fSize = 0.25f;
    Particle_RenderLine( vPos2, vPos, fSize, colStar|0xFF);
  }
  // all done
  Particle_Flush();*/
}
...
void Particles_SummonerDisappear( CEntity *pen, FLOAT tmStart)
{
  ...
  INDEX ctVtx = avVertices.Count();
  for( INDEX iVtx=0; iVtx<ctVtx; iVtx+=2)
  {
    INDEX iRnd=iVtx%CT_MAX_PARTICLES_TABLE;
...
void Particles_GrowingSwirl( CEntity *pen, FLOAT fStretch, FLOAT fStartTime)
{
  ...
  INDEX ctStars = (INDEX)((TM_GROWING_SWIRL_FX_LIFE/TM_SWIRL_SPARK_LAUNCH)/2);
...
// NEW CODE KNIFE PROJECTILE TRAIL
#define KNIFE_TRAIL01_POSITIONS 8
#define KNIFE_TRAIL01_INTERPOSITIONS 2
void Particles_KnifeTrail01_Prepare(CEntity *pen)
{
  pen->GetLastPositions(KNIFE_TRAIL01_POSITIONS);
}
void Particles_KnifeTrail01(CEntity *pen)
{
  CLastPositions *plp = pen->GetLastPositions(KNIFE_TRAIL01_POSITIONS);

  Particle_PrepareTexture(&_toKnifeTrail01,  PBT_BLEND);
  Particle_SetTexturePart( 512, 512, 0, 0);
  ...
  for(INDEX iPos = plp->lp_ctUsed-1; iPos>=1; iPos--)
  {
    pvPos1 = pvPos2;
    pvPos2 = &plp->GetPosition(iPos);
    for (INDEX iInter=0; iInter<KNIFE_TRAIL01_INTERPOSITIONS; iInter++) {
      FLOAT3D vPos = Lerp(*pvPos1, *pvPos2, iInter*1.0f/KNIFE_TRAIL01_INTERPOSITIONS);
      FLOAT fRand = rand()/FLOAT(RAND_MAX);
      FLOAT fAngle = fRand*2.0f*PI;
      FLOAT fSize = (KNIFE_TRAIL01_POSITIONS-iPos)*0.05f/KNIFE_TRAIL01_POSITIONS;
      UBYTE ub = 255-iPos*256/KNIFE_TRAIL01_POSITIONS;
      Particle_RenderSquare( vPos, fSize*4, fAngle, RGBToColor(ub,ub,ub)|ub);
    }
  }
  // all done
  Particle_Flush();
                      ...
                      FLOAT fLaunchPower, FLOAT3D vGDir, FLOAT fGA, FLOAT tmStarted, COLOR colMax)
{
  ...
  INDEX ctSprays=(INDEX) (8-Clamp(3.0f-fDamagePower,0.0f,3.0f)*2);
  for( INDEX iSpray=0; iSpray<ctSprays; iSpray++)
  {
    INDEX iFrame=((int(tmStarted*100.0f))%8+iSpray)%8;
    Particle_SetTexturePart( 256, 256, iFrame, 0);
    ...
    Particle_RenderSquare( vPos, fSize/2, fRotation, col);
  }
  // all done
  Particle_Flush();
}
...
void Particles_ModelGlow3( CEntity *pen, enum ParticleTexture ptTexture, FLOAT fSize, FLOAT iVtxStep, COLOR iCol)
{
  FLOAT fMipFactor = Particle_GetMipFactor()+2;
  ...
  {
    FLOAT3D vPos = avVertices[iVtx];
    Particle_RenderSquare( vPos, fSize, 0, iCol|255);    
  }
...
#define DK_AFTERBURNER_SMOKES 10
void Particles_RunAfterBurner_Prepare(CEntity *pen)
{
  pen->GetLastPositions(DK_AFTERBURNER_SMOKES);
}

void Particles_RunAfterBurner(CEntity *pen, FLOAT tmEnd, FLOAT fStretch, INDEX iGradientType)
{
  FLOAT3D vGDir = ((CMovableEntity *)pen)->en_vGravityDir*-5.0f;
  FLOAT fGA = ((CMovableEntity *)pen)->en_fGravityA*5.0f;

  CLastPositions *plp = pen->GetLastPositions(DK_AFTERBURNER_SMOKES);
  Particle_PrepareTexture(&_toAfterBurner,  PBT_BLEND);
  ...
  for(INDEX iPos = plp->lp_ctUsed-1; iPos>=1; iPos--) {
    ...
    FLOAT fT = (iPos+_pTimer->GetLerpFactor())*_pTimer->TickQuantum;
    FLOAT fRatio = fT/(DK_AFTERBURNER_SMOKES*_pTimer->TickQuantum);
    INDEX iIndex = (INDEX) (fRatio*255);
    INDEX iRnd = (INDEX)(size_t(pvPos1)%CT_MAX_PARTICLES_TABLE);
    ...
    Particle_SetTexturePart( 512, 512, 1, 0);
    FLOAT fAngleS = afStarsPositions[iRnd][1]*360.0f+fT*120.0f*afStarsPositions[iRnd][2];
    FLOAT fSizeS = (0.1f+iPos*1.0f)*fStretch;
    ...
    Particle_SetTexturePart( 512, 512, 0, 0);
    FLOAT fAngleE = afStarsPositions[iRnd][0]*360.0f;//+fT*360.0f;
    FLOAT fSizeE = (0.1f+iPos*0.8f)*fStretch;
    FLOAT3D vVelocityE = FLOAT3D(afStarsPositions[iRnd][0], 
																 afStarsPositions[iRnd][1],
																 afStarsPositions[iRnd][2])*4.0f;
    vPosE = vPosE+vVelocityE*fT+vGDir*fGA/2.0f*(fT*fT)/32.0f;
...
void Particles_Fireworks00(CEmiter2 &em)
{
  Particle_PrepareTexture(&_toFireworks01, PBT_ADDALPHA);
  ...
  for(INDEX i=0; i<em.em_aepParticles.Count(); i++)
  {
    const CEmittedParticle2 &ep=em.em_aepParticles[i];

    if(ep.ep_tmEmitted<0) continue;
		Particle_SetTexturePart( 512, 512, 0, 0);
    FLOAT3D vPos=Lerp(ep.ep_vLastPos, ep.ep_vPos, fLerpFactor);
    FLOAT fRot=Lerp(ep.ep_fLastRot, ep.ep_fRot, fLerpFactor);
		COLOR col=ep.ep_colColor;
    Particle_RenderSquare( vPos, ep.ep_fStretch, fRot, col);

		if(ep.ep_tmEmitted+0.01f>_pTimer->GetLerpedCurrentTick()) continue;
		col=0xAB97B000|CT_OPAQUE;
		Particle_SetTexturePart( 512, 512, 2, 0);
		Particle_RenderLine( ep.ep_vLastPos1, vPos, ep.ep_fStretch*0.8f, col);

		if(ep.ep_tmEmitted+0.02f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos2, ep.ep_vLastPos, ep.ep_fStretch*0.7f, col);

		if(ep.ep_tmEmitted+0.03f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos3, ep.ep_vLastPos1, ep.ep_fStretch*0.6f, col);

		if(ep.ep_tmEmitted+0.04f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos4, ep.ep_vLastPos2, ep.ep_fStretch*0.5f, col);

		if(ep.ep_tmEmitted+0.05f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos5, ep.ep_vLastPos3, ep.ep_fStretch*0.4f, col);

		if(ep.ep_tmEmitted+0.06f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos6, ep.ep_vLastPos4, ep.ep_fStretch*0.3f, col);
  }
  // all done
  Particle_Flush();
}

void Particles_Fireworks01(CEmiter2 &em)
{
  Particle_PrepareTexture(&_toFireworks01, PBT_ADDALPHA);
...
void Particles_Fireworks02(CEmiter2 &em)
{
  Particle_PrepareTexture(&_toFireworks01, PBT_ADDALPHA);
  ...
  for(INDEX i=0; i<em.em_aepParticles.Count(); i++)
  {
    ...
    Particle_RenderSquare( vPos, ep.ep_fStretch, fRot, col);
		...
		if(ep.ep_tmEmitted+0.02f>_pTimer->GetLerpedCurrentTick()) continue;
		col = AdjustColor(col, -1, 0);
		//col = AdjustGamma( col, 40.0f);
		Particle_SetTexturePart( 512, 512, 2, 0);
		Particle_RenderLine( ep.ep_vLastPos1, vPos, ep.ep_fStretch*0.8f, col);

		if(ep.ep_tmEmitted+0.04f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos2, ep.ep_vLastPos, ep.ep_fStretch*0.75f, col);

		if(ep.ep_tmEmitted+0.06f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos3, ep.ep_vLastPos1, ep.ep_fStretch*0.7f, col);

		if(ep.ep_tmEmitted+0.08f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos4, ep.ep_vLastPos2, ep.ep_fStretch*0.65f, col);

		if(ep.ep_tmEmitted+0.1f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos5, ep.ep_vLastPos3, ep.ep_fStretch*0.6f, col);

		if(ep.ep_tmEmitted+0.12f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos6, ep.ep_vLastPos4, ep.ep_fStretch*0.55f, col);

		if(ep.ep_tmEmitted+0.14f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos7, ep.ep_vLastPos5, ep.ep_fStretch*0.5f, col);

		if(ep.ep_tmEmitted+0.16f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos8, ep.ep_vLastPos6, ep.ep_fStretch*0.45f, col);
  }
  // all done
  Particle_Flush();
}

void Particles_Fireworks03(CEmiter2 &em)
{
  Particle_PrepareTexture(&_toFireworks01, PBT_ADDALPHA);
  ...
  for(INDEX i=0; i<em.em_aepParticles.Count(); i++)
  {
    ...
    FLOAT fRot=Lerp(ep.ep_fLastRot, ep.ep_fRot, fLerpFactor);
    INDEX iIndex=INDEX((tmNow-ep.ep_tmEmitted)*2.0f/(ep.ep_tmLife)*255.0f)%255;
    COLOR col=MulColors(ByteSwap(pcol[iIndex]), MulColors(ep.ep_colColor, em.em_colGlobal));//ep.ep_colColor;
    Particle_RenderSquare( vPos, ep.ep_fStretch, fRot, col);
		...
		if(ep.ep_tmEmitted+0.07f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos7, ep.ep_vLastPos5, ep.ep_fStretch*0.5f, col);
...
void Particles_Fireworks04(CEmiter2 &em)
{
  Particle_PrepareTexture(&_toFireworks01, PBT_ADDALPHA);
  ...
  for(INDEX i=0; i<em.em_aepParticles.Count(); i++)
  {
    ...
    Particle_RenderSquare( vPos, ep.ep_fStretch, fRot, col);
		...
		col = 0xFEAE6000|255;
		Particle_SetTexturePart( 512, 512, 2, 0);
		Particle_RenderLine( ep.ep_vLastPos4, vPos, ep.ep_fStretch*0.8f, col);

		if(ep.ep_tmEmitted+0.12f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos6, ep.ep_vLastPos, ep.ep_fStretch*0.7f, col);

		if(ep.ep_tmEmitted+0.14f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( ep.ep_vLastPos8, ep.ep_vLastPos2, ep.ep_fStretch*0.6f, col);
  }
  // all done
  Particle_Flush();
}

void Particles_Fireworks05(CEmiter2 &em)
{
  Particle_PrepareTexture(&_toFireworks01, PBT_ADDALPHA);
...
void Particles_Fireworks06(CEmiter2 &em)
{
  Particle_PrepareTexture(&_toFireworks01, PBT_ADDALPHA);
  ...
  for(INDEX i=0; i<em.em_aepParticles.Count(); i++)
  {
    ...
    if(ep.ep_tmEmitted<0 || tmNow>ep.ep_tmEmitted+ep.ep_tmLife) continue;
		Particle_SetTexturePart( 512, 512, 0, 0);
    FLOAT3D vPos=Lerp(ep.ep_vLastPos, ep.ep_vPos, fLerpFactor);
    FLOAT fRot=Lerp(ep.ep_fLastRot, ep.ep_fRot, fLerpFactor);
    COLOR col=0x4B2F8600|CT_OPAQUE;
    Particle_RenderSquare( vPos, ep.ep_fStretch, fRot, col);
...
void Particles_Fireworks07(CEmiter2 &em)
{
  Particle_PrepareTexture(&_toFireworks01, PBT_ADDALPHA);
  ...
  for(INDEX i=0; i<em.em_aepParticles.Count(); i++)
  {
    ...
    if(ep.ep_tmEmitted<0 || tmNow>ep.ep_tmEmitted+ep.ep_tmLife) continue;
		Particle_SetTexturePart( 512, 512, 3, 0);
    FLOAT3D vPos=Lerp(ep.ep_vLastPos, ep.ep_vPos, fLerpFactor);
    FLOAT fRot=Lerp(ep.ep_fLastRot, ep.ep_fRot, fLerpFactor);
    INDEX iIndex=INDEX((tmNow-ep.ep_tmEmitted)*2.0f/(ep.ep_tmLife)*255.0f)%255;
    COLOR col=MulColors(ByteSwap(pcol[iIndex]), MulColors(ep.ep_colColor, em.em_colGlobal));//ep.ep_colColor;
...
void Particles_Fireworks08(CEmiter2 &em)
{
  Particle_PrepareTexture(&_toFireworks02, PBT_ADDALPHA);
  ...
  for(INDEX i=0; i<em.em_aepParticles.Count(); i++)
  {
    ...
    INDEX iIndex=INDEX((tmNow-ep.ep_tmEmitted)*2.0f/(ep.ep_tmLife)*255.0f)%255;
    COLOR col=ep.ep_colColor;
    //Particle_RenderSquare( vPos, ep.ep_fStretch*0.9f, fRot, col);
		Particle_RenderLine( ep.ep_vLastPos1, vPos, ep.ep_fStretch*0.85f, col);

		if(ep.ep_tmEmitted+0.15f>_pTimer->GetLerpedCurrentTick()) continue;
		col = 0xFFFAF500|180;
		Particle_SetTexturePart( 512, 512, 1, 0);
		Particle_RenderLine( ep.ep_vLastPos2, vPos, ep.ep_fStretch*0.8f, col);

		if(ep.ep_tmEmitted+0.17f>_pTimer->GetLerpedCurrentTick()) continue;
		col = 0xFFE1C300|180;
		Particle_RenderLine( ep.ep_vLastPos3, ep.ep_vLastPos, ep.ep_fStretch*0.8f, col);

		if(ep.ep_tmEmitted+0.19f>_pTimer->GetLerpedCurrentTick()) continue;
		col = 0xFFC79100|180;
		Particle_SetTexturePart( 512, 512, 0, 1);
		Particle_RenderLine( ep.ep_vLastPos4, ep.ep_vLastPos1, ep.ep_fStretch*0.8f, col);

		if(ep.ep_tmEmitted+0.21f>_pTimer->GetLerpedCurrentTick()) continue;
		col = 0xFEAE6000|180;
		Particle_RenderLine( ep.ep_vLastPos5, ep.ep_vLastPos2, ep.ep_fStretch*0.8f, col);

		if(ep.ep_tmEmitted+0.23f>_pTimer->GetLerpedCurrentTick()) continue;
		col = 0xFD952F00|180;
		Particle_RenderLine( ep.ep_vLastPos6, ep.ep_vLastPos3, ep.ep_fStretch*0.8f, col);

		if(ep.ep_tmEmitted+0.26f>_pTimer->GetLerpedCurrentTick()) continue;
		col = 0xF77C0300|180;
		Particle_SetTexturePart( 512, 512, 1, 1);
		Particle_RenderLine( ep.ep_vLastPos7, ep.ep_vLastPos4, ep.ep_fStretch*0.8f, col);

		if(ep.ep_tmEmitted+0.29f>_pTimer->GetLerpedCurrentTick()) continue;
		col = 0xC6630200|180;
		Particle_RenderLine( ep.ep_vLastPos8, ep.ep_vLastPos5, ep.ep_fStretch*0.8f, col);

		/*if(ep.ep_tmEmitted+0.18f>_pTimer->GetLerpedCurrentTick()) continue;
		col = 0x7B686A00|CT_OPAQUE;
		Particle_RenderLine( ep.ep_vLastPos8, ep.ep_vLastPos6, ep.ep_fStretch*0.45f, col);*/
  }
  // all done
  Particle_Flush();
...
void Particles_Fireworks09(CEmiter2 &em)
{
  Particle_PrepareTexture(&_toFireworks02, PBT_ADDALPHA);
...
void Particles_Firework00(CEntity *pen, FLOAT tmStart, FLOAT fSize, COLOR colMultiply)
{
  Particle_PrepareTexture(&_toFireworks01, PBT_ADDALPHA);
  ...
  //FLOAT fLerpFactor=_pTimer->GetLerpFactor();
	CLastPositions *plp = pen->GetLastPositions(8);
  for(INDEX iPos=0; iPos<7; iPos++)
  {
		Particle_SetTexturePart( 512, 512, 0, 0);
    FLOAT3D vPos1 = plp->GetPosition(0);
		FLOAT3D vPos2 = plp->GetPosition(1);
		FLOAT3D vPos3 = plp->GetPosition(2);
		FLOAT3D vPos4 = plp->GetPosition(3);
		FLOAT3D vPos5 = plp->GetPosition(4);
		FLOAT3D vPos6 = plp->GetPosition(5);
		FLOAT3D vPos7 = plp->GetPosition(6);
		FLOAT3D vPos8 = plp->GetPosition(7);

    FLOAT fRot = 0.0f;
		COLOR col = colMultiply;

    Particle_RenderSquare( plp->GetPosition(0), fSize, fRot, col);

		if(tmStart+0.01f>_pTimer->GetLerpedCurrentTick()) continue;
		col=0xAB97B000|CT_OPAQUE;
		Particle_SetTexturePart( 512, 512, 2, 0);
		Particle_RenderLine( vPos3, vPos1, fSize*0.8f, col);

		if(tmStart+0.02f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( vPos4, vPos2, fSize*0.7f, col);

		if(tmStart+0.03f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( vPos5, vPos3, fSize*0.6f, col);

		if(tmStart+0.04f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( vPos6, vPos4, fSize*0.5f, col);

		if(tmStart+0.05f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( vPos7, vPos5, fSize*0.4f, col);

		if(tmStart+0.06f>_pTimer->GetLerpedCurrentTick()) continue;
		Particle_RenderLine( vPos8, vPos6, fSize*0.3f, col);
  }
  // all done
  Particle_Flush();
}

void Particles_Firework01(CEntity *pen, FLOAT tmStart, FLOAT fSize, COLOR colMultiply)
{
}

void Particles_Firework02(CEntity *pen, FLOAT tmStart, FLOAT fSize, COLOR colMultiply)
{
}

void Particles_Firework03(CEntity *pen, FLOAT tmStart, FLOAT fSize, COLOR colMultiply)
{
}

void Particles_Firework04(CEntity *pen, FLOAT tmStart, FLOAT fSize, COLOR colMultiply)
{
}

void Particles_Firework05(CEntity *pen, FLOAT tmStart, FLOAT fSize, COLOR colMultiply)
{
}

void Particles_Firework06(CEntity *pen, FLOAT tmStart, FLOAT fSize, COLOR colMultiply)
{
}

void Particles_Firework07(CEntity *pen, FLOAT tmStart, FLOAT fSize, COLOR colMultiply)
{
}

void Particles_Firework08(CEntity *pen, FLOAT tmStart, FLOAT fSize, COLOR colMultiply)
{
}

void Particles_Firework09(CEntity *pen, FLOAT tmStart, FLOAT fSize, COLOR colMultiply)
{
}

void Particles_PlasmaBall(CEntity *pen, FLOAT fSize, FLOAT tmStarted)
{
	Particle_PrepareTexture(&_toPlasmaBall, PBT_ADDALPHA);
	Particle_SetTexturePart( 256, 256, 0, 0);

	CLastPositions *plp = pen->GetLastPositions(1);

  FLOAT fNow = _pTimer->GetLerpedCurrentTick();
  FLOAT fT = (fNow-tmStarted);
	FLOAT fRotation = fT*100.0f;

	Particle_RenderSquare( plp->GetPosition(0), fSize, fRotation, C_lBLUE|CT_OPAQUE);
  // all done
  Particle_Flush();
}

#define HPB_AFTERBURNER_SMOKES 5
void Particles_PlasmaTrail_Prepare(CEntity *pen)
{
  pen->GetLastPositions(HPB_AFTERBURNER_SMOKES);
}

void Particles_PlasmaTrail(CEntity *pen, FLOAT fStretch)
{
  FLOAT3D vGDir = ((CMovableEntity *)pen)->en_vGravityDir;
  FLOAT fGA = ((CMovableEntity *)pen)->en_fGravityA;

  CLastPositions *plp = pen->GetLastPositions(HPB_AFTERBURNER_SMOKES);
  const FLOAT3D *pvPos1;
  const FLOAT3D *pvPos2 = &plp->GetPosition(plp->lp_ctUsed-1);

  Particle_PrepareTexture(&_toPlasmaSmoke,  PBT_ADDALPHA);
	Particle_SetTexturePart( 256, 256, 0, 0);
  ...
  for(INDEX iPos = plp->lp_ctUsed-1; iPos>=1; iPos--)
  {
    ...
    FLOAT fT=(iPos+_pTimer->GetLerpFactor())*_pTimer->TickQuantum;
    FLOAT fRatio=fT/(HPB_AFTERBURNER_SMOKES*_pTimer->TickQuantum);
    INDEX iIndex=fRatio*155;
    INDEX iRnd=(INDEX)(size_t(pvPos1)%CT_MAX_PARTICLES_TABLE);
    INDEX iRndTex = iRnd*324561+pen->en_ulID;  
		INDEX iAlpha = 60-iPos*10;
    // smoke
    FLOAT3D vPosS = *pvPos1;
    FLOAT fAngleS = afStarsPositions[iRnd][2]*360.0f+fT*120.0f*afStarsPositions[iRnd][3];
    FLOAT fSizeS = ((1.5f+iPos)*1.5f)*fStretch;
    FLOAT3D vVelocityS=FLOAT3D(afStarsPositions[iRnd][2], 
                               afStarsPositions[iRnd][3],
                               afStarsPositions[iRnd][1])*2.0f;
    vPosS=vPosS+vVelocityS*fT+vGDir*fGA/2.0f*(fT*fT)/32.0f;
		Particle_RenderSquare( vPosS, fSizeS, fAngleS, C_vlBLUE|iAlpha);

    FLOAT3D vPosE = (*pvPos1+*pvPos2)/2.0f;
		iRnd=(INDEX)(size_t(pvPos2)%CT_MAX_PARTICLES_TABLE);
    iRndTex = iRnd*324561+pen->en_ulID;
    FLOAT fAngleE = afStarsPositions[iRnd][0]*360.0f;
    FLOAT3D vVelocityE=FLOAT3D(afStarsPositions[iRnd][0], 
                               afStarsPositions[iRnd][1],
                               afStarsPositions[iRnd][2])*1.0f;
    vPosE=vPosE+vVelocityE*fT+vGDir*fGA/2.0f*(fT*fT)/32.0f;
		iAlpha -= 10;
		Particle_RenderSquare( vPosE, fSizeS, fAngleE, C_vlBLUE|iAlpha);
  }
  // all done
  Particle_Flush();
}
