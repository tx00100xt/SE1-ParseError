...
void GetEntityInfoPosition(CEntity *pen, FLOAT *pf, FLOAT3D &vPos) {
  ...
  if (pf != NULL) { // bot as enemy target crash
...
void SendInRange(CEntity *penSource, EventEType eetEventType, const FLOATaabbox3D &boxRange) {
  switch (eetEventType) {
    ...
    case EET_STOPDEAFNESS:
      ...
      break;
    case EET_BECOMEACTIVE:
      penSource->SendEventInRange(EBecomeActive(), boxRange);
      break;
  }
};
...
CEntityPointer SpawnReminder(CEntity *penOwner, FLOAT fWaitTime, INDEX iValue) {
  ...
  try {
    ...
    FatalError(TRANS("Cannot create reminder entity class: %s"), (const char *)strError);
  }
  EReminderInit eri;
...
CEntityPointer SpawnFlame(CEntity *penOwner, CEntity *penAttach, const FLOAT3D &vSource)
{
  ...
  if( penOwner==penAttach || IsOfClass(penAttach, "Player" ) 
    || IsOfClass(penAttach, "PlayerBot" )) return NULL;
  ...
  const FLOAT3D &vPosRatio)
{
  ...
  eSpawn.iFadeOrNot = iFadeOrNot;
  eSpawn.iModelAnim = iModelAnim;
  eSpawn.colDebris = _colDebris;
  eSpawn.vStretch = FLOAT3D(1,1,1);

  // if End Show Big head, make the debris last longer....
  if (IsDerivedFromClass(penSpawner, "ESBigHead")) {
    eSpawn.bExtendLife = TRUE;
  } else {
    eSpawn.bExtendLife = FALSE;
  }
...
FLOAT GetGameDamageMultiplier(void)
{
  ...
  //FLOAT fGameDamageMultiplier = 0.8f;
...
{
  if( !(IsOfClass(pen,"Player") || IsOfClass(pen,"PlayerBot"))) return 1.0f;
  FLOAT fDamage = GetSP()->sp_fDamageStrength*1.0f*GetSP()->sp_fWeaponsDamage;
  FLOAT fDamageSer = GetSP()->sp_fDamageStrength*4.0f*GetSP()->sp_fWeaponsDamage;
  TIME tmNow = _pTimer->CurrentTick();
  TIME tmDamage = 0.0f;
  if ( IsOfClass(pen,"Player")) {
    tmDamage = ((CPlayer*)pen)->m_tmSeriousDamage;
  }
  if ( IsOfClass(pen, "PlayerBot")) {
    tmDamage = ((CPlayerBot*)pen)->m_tmSeriousDamage;
  }
  if( tmDamage>tmNow) return fDamageSer;
  return fDamage;
}
...
INDEX GetGameEnemyMultiplier(void)
{
  // get initial enemy multiplier
  INDEX iEnemyX = GetSP()->sp_iEnemyMultiplier;
  // make sure it is at least 2......
  if (iEnemyX<=2) {
		iEnemyX = 2;
  }
  // if using extra per player
  if (GetSP()->sp_bExtraEnemiesPerPlayer) {
    // get info and calculate extra per player
    FLOAT fPerPlayer = GetSP()->sp_fExtraEnemiesPerPlayerRatio;
    INDEX ctPlayers = _pNetwork->ga_sesSessionState.GetPlayersCount();
    INDEX iExtraPerPlayer = Clamp(INDEX(ctPlayers/fPerPlayer), INDEX(0), INDEX(20));
	  iEnemyX += iExtraPerPlayer;
  }
...
//	Speed Multipler
FLOAT GetGameSpeedAdjuster(void)
{
	FLOAT fSpeedAdjust = 1.0f;
	FLOAT fSpeedMultiplier = GetSP()->sp_fSpeedAdjuster;
	if (fSpeedMultiplier>1.0f) {
      fSpeedAdjust = fSpeedMultiplier;
	}
	return fSpeedAdjust;
}
...
void ParseCPP(void)
{
  ...
  fn.PrintF("Files\\%s.cpp", strFileName);
	...
	if (FileExists(fn)) {
		CTString strLine;
		CTString strLineShort;
		CTFileStream strm;

		// try to
		try {
			CPrintF("^cff0000Opening CPP File...\n");
			// open the file
			strm.Open_t(fn);
		// if anything failed
		} catch (const char *strError) {
		...
		FOREVER {
			// read one line
			strLine = GetLine(strm);    
			// if end of file
			if (strLine == "****") {
				// finish parsing
				break;
			}
			strLineShort = strLine;
	...
	CPrintF("^cff0000DONE -- Parsing CPP File...\n");
}

CTString GetLine(CTStream &strm)
{
  FOREVER {
   if(strm.AtEOF()) {
     ThrowF_t(TRANS("Unexpected end of file"));
...
/*CTString g_strLevelName;
void SetLevelString(void)
{
	g_strLevelName = _pNetwork->ga_fnmWorld.FileName();
}

FLOAT GetLagFactor(void)
{
	FLOAT fLag = 0.0f;
  CDynamicContainer<CEntity> cenEntities;
  // for each entity in the world
	FOREACHINDYNAMICCONTAINER(cenEntities, CEntity, iten) {
		CEntity *pen = iten;
		if (IsDerivedFromClass(pen, "Enemy Base")) {
			CEnemyBase *penEnemy = (CEnemyBase *)pen;
			if (penEnemy->m_bHasBeenSpawned) {
        if (penEnemy->m_bActive) {
					fLag += penEnemy->m_fLagger;
        } else {
          fLag += penEnemy->m_fLagger/2;
        }
      }
    }
  }
