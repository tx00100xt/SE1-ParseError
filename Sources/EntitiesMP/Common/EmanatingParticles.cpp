...
#define ID_EMITER2_VER "EMT1"

CEmittedParticle2::CEmittedParticle2(void)
{
  ep_tmEmitted=-1;
  ep_tmLife=1.0f;
  ep_colColor=C_WHITE|CT_OPAQUE;
...
void CEmittedParticle2::Write_t( CTStream &strm)
{
  strm.WriteRawChunk_t( this, sizeof(CEmittedParticle2));
}

void CEmittedParticle2::Read_t( CTStream &strm)
{
  strm.ReadRawChunk_t( this, sizeof(CEmittedParticle2));
}

CEmiter2::CEmiter2(void)
{
  em_et2Type=ET_FIREWORKS00;
  em_bInitialized=FALSE;
  em_tmStart=-1;
  em_tmLife=0;
  em_vG=FLOAT3D(0,1,0);
...
void CEmiter2::Initialize(CEntity *pen)
{
  em_vG=GetGravity(pen);
  em_bInitialized=TRUE;
  em_tmStart=_pTimer->CurrentTick();
...
FLOAT3D CEmiter2::GetGravity(CEntity *pen)
{
  if(pen->GetPhysicsFlags()&EPF_MOVABLE)
  {
    return ((CMovableEntity *)pen)->en_vGravityDir*
...
void CEmiter2::AddParticle(FLOAT3D vPos, FLOAT3D vSpeed, FLOAT fRot, FLOAT fRotSpeed, 
                          FLOAT tmBirth, FLOAT tmLife, FLOAT fStretch, COLOR colColor, BOOL bPrimary)
{
  CEmittedParticle2 &em=em_aepParticles.Push();
  em.ep_fLastRot=fRot;
  em.ep_fRot=fRot;
  em.ep_fRotSpeed=fRotSpeed;
	em.ep_vLastPos8=vPos;
	em.ep_vLastPos7=vPos;
	em.ep_vLastPos6=vPos;
	em.ep_vLastPos5=vPos;
	em.ep_vLastPos4=vPos;
	em.ep_vLastPos3=vPos;
	em.ep_vLastPos2=vPos;
	em.ep_vLastPos1=vPos;
  em.ep_vLastPos=vPos;
  em.ep_vPos=vPos;
  em.ep_vSpeed=vSpeed;
  em.ep_colLastColor=colColor;
  ...
  em.ep_tmLife=tmLife;
  em.ep_fStretch=fStretch;
	em.ep_bPrimary = bPrimary;
}

void CEmiter2::AnimateParticles(void)
{
  FLOAT tmNow=_pTimer->CurrentTick();
  INDEX ctCount=em_aepParticles.Count();
  INDEX iCurrent=0;
	//CPrintF("iCurrent %d, ctCount %d\n", iCurrent, ctCount);
  while( iCurrent<ctCount)
  {
    CEmittedParticle2 &ep=em_aepParticles[iCurrent];
    // not yet alive
    if(ep.ep_tmEmitted<0)
    {
      iCurrent++;
    ...
    {
      CEmittedParticle2 &emLast=em_aepParticles[ctCount-1];
      ep=emLast;
      ctCount--;
    }
    // it is alive, animate it
    ...
    {
      // animate position
			ep.ep_vLastPos8=ep.ep_vLastPos7;
			ep.ep_vLastPos7=ep.ep_vLastPos6;
			ep.ep_vLastPos6=ep.ep_vLastPos5;
			ep.ep_vLastPos5=ep.ep_vLastPos4;
			ep.ep_vLastPos4=ep.ep_vLastPos3;
			ep.ep_vLastPos3=ep.ep_vLastPos2;
			ep.ep_vLastPos2=ep.ep_vLastPos1;
			ep.ep_vLastPos1=ep.ep_vLastPos;
      ep.ep_vLastPos=ep.ep_vPos;
      ep.ep_vSpeed=ep.ep_vSpeed+em_vG*_pTimer->TickQuantum;
      ep.ep_vPos=ep.ep_vPos+ep.ep_vSpeed*_pTimer->TickQuantum;
      // animate rotation
...
void CEmiter2::RenderParticles(void)
{
  switch(em_et2Type)
  {
  case ET_FIREWORKS00:
    Particles_Fireworks00(*(CEmiter2*)this);
    break;
  case ET_FIREWORKS01:
    Particles_Fireworks01(*(CEmiter2*)this);
    break;
  case ET_FIREWORKS02:
    Particles_Fireworks02(*(CEmiter2*)this);
    break;
  case ET_FIREWORKS03:
    Particles_Fireworks03(*(CEmiter2*)this);
    break;
  case ET_FIREWORKS04:
    Particles_Fireworks04(*(CEmiter2*)this);
    break;
  case ET_FIREWORKS05:
    Particles_Fireworks05(*(CEmiter2*)this);
    break;
  case ET_FIREWORKS06:
    Particles_Fireworks06(*(CEmiter2*)this);
    break;
  case ET_FIREWORKS07:
    Particles_Fireworks07(*(CEmiter2*)this);
    break;
  case ET_FIREWORKS08:
    Particles_Fireworks08(*(CEmiter2*)this);
    break;
  case ET_FIREWORKS09:
    Particles_Fireworks09(*(CEmiter2*)this);
    break;
  default:
    {
    }
...
void CEmiter2::Read_t( CTStream &strm)
{
  if (strm.PeekID_t()!=CChunkID(ID_EMITER2_VER)) return;
  strm.GetID_t();
  INDEX ctMaxParticles;
  strm>>ctMaxParticles;
  ...
  INDEX ietType;
  strm>>ietType;
  em_et2Type=(CEmiter2Type) ietType;
  strm>>em_tmStart;
  strm>>em_tmLife;
  strm>>em_vG;
  strm>>em_colGlobal;
  ...
  for(INDEX i=0; i<em_aepParticles.Count(); i++)
  {
    CEmittedParticle2 &em=em_aepParticles[i];
    em.Read_t(strm);
  }
}

void CEmiter2::Write_t( CTStream &strm)
{
  if( !em_bInitialized) return;
  INDEX ctMaxParticles=em_aepParticles.Count();
  strm.WriteID_t(CChunkID(ID_EMITER_VER));
