...
//      iAnim = STONEMAN_ANIM_DEATH03;
  ...
  {
    INDEX ctShouldSpawn = Clamp( INDEX((m_fMaxHealth-GetHealth())/m_fSpawnDamage), INDEX(0), INDEX(4));
    ...
    ((CElemental&)*pen).m_bHasBeenSpawned = TRUE;
    ((CElemental&)*pen).m_tmSpawned = _pTimer->CurrentTick();
...
*/
  ...
  void HitGround(void) {
    ...
    // damage
    if (m_EecChar==ELC_LARGE) {
      InflictRangeDamage(this, DMT_EXPLOSION, 150.0f, vSource, 7.5f, m_fCloseDistance);
//      ShakeItBaby(_pTimer->CurrentTick(), 5.0f);
//      SpawnShockwave(10.0f);
    } else if (m_EecChar==ELC_BIG) {
      InflictRangeDamage(this, DMT_EXPLOSION, 75.0f, vSource, 5.0f, m_fCloseDistance);
//      ShakeItBaby(_pTimer->CurrentTick(), 2.0f);
//      SpawnShockwave(3.0f);
    } else {
      InflictRangeDamage(this, DMT_EXPLOSION, 25.0f, vSource, 2.5f, m_fCloseDistance);
//      SpawnShockwave(1.0f);
    }
  };
