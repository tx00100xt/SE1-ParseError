...
enum DestinationType {
  0 DT_PLAYERCURRENT	"Player Current",    // go to where player is now
  1 DT_PLAYERSPOTTED	"Player Spotted",    // go to where player was last seen
  ...
  3 DT_PATHPERSISTENT "Path Persistent",    // go to navigation marker - until you really get there
	4 DT_PATHPAUSED     "Path Paused",    // pause until we can look for another marker
};
...
// NEW
200 BOOL  m_bCountAsEvent = TRUE,
201 FLOAT m_fLagger = 1.0f,
202 BOOL  m_bActive = FALSE,  // external active flag
203 FLOAT m_tmSpawned = 0.0f,   // when enemy was spawned
204 FLOAT m_fSize = 0.0f,
205 BOOL  m_bHasBeenSpawned = FALSE,  // whether or not this enemy has been spawned or is already existing in the map
206 BOOL  m_bIsDummy = FALSE, // whether or not this is a dummy enemy for purging purposes
207 INDEX m_iCount = 0,
208 BOOL  m_bUseTactics = FALSE,
209 FLOAT m_fTactic1 = 0.0f,
210 FLOAT m_fTactic2 = 0.0f,
211 FLOAT m_fTactic3 = 0.0f,
212 FLOAT m_fTactic4 = 0.0f,
213 FLOAT m_fTactic5 = 0.0f,      
214 INDEX m_tctType = 0, 
215 BOOL  m_bPESpawned = FALSE,
216 INDEX m_iFixupCount = 0,
217 BOOL  m_bKick = FALSE,
218 BOOL  m_bTrapped = FALSE,
219 INDEX m_iTrappedCount = 0,
220 CEntityPointer m_penBotTargeted,
221 CEntityPointer m_penSpawnerTarget2,
222 BOOL  m_bFrozen = FALSE,
223 CEntityPointer m_penSpawnerBattery,
224 FLOAT m_tmSlowDown = 0.0f,
225 FLOAT m_fMoveSpeedSlow = 0.0f,
//226 FLOAT m_fRed = 0.0f,
//227 FLOAT m_fGreen = 0.0f,
//228 FLOAT m_fBlue = 0.0f,
229 FLOAT m_fParticleSize = 1.0f,
230 COLOR m_colStar = C_WHITE,
231 BOOL  m_bMovingThruMarkers = FALSE,
232 BOOL  m_bCannotReachPlayer = FALSE,
233 FLOAT m_tmPause = 0.0f,
234 BOOL  m_bRandomSize = TRUE,
235 FLOAT m_fLastDistance = 0.0f,
236 BOOL  m_bAttacking = FALSE,
237 FLOAT3D m_vLastPosition = FLOAT3D(0, 0, 0),
...
//*/
  ...
  void SwitchToAnotherPlayer(void)
  {
    // if in single player
    if (GetSP()->sp_bSinglePlayer) {
      // no need to check
  ...
  {
    // if template or if frozen
    if (m_bTemplate || m_bFrozen) {
      // do nothing
      return;
    }

// NEW CODE XTRA SPAWNER FIXER
    FLOAT fAge = _pTimer->CurrentTick()-m_tmSpawned;
    FLOAT fInvuln = 0.15f;
    if (m_bPESpawned) {
      fInvuln = 0.4f;
    }
...
//*/
    ...
    {
      m_vDamage+=(vDirectionFixed/2-en_vGravityDir/2)*fKickDamage*2;
    } else if( dmtType == DMT_CANNONBALL ) {
      m_vDamage+=(vDirectionFixed/2-en_vGravityDir/2)*fKickDamage*3;
    } else {
    ...
    if (dmtType==DMT_ACID || dmtType==DMT_EXPLOSION || dmtType==DMT_CANNONBALL) {
      m_fSprayDamage = 0.0f;
    } else {
      if ((m_tmSpraySpawned<=_pTimer->CurrentTick()-_pTimer->TickQuantum*8 || 
        ...
        if( m_fMaxDamageAmmount > 10.0f) {
          eSpawnSpray.fDamagePower = 1.8f;
        } else if(m_fSprayDamage+fNewDamage>50.0f) {
          eSpawnSpray.fDamagePower = 2.8f;
        } else {
          eSpawnSpray.fDamagePower = 1.5f;
        }
...
//*/
  ...
  BOOL AdjustShadingParameters(FLOAT3D &vLightDirection, COLOR &colLight, COLOR &colAmbient)
  {
    ...
    if( m_bFadeOut) {
      FLOAT fTimeRemain = m_fFadeStartTime + m_fFadeTime - _pTimer->CurrentTick();
      if( fTimeRemain < 0.0f) { fTimeRemain = 0.0f; }
			//if( _pTimer->CurrentTick()+0.2f ) { fTimeRemain = 0.0f; }
      COLOR colAlpha;
      if(en_RenderType == RT_SKAMODEL || en_RenderType == RT_SKAEDITORMODEL) {
        colAlpha = GetModelInstance()->GetModelColor();
        colAlpha = (colAlpha&0xFFFFFF00) + (COLOR(fTimeRemain/m_fFadeTime*0xFF)&0xFF);
  ...
  BOOL ShouldCeaseAttack(void)
  {
    ...
    if (m_penEnemy==NULL ||
      ...
      return TRUE;
		...
		if (m_fMoveSpeed>0.0f && en_vCurrentTranslationAbsolute.Length()<0.5f) {
			// give up, or crash, or whatever...
			return TRUE;
		}

		
		// if we have an enemy and it's a player
		if (m_penEnemy!=NULL) {
			if (IsOfClass(m_penEnemy, "Player")) {
				// and the player is just standing there
				CMovableEntity *penPlayer = (CMovableEntity*)(CEntity*)m_penEnemy;
				if (penPlayer->en_vCurrentTranslationAbsolute.Length()<0.5f) {
					// and we can't reach them
	  ...
	  if (m_fMoveSpeed>0.0f && en_vCurrentTranslationAbsolute.Length()<1.0f) {
      //CPrintF("Enemy Base, Stuck\n");
      m_iFixupCount++;
      if (m_iFixupCount>7) {
        m_iFixupCount = 0;
				return TRUE;
    ...
    if (m_bKick) {
      if (en_vCurrentTranslationAbsolute.Length()<1.0f) {
        ...
        if (m_iFixupCount>7) {
          //m_iTrappedCount++;
          m_iFixupCount = 0;
          //if (m_iTrappedCount>1) {
            /*CEntity *pen = this;
            CMovableEntity *pmen = (CMovableEntity *)pen;
            FLOAT3D vDir;
            AnglesToDirectionVector(ANGLE3D(0,90,0), vDir);
            FakeJump(pmen->en_vIntendedTranslation, vDir, 20.0f, -0.0f, 1.0f, 200.0f, 5.0f);
          //}
          // cease attack
          return TRUE;
        }
  ...
  BOOL IsValidForEnemy(CEntity *penPlayer)
  {
    ...
    return 
      penPlayer!=NULL && 
      (IsDerivedFromClass(penPlayer, "Player") || IsOfClass(penPlayer, "PlayerBot"))
      && penPlayer->GetFlags()&ENF_ALIVE;
  }
 ...
 ************************************************************/
  ...
  virtual void SetSpeedsToDesiredPosition(const FLOAT3D &vPosDelta, FLOAT fPosDist, BOOL bGoingToPlayer)
  {
    ...
    // otherwise if following tactics, move anyway
    } else if (m_bTacticActive && m_bUseTactics) {
      // move and rotate towards it  
      if (fEnemyDistance<fCloseDistance) {
        m_fMoveSpeed = GetProp(m_fCloseRunSpeed);
        m_aRotateSpeed = GetProp(m_aCloseRotateSpeed);
      ...
      // if going to some other location (some pathfinding AI scheme)
		...
		// slow down if we hit something
		if (m_tmSlowDown>_pTimer->CurrentTick()) {
			m_fMoveSpeed *= 0.8f;
			//m_aRotateSpeed *= 0.1f;
		}
  }
  ...
  virtual ULONG SetDesiredMovement(void) 
  {
    ...
    if (m_dtDestination==DT_PLAYERCURRENT && m_bUseTactics) {
      ApplyTactics(vDelta);
    }
  ...
  BOOL CheckTouchForPathFinding(const ETouch &eTouch)
  {
    ...
    //CPrintF("%s, %.3f\n", m_strName, (eTouch.plCollision%vDir));
 ...
 ************************************************************/
  void InitializeTactics( void )   {
    ...
    m_vTacticsStartPosition = GetPlacement().pl_PositionVector;
 		...
 		FLOAT  m_fParam1 = FRnd()*-60; 
		FLOAT  m_fParam2 = FRnd()*60;  
		FLOAT  m_fParam3 = FRnd(); 
		FLOAT  m_fParam4 = -8+FRnd()*16; 
		FLOAT  m_fParam5 = FRnd()*40; 
		FLOAT  m_tmLastActivation = 0.0f;
    ...
    switch(iRnd)
    {
			case 0: m_tctType = 1; break;
			case 1: m_tctType = 2; break;
			case 2: m_tctType = 3; break;
    }
		/*CPrintF("Param: 1=%.3f, 2=%.3f, 3=%.3f, 4=%.3f, 5=%.3f\n", m_fParam1, m_fParam2, m_fParam3, m_fParam4, m_fParam5);
		CPrintF("m_tctType = %s\n", strType);
		CPrintF("\n");*/
		
    FLOAT fSign;
    // sign for randomized parameters
    if (Sgn(m_fParam2)>0 && Sgn(m_fParam1)>0) {
      fSign = +1.0f;
    } else if (Sgn(m_fParam2)<0 && Sgn(m_fParam1)<0) {
      fSign = -1.0f;
    } else {
      fSign = Sgn(FRnd()-0.5f);
    }
    ...
    switch (m_tctType) {
      case 1: {
        ...
        m_fTactic1=Lerp(Abs(m_fParam1), Abs(m_fParam2), FRnd())*fSign;
        // 2) time dump
        m_fTactic2=m_fParam4;
        // 3) dump factor, factor (0-1) of min distance when linear behaviour begins
        m_fTactic3=m_fParam3;
        // 4) initial distance
        m_fTactic4=(m_penEnemy->GetPlacement().pl_PositionVector - m_vTacticsStartPosition).Length();
        // 5) tactics stop distance
        m_fTactic5=m_fParam5;
        break; }

      case 2:
        // 1) randomized distance
        m_fTactic1=Lerp(m_fParam4, m_fParam5, FRnd());
        // 4) emission angle
        m_fTactic4=Lerp(Abs(m_fParam1), Abs(m_fParam2), FRnd())*fSign;
        // 2) tolerance strip width
        m_fTactic2=m_fAttackRunSpeed*2.0f*90.0f/m_aAttackRotateSpeed;
        //m_fTacticVar2=2.0f*m_fAttackRunSpeed;
        // 3) fade in/out ratio
        m_fTactic3=m_fParam3;
        // 5) initial distance
        m_fTactic5=(GetPlacement().pl_PositionVector - m_penEnemy->GetPlacement().pl_PositionVector).Length();
        // as a precausion, assume minimal strip width of 2m
        m_fTactic2=ClampDn(m_fTactic2, 2.0f);
      ...
      case 3:
        ...
        m_fTactic2=Lerp(m_fParam3, m_fParam4, FRnd());
        break;
    }
  }
  ...
  virtual void ApplyTactics( FLOAT3D &vDesiredPos) {
    ...
    if (m_penEnemy==NULL) {
			//CPrintF("No Enemy, Ugh!!!\n");
			m_penEnemy = GetWatcher()->CheckAnotherPlayer(m_penEnemy);
    }
    if (m_penEnemy==NULL) {
			//CPrintF("No Enemy Still, Double Ugh!!!\n");
      return;
    }
    ...
    if (m_bTacticActive) {
      ...
      case 1: {
        ...
        if (CalcDist(m_penEnemy)<m_fTactic5) {
        ...
        if(m_fTactic3>0) {
          // get enemy distance
          FLOAT fClamped=Clamp(CalcDist(m_penEnemy)-(m_fTactic4*m_fTactic3), 0.0f, m_fTactic4);
          fDistanceRatio=fClamped/(m_fTactic4*(1-m_fTactic3));
        }
        
        fTimeRatio=1.0f;
        if(m_fTactic2>0) {
          fTimeRatio=1.0f-(ClampUp((_pTimer->CurrentTick() - m_tmTacticsActivation)/m_fTactic2, 1.0f));
        }
      
        angEnemy(1) = m_fTactic1*fDistanceRatio*fTimeRatio;
        angEnemy(2) = 0.0f;
        angEnemy(3) = 0.0f;
      ...
      case 2: {
        ...
        if (vLinePlayerToThis.Length()<m_fTactic1) {
          m_bTacticActive = FALSE;
        // if closer to spawner-enemy line then supposed to
        } else if (fLineDist<m_fTactic1) {
          if (fLineDist<1.0f) { fLineDist=1.0f; }
          angEnemy(1) = m_fTactic4/fLineDist;
          angEnemy(2) = 0.0f;
          angEnemy(3) = 0.0f;
        ...
        } else if (fLineDist>m_fTactic1+m_fTactic2) {       
          if (fLineDist<1.0f) { fLineDist=1.0f; }
          angEnemy(1) = -m_fTactic4/fLineDist;
          angEnemy(2) = 0.0f;
          angEnemy(3) = 0.0f;
      ...
      case 3: {
        if (_pTimer->CurrentTick()<m_tmTacticsActivation+m_fTactic2) {
          angEnemy(1) = m_fTactic1;
          angEnemy(2) = 0.0f;
          angEnemy(3) = 0.0f;        
        } else {
          m_bTacticActive = FALSE;
 ...
 ************************************************************/
  ...
  virtual void BlowUp(void)
  {
    ...
    if( bGibs && !m_bRobotBlowup)
    {
      ...
      // spawn debris
      Debris_Begin(EIBT_FLESH, DPT_NONE, BET_NONE, m_fBlowUpSize*0.7f, vNormalizedDamage, vBodySpeed, 1.0f, 0.0f);
      for( INDEX iDebris = 0; iDebris<m_fBodyParts; iDebris++) {
        ...
        Debris_Spawn( this, this, ulFleshModel, ulFleshTexture, 0, 0, 0, IRnd()%4, 0.5f,
                      FLOAT3D(FRnd()*2.0f-1.0f, FRnd()*0.6f+0.2f, FRnd()*2.0f-1.0f));
      }
 ...
 ************************************************************/
  ...
  virtual void AdjustDifficulty(void)
  {
    ...
    //fMoveSpeed *= 0.8f+(FRnd()*0.75f);
    //fAttackSpeed *= (0.5f+FRnd()*0.75f);
 ...
 ************************************************************/
  ...
  void RenderParticles(void) {
		if (GetSP()->sp_bMental) {
			if (m_bHasBeenSpawned) {
				if (_pTimer->CurrentTick()>m_tmSpawned+0.1f) {
					Particles_ModelGlow3(this, PT_STAR07, 0.17f+m_fParticleSize, 2, m_colStar);
				}
			} else {
				Particles_ModelGlow3(this, PT_STAR07, 0.17f+m_fParticleSize, 2, m_colStar);
			}
    ...
    /*if(m_penMarker!=NULL) {
      ...
      FLOAT3D vTarget = m_penMarker->GetPlacement().pl_PositionVector;
    ...
    if(m_penEnemy!=NULL) {
      ...
      vTarget += FLOAT3D(0, 1.5f, 0);
      COLOR colLine = C_dGREEN|CT_OPAQUE;
			if (m_bCannotReachPlayer) {
				colLine = C_dRED|CT_OPAQUE;
			}
      Particles_Raycast(vSource, vTarget, 64, 0.5f, colLine);
    }*/
  }
  ...
  // determine if you can see a marker behind you
  BOOL IsMarkerBehind(CEntity *penEntity, FLOAT fCosHalfFrustum) 
  {
		// get direction to the entity
		FLOAT3D vDelta = CalcDelta(penEntity);
		// find front vector
  ...
  BOOL IsMarkerVisible(CEntity *penEntity) 
  {
	...
	/*void FindNewMarker(CEntity *penCurrent, BOOL bIntitial)
	{
		CEntity *penNew = NULL;
		FLOAT fDistance = 200.0f;
		INDEX iMarkerCount = 0;
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      CEntity *pen = iten;
      if (IsOfClass(pen, "Enemy Marker") && pen!=penCurrent) {
				FLOAT fMarkerDistance = (GetPlacement().pl_PositionVector - pen->GetPlacement().pl_PositionVector).Length();
				if (fMarkerDistance<fDistance) {
					fDistance = fMarkerDistance;
					if (bIntitial) {
						if (IsMarkerVisible(pen)) {
							penNew = pen;
							break;
						}
					} else {
						if (IsMarkerBehind(pen, 80.0f)) {
							penNew = pen;
							break;
						}
					}
		...
		if (penNew==NULL) {
			CPrintF("iMarkerCount: %d,  fDist: %.0f, penNew = NULL\n", iMarkerCount, fDistance);
		} else {
			CPrintF("iMarkerCount: %d,  fDist: %.0f, penNew != NULL\n", iMarkerCount, fDistance);
			m_penMarker = penNew;
		}
	...
	void FindNewMarker(CEntity *penCurrent, BOOL bIntitial)
	{
		...
		if (penNew!=NULL) {
			m_penMarker = penNew;
			//CPrintF("iMarkerCount: %d,  iSectorCount: %d, penNew != NULL\n", iMarkerCount, iSectorCount);
		} /*else {
			CPrintF("iMarkerCount: %d,  iSectorCount: %d, penNew = NULL\n", iMarkerCount, iSectorCount);
		}*/
	}
...
//**********************************************************
  ...
  MoveToDestination(EVoid) 
  {
    ...
    m_fMoveTime = _pTimer->CurrentTick() + 15.0f;
		m_iTrappedCount = 0;
		//m_fMoveSpeedSlow = m_fMoveSpeed*0.1f;
    // while not there and time not expired
    while (CalcDistanceInPlaneToDestination()>m_fMoveSpeed*m_fMoveFrequency*2.0f && m_fMoveTime>_pTimer->CurrentTick()) {
      // every once in a while
      wait (m_fMoveFrequency) {
        on (EBegin) : {
					if (m_bMovingThruMarkers) {
						if (m_penMarker==NULL) {
							resume;
						} else if (m_bTrapped) {
							m_iTrappedCount++;
							if (m_iTrappedCount==1) {
								...
								FindNewMarker(m_penMarker, FALSE);
								if (m_penMarker!=NULL) {
									m_vDesiredPosition = m_penMarker->GetPlacement().pl_PositionVector;
								}
								//resume;
							} else if (m_iTrappedCount>=2) {
								m_iTrappedCount = 0;
								m_bTrapped = FALSE;
								m_fMoveSpeed = 30.0f;
								m_aRotateSpeed = 1000.0f;
  ...
  MoveThroughMarkers() 
  {
    ...
    GetWatcher()->SendEvent(EStart());
		...
		// make sure there isn't something blocking our way to the marker
		if (m_penMarker!=NULL) {
			if (!IsMarkerVisible(m_penMarker)) {
				FindNewMarker(m_penMarker, FALSE);
			}  
		}
    ...
    while (m_penMarker!=NULL && IsOfClass(m_penMarker, "Enemy Marker")) {
      ...
      autocall MoveToDestination() EReturn;
			...
			if (pem->m_bStartTactics && m_bUseTactics){
				// start to see/hear
				m_bBlind = FALSE;
				m_bDeaf = FALSE;
  ...
  PerformAttack()
  {
    ...
    while(TRUE)
    {
      ...
      if (ShouldCeaseAttack()) {
			//if (ShouldCeaseAttack() || m_bCannotReachPlayer) {
        // cease it
        SetTargetNone();
        return EReturn();
      }
      ...
      if (m_fRangeLast>GetProp(m_fCloseDistance) && fEnemyDistance<=GetProp(m_fCloseDistance)) {
        ...
        m_fShootTime = 0.0f;
			...
			// If we can't move as fast as we want
			/*if (m_fRangeLast != 1E9f) {
				FLOAT3D vCurrentPosition = GetPlacement().pl_PositionVector;
				FLOAT fDistanceMoved = (vCurrentPosition - m_vLastPosition).Length();
				FLOAT fDistanceWantedToMove = m_fMoveFrequency * m_fMoveSpeed;
				if (fDistanceWantedToMove > 0) {
					FLOAT fMovePercentage = fDistanceMoved / fDistanceWantedToMove;
					//CPrintF("fDistanceMoved: %.3f\n fDistanceWantedToMove: %.3f\n fMovePercentage: %.3f\n", 
						//fDistanceMoved, fDistanceWantedToMove, fMovePercentage);
					if (fDistanceMoved < fDistanceWantedToMove) {
					}
				}
				m_vLastPosition = vCurrentPosition;
				//m_fLastDistance = fEnemyDistance;
			}*/

			//FLOAT3D vDesiredTranslation = GetDesiredTranslation();
      ...
      wait(m_fMoveFrequency) {
        ...
        on (EBegin) : {
          ...
          // if going to where player was last seen
            ...
            m_vDesiredPosition = m_vPlayerSpotted;
					// if we are still looking for a marker to go to
					} else if (m_dtDestination==DT_PATHPAUSED) {
						// stop where you are
						StopMoving();
						//StandingAnim();
            // find next path marker
            //FindNextPathMarker();
					...
					if (m_bCannotReachPlayer) {
						if (_pTimer->CurrentTick()>m_tmPause) {
							m_bCannotReachPlayer = FALSE;
						} else {
							m_fMoveSpeed = 0.0f;
							m_aRotateSpeed = 1000.0f;
        ...
        on (ETouch eTouch) : { 
					// if something is blocking my way to the player
					if ((eTouch.penOther->GetFlags()&ENF_ALIVE) && !IsOfClass(eTouch.penOther, "Player")) {
						if (IsInFrustum(eTouch.penOther, CosFast(30.0f))) {
							m_bCannotReachPlayer = TRUE;
							m_tmPause = _pTimer->CurrentTick()+0.25f;
							resume;
							//SwitchToAnotherPlayer();
							//stop;
						}
					}
					// if it's a model we must destroy
          else if( IfTargetCrushed(eTouch.penOther, (FLOAT3D&)eTouch.plCollision))
          {
            resume;
          }
          ...
          // if touch is ignored
					...
					else if (eTouch.penOther->GetRenderType()==CEntity::RT_BRUSH) {
            FLOAT3D vDir = en_vDesiredTranslationRelative;
            vDir.SafeNormalize();
            vDir*=GetRotationMatrix();
            // if the touched plane is more or less orthogonal to the current velocity
            if ((eTouch.plCollision%vDir)<-0.25f) { 
							if (m_bTacticActive && m_bUseTactics) { m_bTacticActive = 0; }
							m_bCannotReachPlayer = TRUE;
							m_tmPause = _pTimer->CurrentTick()+0.25f;
							//SwitchToAnotherPlayer();
							//stop;
        ...
        on (EReturn) : { stop; }  // returned from subprocedure
			...
			//CPrintF("PerformAttack() - m_dtDestination: %s\n", strDestinationType);
    }
  }
    ...
    wait(GetAnimLength(m_iChargeHitAnimation)) {
      ...
      on (ETouch etouch) : {
        ...
        if ((etouch.penOther->GetFlags()&ENF_ALIVE) && IsInPlaneFrustum(etouch.penOther, CosFast(60.0f))
					&& IsOfClass(etouch.penOther, "Player")) {
          // get your direction
          FLOAT3D vSpeed;
...
//**********************************************************
  ...
  Die(EDeath eDeath)
  {
    ...
    if (IsOfClass(penKiller, "PlayerBot")) {
      //CPrintF("Player gets credit for bot kill\n");
      penKiller = ((CPlayerBot&)*penKiller).m_penOwner;
    }
    ...
    if (m_penSpawnerTarget!=NULL) {
      //CPrintF("EnemyBase Death event sent to Spawner\n");
      //SendToTarget(m_penSpawnerTarget, EET_TRIGGER, this);
      SendToTarget(m_penSpawnerTarget, EET_START, this);
    }
    /*if (m_penSpawnerTarget2!=NULL) {
      //CPrintF("EnemyBase Death event sent to Spawner\n");
      //SendToTarget(m_penSpawnerTarget, EET_TRIGGER, this);
      SendToTarget(m_penSpawnerTarget2, EET_START, this);
    }*/
		if (m_penSpawnerBattery!=NULL) {
      SendToTarget(m_penSpawnerBattery, EET_STOPATTACK, this);
    }
  ...
  Death(EVoid) 
  {
   ...
   // SetCollisionFlags(ECF_CORPSE);
    ...
    m_fTactic4 = -1.0f;//WaitForDust(m_vTacticsStartPosition);
    // remember start time
    m_fTactic5=_pTimer->CurrentTick();
    ...
    SetCollisionFlags(ECF_CORPSE&~((ECBI_PROJECTILE_MAGIC|ECBI_PROJECTILE_SOLID)<<ECB_TEST));
		...
		FLOAT fAnimWait = GetAnimLength(iAnim);
		m_fFadeTime = fAnimWait/4*3;

    // if no dust should be spawned
    if( m_fTactic4<0) {
      autowait(fAnimWait/4);
			// start fading
    ...
    // should spawn dust
      ...
      while(_pTimer->CurrentTick()<m_fTactic5+GetCurrentAnimLength()) {
        autowait(_pTimer->TickQuantum);
        if(en_penReference!=NULL && _pTimer->CurrentTick()>=m_fTactic5+m_fTactic4 && m_fTactic3<0) {
...
//**********************************************************
  ...
  Active(EVoid) 
  {
    m_bActive = TRUE;
    m_fDamageConfused = 0.0f;
		m_bMovingThruMarkers = FALSE;
		/*if (m_penMarker==NULL) {
			FindNewMarker(NULL, TRUE);
		}*/
    // logic loop
    wait () {
      // initially
      ...
      on (EReconsiderBehavior) : {
				//m_penEnemy = NULL;
				/*if (m_bCannotReachPlayer) {
					// if in coop find us another player to attack
					if (GetSP()->sp_bCooperative) {
						if (_pTimer->CurrentTick()>m_tmPause) {
							m_penEnemy = GetWatcher()->CheckAnotherPlayer(m_penEnemy);
							m_bCannotReachPlayer = FALSE;
				...
				m_bCannotReachPlayer = FALSE;
        // if we have an enemy and we can reach him 
        if (m_penEnemy!=NULL) {
				//if (m_penEnemy!=NULL && !m_bCannotReachPlayer) {
          // attack it
					m_bMovingThruMarkers = FALSE;
          call AttackEnemy();
        ...
        if (bTargetChanged) {
          ...
          SendEvent(EReconsiderBehavior());
				...
				if (eTouch.penOther->GetFlags()&ENF_ALIVE) {
					if (IsInFrustum(eTouch.penOther, CosFast(30.0f))) {
						m_tmSlowDown = _pTimer->CurrentTick()+m_fMoveFrequency+0.1f;
					}
				}
				if (m_bMovingThruMarkers) {
  ...
  Inactive(EVoid) 
  {
    ...
    wait() {
      ...
      on (ETrigger eTrigger) : {
        CEntity *penCaused = FixupCausedToPlayer(this, eTrigger.penCaused);
        // if can set the trigerer as soft target
        if (SetTargetSoft(penCaused) && !m_bFrozen) {
      ...
      on (EDamage eDamage) : {
        ...
        if (SetTargetHard(eDamage.penInflictor) && !m_bFrozen) {
          // notify wounding to others
          WoundedNotify(eDamage);
          // make pain sound
          WoundSound();
  ...
  MainLoop(EVoid) 
  {
    ...
    autowait(_pTimer->TickQuantum);
		...
		if (GetSP()->sp_bMental && !IsOfClass(this, "Summoner")) {
			m_colStar = RGBToColor( IRnd()%255, IRnd()%255, IRnd()%255 );
			m_fParticleSize = en_pciCollisionInfo->GetMaxFloorRadius()*0.1f;
			//CPrintF("m_fParticleSize: %.3f, Name %s\n", m_fParticleSize, m_strName);
		}
    ...
    // ASSERT failure prob fix?
    /*if (m_fIgnoreRange<=m_fAttackDistance) {
      m_fIgnoreRange = m_fAttackDistance+1.0f;
    }*/
