  ...
  void Explode(void) {
    if (!m_bExploded) {
      ...
      if (m_hdtType==HDT_BOMBERMAN) {
        ...
        InflictDirectDamage(this, this, DMT_CLOSERANGE, 50.0f, vSource, 
          -en_vGravityDir);
        InflictRangeDamage(this, DMT_EXPLOSION, 20.0f, vSource, 2.75f, 8.0f);
      }
      ...
      eSpawnEffect.betType = BET_KAMI_EXPLOSION;//BET_BOMB;
      eSpawnEffect.vStretch = FLOAT3D(0.75,0.75,0.75);
...
//    m_fMaxHealth = 19.5f;
    ...
    en_fDensity = 2000.0f;
    m_fBlowUpSize = 2.0f;
		/*if (m_hdtType == HDT_ROCKETMAN) {
			m_hdtType = HDT_BOMBERMAN;
		}*/
    ...
    switch (m_hdtType) {
      case HDT_FIRECRACKER:
        ...
        // setup moving speed
        /*m_fWalkSpeed = FRnd() + 5.5f*fWhatever;
        m_aWalkRotateSpeed = AngleDeg(FRnd()*10.0f + 500.0f)*fWhatever;
        m_fAttackRunSpeed = FRnd() + 5.0f*fWhatever;
        m_aAttackRotateSpeed = AngleDeg(FRnd()*75 + 245.0f)*fWhatever;
        m_fCloseRunSpeed = FRnd() + 5.0f*fWhatever;
        m_aCloseRotateSpeed = AngleDeg(FRnd()*75 + 245.0f)*fWhatever;*/
      ...
      case HDT_ROCKETMAN:
        ...
        /*m_fWalkSpeed = FRnd() + 5.5f*fWhatever;
        m_aWalkRotateSpeed = AngleDeg(FRnd()*10.0f + 500.0f)*fWhatever;
        m_fAttackRunSpeed = FRnd()*2.0f + 6.0f*fWhatever;
        m_aAttackRotateSpeed = AngleDeg(FRnd()*75 + 245.0f)*fWhatever;
        m_fCloseRunSpeed = FRnd()*2.0f + 6.0f*fWhatever;
        m_aCloseRotateSpeed = AngleDeg(FRnd()*75 + 245.0f)*fWhatever;*/
        m_fWalkSpeed = FRnd() + 1.5f;
        m_aWalkRotateSpeed = AngleDeg(FRnd()*10.0f + 500.0f);
      ...
      case HDT_BOMBERMAN:
        ...
        /*m_fWalkSpeed = FRnd() + 5.5f*fWhatever;
        m_aWalkRotateSpeed = AngleDeg(FRnd()*10.0f + 500.0f)*fWhatever;
        m_fAttackRunSpeed = FRnd() + 4.0f*fWhatever;
        m_aAttackRotateSpeed = AngleDeg(FRnd()*75 + 245.0f)*fWhatever;
        m_fCloseRunSpeed = FRnd() + 4.0f*fWhatever;
        m_aCloseRotateSpeed = AngleDeg(FRnd()*75 + 245.0f)*fWhatever;*/
        m_fWalkSpeed = FRnd() + 1.5f;
        m_aWalkRotateSpeed = AngleDeg(FRnd()*10.0f + 500.0f);
    ...
    GetModelObject()->StretchModel(FLOAT3D(1.25f*fWhatever, 1.5f*fWhatever, 1.25f*fWhatever));
    ModelChangeNotify();
    StandingAnim();
    m_fStepHeight = 10.0f; 
	  if (strLevelName=="2_1_Ziggurrat") {
			if (m_strName=="Kamikaze N Temp" && m_penDeathTarget!=NULL ) {
				m_hdtType = HDT_BOMBERMAN;
				m_fStepHeight = 2.0f;
//				m_penMarker = NULL;
			}
