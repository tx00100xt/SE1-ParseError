...
properties:
 ...
 30 FLOAT fWhatever = 1.0f,

 40 BOOL m_bStretchModel = FALSE,
...
//*/
  ...
  BOOL AdjustShadingParameters(FLOAT3D &vLightDirection, COLOR &colLight, COLOR &colAmbient)
  {
    if (GetSP()->sp_bMental) {
      if (GetHealth()<=0) {
				if (m_bStretchModel) {
					colAmbient = C_MAGENTA;
				} else {
					colAmbient = C_vdBROWN;
				}
      } else {
				if (m_bStretchModel) {
					GetModelObject()->mo_colBlendColor = C_MAGENTA|0;
				} else {
					GetModelObject()->mo_colBlendColor = C_vdBROWN|0;
				}
   ...
   void AdjustDifficulty(void)
  {
    ...
    //fMoveSpeed *= (0.5f+FRnd()*0.7f);
    //fAttackSpeed *= (0.5f+FRnd()*0.7f);
 ...
 ************************************************************/
  Main(EVoid) {
    ...
    // setup moving speed
    /*m_fWalkSpeed = (FRnd() + 5.0f)*fWhatever;
    m_aWalkRotateSpeed = (FRnd()*25.0f + 30.0f)*fWhatever;
    m_fAttackRunSpeed = (FRnd()*20.0f + 10.0f)*fWhatever;
    m_aAttackRotateSpeed = (FRnd()*50.0f + 250.0f)*fWhatever;
    m_fCloseRunSpeed = (FRnd()*5.0f + 4.0f)*fWhatever;
    m_aCloseRotateSpeed = (FRnd()*50.0f + 245.0f)*fWhatever;
    ...
    m_fFlyWalkSpeed = (FRnd()/2 + 1.0f)*fWhatever;
    m_aFlyWalkRotateSpeed = (FRnd()*20.0f + 350.0f)*fWhatever;
    m_fFlyAttackRunSpeed = (FRnd()*10.0f + 20.0f)*fWhatever;
    m_aFlyAttackRotateSpeed = (FRnd()*40.0f + 150.0f)*fWhatever;
    m_fFlyCloseRunSpeed = (FRnd()*10.0f + 20.0f)*fWhatever;
    m_aFlyCloseRotateSpeed = (FRnd()*50.0f + 500.0f)*fWhatever;
    m_fGroundToAirSpeed = m_fFlyCloseRunSpeed*fWhatever;
    m_fAirToGroundSpeed = m_fFlyCloseRunSpeed*fWhatever;*/
    // setup moving speed
