...
properties:
 ...
 1002 FLOAT fX = 0.0f,
 //1003 CTString strLevelName = "",
 1004 INDEX m_iCount = 0,
 1006 BOOL  m_bTriggerOnlyOnce = FALSE,
 1007 BOOL  m_bTriggered = FALSE,
...
procedures:
  ...
  Main() {
    ...
    if (strLevelName=="02_SandCanyon") {
			fX = GetPlacement().pl_PositionVector(1);
			if (fX == 72.0f) {
				m_bTriggerOnlyOnce = TRUE;
			}
    }

    if (strLevelName=="15_TheGreatPyramid") {
      if (m_strName=="MB_LowerDoor") {
        m_ctDebrises = 0;
      }
    }
    ...
    wait() {
      ...
      on (ETrigger) : {
				if (m_bTriggerOnlyOnce && m_bTriggered) {
					resume;
				}
				m_bTriggered = TRUE;
        // if not already moving and have target
