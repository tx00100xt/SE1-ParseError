...
properties:
  ...
  3 FLOAT m_fSantaHealth "Santa Health" = 1000000000000000000.0f,
...
//    StartModelAnim(iAnim, 0);
  ...
  void DeathNotify(void) {
    ...
    /*if (GetSP()->sp_bCooperative) {
       if (m_penTemplate0!=NULL && IsOfClass(m_penTemplate0, "Ammo Pack")) {
         m_penTemplate0 = NULL;
       }
       if (m_penTemplate1!=NULL && IsOfClass(m_penTemplate1, "Ammo Pack")) {
         m_penTemplate1 = NULL;
       }
       if (m_penTemplate2!=NULL && IsOfClass(m_penTemplate2, "Ammo Pack")) {
         m_penTemplate2 = NULL;
       }
       if (m_penTemplate3!=NULL && IsOfClass(m_penTemplate3, "Ammo Pack")) {
         m_penTemplate3 = NULL;
       }
       if (m_penTemplate4!=NULL && IsOfClass(m_penTemplate4, "Ammo Pack")) {
         m_penTemplate4 = NULL;
       }
    }*/
    CPlacement3D pl = GetPlacement();
    if (m_penTemplate0!=NULL) {
      ...
      FLOAT fR = FRnd()*2.0f+1.0f;
      CEntity *penSpawned0 = GetWorld()->CopyEntityInWorld( *m_penTemplate0,
        CPlacement3D(FLOAT3D(-32000.0f+FRnd()*200.0f, -32000.0f+FRnd()*200.0f, 0), ANGLE3D(0, 0, 0)) );      
      pl.pl_PositionVector += FLOAT3D(CosFast(fA)*fR, 0.5f, SinFast(fA)*fR);  // a bit up in the air
      penSpawned0->Teleport(pl, FALSE);
    }
    if (m_penTemplate1!=NULL) {
      FLOAT fA = FRnd()*360.0f;
      FLOAT fR = FRnd()*2.0f+1.0f;
      CEntity *penSpawned1 = GetWorld()->CopyEntityInWorld( *m_penTemplate1,
        CPlacement3D(FLOAT3D(-32000.0f+FRnd()*200.0f, -32000.0f+FRnd()*200.0f, 0), ANGLE3D(0, 0, 0)) );
      pl.pl_PositionVector += FLOAT3D(CosFast(fA)*fR, 0.5f, SinFast(fA)*fR);  // a bit up in the air
      penSpawned1->Teleport(pl, FALSE);
    }
    if (m_penTemplate2!=NULL) {
      FLOAT fA = FRnd()*360.0f;
      FLOAT fR = FRnd()*2.0f+1.0f;
      CEntity *penSpawned2 = GetWorld()->CopyEntityInWorld( *m_penTemplate2,
        CPlacement3D(FLOAT3D(-32000.0f+FRnd()*200.0f, -32000.0f+FRnd()*200.0f, 0), ANGLE3D(0, 0, 0)) );
      pl.pl_PositionVector += FLOAT3D(CosFast(fA)*fR, 0.5f, SinFast(fA)*fR);  // a bit up in the air
      penSpawned2->Teleport(pl, FALSE);
    }
    if (m_penTemplate3!=NULL) { 
      FLOAT fA = FRnd()*360.0f;
      FLOAT fR = FRnd()*2.0f+1.0f;
      CEntity *penSpawned3 = GetWorld()->CopyEntityInWorld( *m_penTemplate3,
        CPlacement3D(FLOAT3D(-32000.0f+FRnd()*200.0f, -32000.0f+FRnd()*200.0f, 0), ANGLE3D(0, 0, 0)) );
      pl.pl_PositionVector += FLOAT3D(CosFast(fA)*fR, 0.5f, SinFast(fA)*fR);  // a bit up in the air
      penSpawned3->Teleport(pl, FALSE);
    }
    if (m_penTemplate4!=NULL) { 
      FLOAT fA = FRnd()*360.0f;
      FLOAT fR = FRnd()*2.0f+1.0f;
      CEntity *penSpawned4 = GetWorld()->CopyEntityInWorld( *m_penTemplate4,
        CPlacement3D(FLOAT3D(-32000.0f+FRnd()*200.0f, -32000.0f+FRnd()*200.0f, 0), ANGLE3D(0, 0, 0)) );
      pl.pl_PositionVector += FLOAT3D(CosFast(fA)*fR, 0.5f, SinFast(fA)*fR);  // a bit up in the air
      penSpawned4->Teleport(pl, FALSE);
    }
