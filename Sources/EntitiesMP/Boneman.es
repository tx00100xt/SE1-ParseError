...
//*/
  ...
  virtual CTString GetPlayerKillDescription(const CTString &strPlayerName, const EDeath &eDeath)
  {
    ...
    if (eDeath.eLastDamage.dmtType==DMT_CLOSERANGE) {
      str.PrintF(TRANSV("A Kleer ripped %s a new one"), (const char *) strPlayerName);
    } else {
...
//*/
  ...
  void StandingAnim(void) {
    ...
    //StartModelAnim(BONEMAN_ANIM_ATTACKFAR, AOF_PAUSED|AOF_NORESTART);
		//GetModelObject()->SelectFrameInTime(0.070f);
    DeactivateRunningSound();
  };
 ...
 ************************************************************/
  ...
  /*void BlowUp(void) {
    ...
    //vNormalizedDamage *= 0.005f;

    FLOAT3D vBodySpeed = en_vCurrentTranslationAbsolute-en_vGravityDir*(en_vGravityDir%en_vCurrentTranslationAbsolute);
    //vBodySpeed *= 0.005f;
		vNormalizedDamage = FLOAT3D(0, 0, 0);
		vBodySpeed = vNormalizedDamage;//FLOAT3D(FRnd(), FRnd()*2.0f, FRnd());
    // spawn debris
    Debris_Begin(EIBT_BONES, DPT_NONE, BET_NONE, fEntitySize, -vNormalizedDamage*3, vBodySpeed, 0.0001f, C_vdRED);
  ...
  BOOL AdjustShadingParameters(FLOAT3D &vLightDirection, COLOR &colLight, COLOR &colAmbient)
  {
    if (GetSP()->sp_bMental) {
      /*if (GetHealth()<=0) {
				...
				if (m_bStretchModel) {
					colAmbient = C_dCYAN;
					GetModelObject()->mo_colBlendColor = C_dCYAN|0;
				} else {
					colAmbient = C_vdRED;
 ...
 ************************************************************/
  Main(EVoid) {
    ...
    if (m_bStretchModel) {
	  ...
	  GetModelObject()->StretchModel(FLOAT3D(1*fWhatever,1*fWhatever,1*fWhatever));
		ModelChangeNotify();
    StandingAnim();
    m_sptType = SPT_BONES;
    // setup moving speed
    /*m_fWalkSpeed = FRnd() + 2.5f*fWhatever;
    m_aWalkRotateSpeed = FRnd()*25.0f + 45.0f*fWhatever;
    m_fAttackRunSpeed = FRnd()*3.0f + 10.0f*fWhatever;
    m_aAttackRotateSpeed = FRnd()*200 + 600.0f*fWhatever;
    m_fCloseRunSpeed = FRnd() + 13.0f*fWhatever;
    m_aCloseRotateSpeed = FRnd()*100 + 1000.0f*fWhatever;*/
...
//    m_fFallHeight = -1;
    m_fStepHeight = 10.0f; 
	  /*if (strLevelName == "2_1_Ziggurrat" ) {
			if ( m_strName == "Boneman NBD Count Column room" ) {
				m_fStepHeight = 2.0f;
				//m_penMarker = NULL;
        m_fFallHeight = -1;
			}
		}
	  if (strLevelName == "2_2_Persepolis" ) {
			if ( m_strName == "Boneman ND Flying Count Temp" ) {
				m_penMarker = NULL;
			}
		}*/
	  m_fLagger = 1.8f;
