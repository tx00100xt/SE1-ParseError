 ...
 ************************************************************/
  Main(EVoid) {
    ...
    m_aWalkRotateSpeed = AngleDeg(FRnd()*25.0f + 45.0f)*fWhatever;
    m_fAttackRunSpeed = FRnd()*2.0f + 13.0f*fWhatever;
    m_fAttackRotateRunInto = AngleDeg(FRnd()*30 + 50.0f)*fWhatever;
    m_aAttackRotateSpeed = m_fAttackRotateRunInto;
    m_fCloseRunSpeed = FRnd() + 10.5f*fWhatever;
    m_aCloseRotateSpeed = AngleDeg(FRnd()*50 + 250.0f)*fWhatever;*/
...
//		CPrintF( "%s\n", m_strName);
	  if (strLevelName == "2_1_Ziggurrat" ) {
			if ( m_strName == "ChainSaw NBD Count" ) {
				m_fStepHeight = 2.0f;
				//m_penMarker = NULL;
				m_fFallHeight = -1;
				m_bDeaf = FALSE; 
    ...
    m_iRunType = IRnd()%3;    
	  m_fLagger = 1.25f; 
    GetModelObject()->StretchModel(FLOAT3D(FREAK_SIZE*fWhatever, FREAK_SIZE*fWhatever, FREAK_SIZE*fWhatever));
    ModelChangeNotify();
