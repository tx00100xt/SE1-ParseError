...
properties:
 ...
 20 BOOL  m_bCanBeenPicked = FALSE,
 21 BOOL  m_bCreateTouchBox = TRUE,

components:

  1 model   MODEL_ITEM      "Models\\Items\\ItemHolder\\ItemHolder.mdl",
  2 class   CLASS_BOX       "Classes\\Box.ecl",
 ...
 ************************************************************/
  ...
  {
    if (m_penTarget!=NULL && !(m_bPickupOnce || m_bRespawn)) {
      if (IsDerivedFromClass(m_penTarget, "Trigger")) {
        CEntity *pen = m_penTarget;
        CTrigger *penTrigger = (CTrigger *)pen;
        // if it doesn't count as a secret
        if (penTrigger->m_fScore == 0.0f) {
					CPlacement3D pl;
					CEntity *pen = NULL;
					pl = GetPlacement();
					pen = CreateEntity(pl, CLASS_BOX);
					CBox *box = ((CBox*)pen);
					box->m_penTarget = m_penTarget;
					box->m_iSize = GetSize();
					box->m_penOwner = this;
					box->SetParent(this);
					box->Initialize();
  ...
  ItemLoop(EVoid)
  {
    ...
    wait() {
      ...
      on (EPass epass) : { 
        ...
        if (IsDerivedFromClass(epass.penOther, "Enemy Base")) {
          pass;
        }

        //CPrintF("Item Picked in Item Base Class\n");
        //m_bCanBeenPicked = FALSE;

        if ( !(m_bPickupOnce||m_bRespawn) ) {
					if (m_penTarget) {
						//CPrintF("Item has sent event. %.0f\n", _pTimer->CurrentTick());
