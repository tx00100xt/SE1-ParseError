...
#include <EntitiesMP/condition.h>
...
{
  INDEX ctSpaces = (40-str.LengthNaked())/2;
  if (ctSpaces<0) {
    ctSpaces=0;
  }
    ...
    if (IsDerivedFromClass(pen, "Enemy Base") && !IsOfClass(pen, "Devil")) {
      ...
      if (penEnemy->m_penEnemy==NULL) {
        continue;
			...
			//CPrintF("Prior: Enemy = %s, Health %.0f\n", penEnemy->GetName(), penEnemy->GetHealth());
      penKiller->InflictDirectDamage(pen, penKiller, DMT_ACID, 
        penEnemy->GetHealth()*3, pen->GetPlacement().pl_PositionVector, FLOAT3D(0,1,0));
			//CPrintF("Post: Enemy = %s, Health %.0f\n", penEnemy->GetName(), penEnemy->GetHealth());
    }
  }}
}
...
//#define PLF_LEADER                (1UL<<12)  // only allow the leaders to trigger entities
#define PLF_INVULNERABLE          (1UL<<12)  // marks that a player is invulnerable
...
INDEX cht_bDFB = FALSE;
INDEX cht_bDFBD = FALSE;
INDEX cht_bDFBA = FALSE;
...
static FLOAT plr_fDeceleration  = 105.0f;
static FLOAT plr_fSpeedForward  = 26.0f;
static FLOAT plr_fSpeedBackward = 26.0f;
static FLOAT plr_fSpeedSide     = 26.0f;
static FLOAT plr_fSpeedUp       = 17.0f;*/
static FLOAT plr_fAcceleration  = 100.0f;
static FLOAT plr_fDeceleration  = 60.0f;
static FLOAT plr_fSpeedForward  = 20.0f; // 10
static FLOAT plr_fSpeedBackward = 20.0f; // 10
static FLOAT plr_fSpeedSide     = 20.0f; // 10
static FLOAT plr_fSpeedUp       = 17.0f; // 11
...
FLOAT gfx_fEnvParticlesDensity = 0.0f;
FLOAT gfx_fEnvParticlesRange = 0.0f;
//INDEX gfx_bRenderFog = FALSE;
...
extern INDEX dwk_bShowExtraStats = FALSE;
extern INDEX dwk_bShowEnemyCount = FALSE;
extern INDEX dwk_bShowPlayerCoords = FALSE;
extern INDEX hud_bShowEnemyRadar = FALSE;
extern INDEX hud_bShowPlayerRadar = FALSE;
extern INDEX hud_bShowPlayerRadar2 = FALSE;
extern INDEX hud_iCrosshairColor = 0;
extern INDEX hud_bColorCritterRadar = FALSE;
extern FLOAT hud_fFOV = 1.0f;
extern INDEX hud_bShowScore = FALSE;
extern INDEX hud_bShowPEInfo = FALSE;
//extern INDEX dwk_bBotView = FALSE;
//extern INDEX dwk_iBotView = 0;
extern INDEX hud_iView = 0;
extern INDEX hud_bShowOverlordInfo = FALSE;

// bot options
//extern CTString bot_strName1 = ""; 
//extern CTString bot_strName2 = "";
//extern CTString bot_strName3 = "";
...
//  CPrintF("compose: prescan %d, x:%g\n", bPreScan, paAction.pa_aRotation(1));
  ...
  if(pctlCurrent.bWalk) {
    ...
    paAction.pa_vTranslation(1) /= 2.0f;
		paAction.pa_vTranslation(2) /= 1.45f;
  }
  ...
  //FLOAT fCurrentTrans = en_vCurrentTranslationAbsolute.Length();
  //FLOAT fCurrentRot = en_aCurrentRotationAbsolute(1);
  //CPrintF("fCurrentTrans = %.2f, fCurrentRot = %.2f\n", fCurrentTrans, fCurrentRot);
  //if (fCurrentTrans<0.01f && !bFiring) {
  /*if (!pctlCurrent.bFire && !pctlCurrent.bMoveForward && !pctlCurrent.bMoveBackward
    && !pctlCurrent.bMoveLeft && !pctlCurrent.bMoveRight) {
    //CPrintF("Player not moving\n");
    //m_ulFlags|=PLF_INVULNERABLE;
    penThis->SetFlags(penThis->GetFlags() | ENF_INVISIBLE);
  } else {
    //m_ulFlags&=~PLF_INVULNERABLE;
    penThis->SetFlags(penThis->GetFlags() & ~ENF_INVISIBLE);
  }*/
...
void CPlayer_OnInitClass(void)
{
  ...
  _pShell->DeclareSymbol("user INDEX ctl_bFireSecondary;",         (void *) &pctlCurrent.bFireSecondary);
  ...
  _pShell->DeclareSymbol("INDEX cht_bDFB;", (void *) &cht_bDFB);
  _pShell->DeclareSymbol("INDEX cht_bDFBD;", (void *) &cht_bDFBD);
  ...
  // mod chit
  _pShell->DeclareSymbol("persistent user INDEX dwk_bShowExtraStats;",    (void *) &dwk_bShowExtraStats);
  _pShell->DeclareSymbol("persistent user INDEX dwk_bShowEnemyCount;",    (void *) &dwk_bShowEnemyCount);
  _pShell->DeclareSymbol("persistent user INDEX dwk_bShowPlayerCoords;",  (void *) &dwk_bShowPlayerCoords);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowEnemyRadar;",    (void *) &hud_bShowEnemyRadar);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowPlayerRadar;",   (void *) &hud_bShowPlayerRadar);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowPlayerRadar2;",  (void *) &hud_bShowPlayerRadar2);
  _pShell->DeclareSymbol("persistent user INDEX hud_iCrosshairColor;",    (void *) &hud_iCrosshairColor);
  _pShell->DeclareSymbol("persistent user INDEX hud_bColorCritterRadar;", (void *) &hud_bColorCritterRadar);
  _pShell->DeclareSymbol("persistent user FLOAT hud_fFOV;",               (void *) &hud_fFOV);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowScore;",         (void *) &hud_bShowScore);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowPEInfo;",        (void *) &hud_bShowPEInfo);
	//_pShell->DeclareSymbol("persistent user INDEX dwk_bBotView;",           (void *) &dwk_bBotView);
  //_pShell->DeclareSymbol("persistent user INDEX dwk_iBotView;",           (void *) &dwk_iBotView);
  _pShell->DeclareSymbol("persistent user INDEX hud_iView;",              (void *) &hud_iView);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowOverlordInfo;",  (void *) &hud_bShowOverlordInfo);

//  _pShell->DeclareSymbol("persistent user CTString bot_strName1;",        (void *) &bot_strName1);
//  _pShell->DeclareSymbol("persistent user CTString bot_strName2;",        (void *) &bot_strName2);
//  _pShell->DeclareSymbol("persistent user CTString bot_strName3;",        (void *) &bot_strName3);
...
//  _pShell->DeclareSymbol("persistent user FLOAT gfx_bRenderFog;", (void *) &gfx_bRenderFog);
...
//	_pShell->Execute("gfx_bRenderFog = 0;");
}
...
properties:
 ...
 167 FLOAT m_tmInvulnerabilityMax = 1E6f,
 168 FLOAT m_tmSeriousDamageMax   = 1E6f,
 169 FLOAT m_tmSeriousSpeedMax    = 1E6f,
 ...
 1000 INDEX m_iMoveMarker = 0,
 1001 CEntityPointer m_penStar,
 1002 CEntityPointer m_penBot1,
 1003 CEntityPointer m_penBot2,
 1004 CEntityPointer m_penBot3,
 1005 CEntityPointer m_penBot4,
 1006 CEntityPointer m_penBot5,
 1007 CEntityPointer m_penBot6,
 /*1008 CEntityPointer m_penBot7,
 1009 CEntityPointer m_penBot8,
 1010 CEntityPointer m_penBot9,
 1011 CEntityPointer m_penBot10,
 1012 CEntityPointer m_penBot11,
 1013 CEntityPointer m_penBot12,
 1014 CEntityPointer m_penBot13,
 1015 CEntityPointer m_penBot14,
 1016 CEntityPointer m_penBot15,
 1017 CEntityPointer m_penBot16,
 1018 CEntityPointer m_penBot17,
 1019 CEntityPointer m_penBot18,
 1020 CEntityPointer m_penBot19,
 1021 CEntityPointer m_penBot20,
 1022 CEntityPointer m_penBot21,
 1023 CEntityPointer m_penBot22,
 1024 CEntityPointer m_penBot23,
 1025 CEntityPointer m_penBot24,
 1026 CEntityPointer m_penBot25,
 1027 CEntityPointer m_penBot26,
 1028 CEntityPointer m_penBot27,
 1029 CEntityPointer m_penBot28,
 1030 CEntityPointer m_penBot29,
 1031 CEntityPointer m_penBot30,
 1032 CEntityPointer m_penBot31,
 1033 CEntityPointer m_penBot32,
 1034 CEntityPointer m_penBot33,
 1035 CEntityPointer m_penBot34,
 1036 CEntityPointer m_penBot35,
 1037 CEntityPointer m_penBot36,
 1038 CEntityPointer m_penBot37,
 1039 CEntityPointer m_penBot38,
 1040 CEntityPointer m_penBot39,
 1041 CEntityPointer m_penBot40,
 1042 CEntityPointer m_penBot41,
 1043 CEntityPointer m_penBot42,
 1044 CEntityPointer m_penBot43,
 1045 CEntityPointer m_penBot44,
 1046 CEntityPointer m_penBot45,
 1047 CEntityPointer m_penBot46,
 1048 CEntityPointer m_penBot47,
 1049 CEntityPointer m_penBot48,
 1050 CEntityPointer m_penBot49,
 1051 CEntityPointer m_penBot50,*/

 1030 CPlacement3D m_plFlySpawn = CPlacement3D(FLOAT3D(0,0,0), ANGLE3D(0,0,0)),
 //1031 BOOL m_bRenderParticles = FALSE,
 1032 BOOL m_bDoSeriousSpeedWarp = FALSE,
 1033 FLOAT m_fFOV = 90.0f,
 //1031 INDEX m_iTeam = 0,
 //1032 INDEX m_iTeamPlayer = 0,

{
  ...
  CModelObject m_moRender;                  // model object to render - this one can be customized
	...
	CDynamicContainer<CEntity> m_dcDisplay;
}
...
//      SetupLightSource(); //? is this ok !!!!
    ...
    // if normal copying
      ...
      m_ctUnreadMessages = penOther->m_ctUnreadMessages;
		...
		// Fragman's Display code
		//m_dcDisplay=penOther->m_dcDisplay;
		// End of Fragman's Display code
  }
  ...
  void Write_t( CTStream *ostr) // throw char *
  {
    ...
    ostr->Write_t(&m_psGameTotal , sizeof(m_psGameTotal ));
		...
		INDEX ctDisplay =m_dcDisplay.Count(); 
		(*ostr)<<ctDisplay; 
		for(INDEX iDisplay=0; iDisplay<ctDisplay; iDisplay++){ 
			WriteEntityPointer_t(ostr, &m_dcDisplay[iDisplay]); 
			//ostr->Write_t(&m_dcDisplay[iDisplay] , sizeof(m_dcDisplay[iDisplay] )); 			
		}*/
		// End of Fragman's Display code

		// Parse Error Serious Speed Warp code
		//ostr->Write_t(&m_fFOV, sizeof(m_fFOV));
		// End
  }
  /* Read from stream. */
  ...
  { 
    ...
    (*istr) >> m_psGameTotal;
		...
		// Fragman's Display code
		// load array of display pointers 
		/*istr->ExpectID_t("DISP"); 
		INDEX ctDisplay=0; 
		...
		if( ctDisplay>0) { 
			m_dcDisplay.Clear();           
			for(INDEX iDisplay=0; iDisplay<ctDisplay; iDisplay++){ 
				CEntityPointer penbeh=NULL; 
				ReadEntityPointer_t(istr,penbeh); 
				CEntity *penblah=penbeh; 
				if(IsOfClass(penblah,"Display")){ 
					m_dcDisplay.Add(penblah); 
				} 
			} 
		}*/
		// End of Fragman's Display code
		...
		// Parse Error Serious Speed Warp code
		//istr->Read_t(&m_fFOV, sizeof(m_fFOV));
		// End
...
//*/
  ...
  void GetDetailStatsCoop(CTString &strStats)
  {
    ...
    //strStats += CTString("^c0000FF")+CenterString(TRANS("YOUR STATS"))+"^r\n";
		strStats += CTString("^c0000FF")+TRANS("YOUR STATS")+"^r";
    strStats+="\n";
    ...
    extern INDEX SetAllPlayersStats( INDEX iSortKey);
    extern CPlayer *_apenPlayers[NET_MAXGAMEPLAYERS];
    const INDEX ctPlayers = SetAllPlayersStats(5); // sort by frags
    ...
    // add squad stats
    strStats+="\n";
    //strStats += CTString("^c0000FF")+CenterString(TRANS("SQUAD TOTAL"))+"^r\n";
		strStats += CTString("^c0000FF")+TRANS("SQUAD TOTAL")+"^r\n";
    strStats+="\n";
    ...
    strStats+="\n";
    strStats+="\n";
    strStats+= CTString("^c0000FF")+TRANS("TOTAL")+"^r\n";
    ...
    strStats+="\n";
    //strStats += CTString("^c0000FF")+CenterString(TRANS("OTHER PLAYERS"))+"^r\n";
		strStats += CTString("^cFF0000")+TRANS("OTHER PLAYERS")+"^r";
    strStats+="\n";
  ...
  void GetDetailStatsSP(CTString &strStats, INDEX iCoopType)
  {
    ...
    if (iCoopType<=1) {
      ...
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%d", (const char *) TRANS("TOTAL SCORE"), m_psGameStats.ps_iScore));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%s", (const char *) TRANS("DIFFICULTY"), (const char *) GetDifficultyString()));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%s", (const char *) TRANS("STARTED"), (const char *) GetStatsRealWorldStarted()));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%s", (const char *) TRANS("PLAYING TIME"), (const char *) TimeToString(GetStatsRealWorldTime())));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%dX", (const char *) TRANS("EXTRA ENEMY RATIO"), GetGameEnemyMultiplier()));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%d", (const char *) TRANS("ENEMY MAX"), GetSP()->sp_iEnemyMax));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%.0f%s", (const char *) TRANS("ENEMY SPEED"), GetSP()->sp_fEnemySpeedAdjuster*100, "%"));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%.0f%s", (const char *) TRANS("PROJECTILE SPEED"), GetSP()->sp_fProjectileSpeed*100, "%"));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%.0f%s", (const char *) TRANS("WEAPONS FIRE RATE"), GetSP()->sp_fWeaponsFireRate*100, "%"));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%.0f%s", (const char *) TRANS("WEAPONS DAMAGE"), GetSP()->sp_fWeaponsDamage*100, "%"));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%.0f%s", (const char *) TRANS("PLAYER DAMAGE"), GetSP()->sp_fPlayerDamageAdjuster*100, "%"));
      strStats+="\n";
...
//      strStats+=CTString("^cFFFFFF")+TRANS("THIS LEVEL")+"^r\n";
    ...
    /*if (iCoopType>=1) {
      if(m_strLevelStats!="") {
        //strStats += CTString("^cFFFFFF")+TRANS("Per level statistics") +"^r\n\n" + m_strLevelStats;
				strStats+=AlignString(CTString(0, "%s:^r\n%s", (const char *) TRANS("Per level statistics"), (const char *) m_strLevelStats));
      }
    }*/
  }
  ...
  void GetMSLegacyPlayerInf( INDEX iPlayer, CTString &strOut)
  {
    ...
    strKey.PrintF("\\player_%d\\%s", iPlayer, (const char*)GetPlayerName());
	  strOut+=strKey;
    if (GetSP()->sp_bUseFrags || GetSP()->sp_bCooperative) {
  ...
  {
    // if already received
    if (HasMessage(fnmMessage) && GetSP()->sp_bCooperative) {
      // do nothing
      return;
    }
    // add it to array
  ...
  CModelObject *GetModelForRendering(void)
  {
    ...
    BOOL bBlink = TRUE;
    if (tmSpawnInvulnerability>16) {
      bBlink = FALSE;
    }
    if (bBlink && tmSpawnInvulnerability>0 && tmNow-m_tmSpawned<tmSpawnInvulnerability) {
      // blink fast
 ...
 ************************************************************/
    ...
    CPlacement3D &plViewer, COLOR &colBlend, BOOL bCamera)
  {
    ...
    /*if (dwk_bBotView && GetFlags()&ENF_ALIVE) {
			if (m_penBot1!=NULL && dwk_iBotView==1) {
				if (((CPlayerBot&)*m_penBot1).m_penView!=NULL) {
					plViewer = ((CPlayerBot&)*m_penBot1).m_penView->GetLerpedPlacement();
					penViewer = ((CPlayerBot&)*m_penBot1).m_penView;
				}
			} else if (m_penBot2!=NULL && dwk_iBotView==2) {
				if (((CPlayerBot&)*m_penBot2).m_penView!=NULL) {
					plViewer = ((CPlayerBot&)*m_penBot2).m_penView->GetLerpedPlacement();
					penViewer = ((CPlayerBot&)*m_penBot2).m_penView;
				}
			} else if (m_penBot3!=NULL && dwk_iBotView==3) {
				if (((CPlayerBot&)*m_penBot3).m_penView!=NULL) {
					plViewer = ((CPlayerBot&)*m_penBot3).m_penView->GetLerpedPlacement();
					penViewer = ((CPlayerBot&)*m_penBot3).m_penView;
				}
			} else if (m_penBot4!=NULL && dwk_iBotView==4) {
				if (((CPlayerBot&)*m_penBot4).m_penView!=NULL) {
					plViewer = ((CPlayerBot&)*m_penBot4).m_penView->GetLerpedPlacement();
					penViewer = ((CPlayerBot&)*m_penBot4).m_penView;
				}
			} else if (m_penBot5!=NULL && dwk_iBotView==5) {
				if (((CPlayerBot&)*m_penBot5).m_penView!=NULL) {
					plViewer = ((CPlayerBot&)*m_penBot5).m_penView->GetLerpedPlacement();
					penViewer = ((CPlayerBot&)*m_penBot5).m_penView;
				}
			} else if (m_penBot6!=NULL && dwk_iBotView==6) {
				if (((CPlayerBot&)*m_penBot6).m_penView!=NULL) {
					plViewer = ((CPlayerBot&)*m_penBot6).m_penView->GetLerpedPlacement();
					penViewer = ((CPlayerBot&)*m_penBot6).m_penView;
				}
			}
    }*/
    ...
    ANGLE aFOV = 90.0f; //plr_fFOV;

    // if sniper active
    if (((CPlayerWeapons&)*m_penWeapons).m_iCurrentWeapon!=WEAPON_CHAINSAW && m_ulFlags&PLF_ISZOOMING)
    {
      aFOV = Lerp(((CPlayerWeapons&)*m_penWeapons).m_fSniperFOVlast,
			...
			aFOV = m_fFOV;
			/*if (m_bDoSeriousSpeedWarp) {
				//TIME tmNow = _pTimer->GetLerpedCurrentTick();
				m_fFOV += 0.25f;
				aFOV = m_fFOV;
				if (m_fFOV>=110) {
					m_fFOV = 110.0f;
					m_bDoSeriousSpeedWarp = FALSE;
				}
			} else {
				TIME tmNow = _pTimer->GetLerpedCurrentTick();
				if (m_tmSeriousSpeed<tmNow) {
					m_fFOV = 90.0f;
				} else if (m_tmSeriousSpeed<tmNow+0.8f) {
					m_fFOV -= 0.25f;
					if (m_fFOV<=90) {
						m_fFOV = 90.0f;
					}
    ...
    apr = prPerspectiveProjection; //*** flying crash line
...
//      pdp->PutTextCXY( strMsg, pixDPWidth*0.5f, pixDPHeight*0.5f, SE_COL_BLUE_NEUTRAL_LT|CT_OPAQUE);
      strMsg.PrintF(TRANS("Error: Parse Error"));
      pdp->PutTextCXY( strMsg, pixDPWidth*0.5f, pixDPHeight*0.5f, C_GRAY|255);
    }
  }

	/*void RenderWaterFog(void)
	{
  ...
  void RenderPlayerView(CDrawPort *pdp, BOOL bShowExtras)
  {
    ...
    for (INDEX iEye=STEREO_LEFT; iEye<=(Stereo_IsEnabled()?STEREO_RIGHT:STEREO_LEFT); iEye++) {
      ...
      RenderView(*en_pwoWorld, *penViewer, apr, *pdp); //*** flying crash line, apr
...
/*
    if (_pTimer->CurrentTick()<m_tmAnalyseEnd) {
      ...
      pdp->PutTextCXY( TRANS("Analyzing..."), pixDPWidth*0.5f, pixDPHeight*0.2f, SE_COL_BLUE_NEUTRAL_LT|ubA);
		...
		// Fragman's Display code
		/*{FOREACHINDYNAMICCONTAINER(m_dcDisplay, CEntity, iten) { 
			CEntity *penen = iten; 
			if(penen){ 
				CDisplay *pen=(CDisplay *)penen; 
				if(pen&&pen->m_bActive){ 
					if(pen->m_eDT==EDT_CONSOLE){ 
						CPrintF(pen->m_strParsed); 
					}else if(pen->m_eDT==EDT_TEXT){ 
						DrawBars(this,pdp,pen,FALSE); 
						if(pen->m_eft==EFT_NORMAL){ 
						pdp->SetFont( _pfdDisplayFont); 
						}else if(pen->m_eft==EFT_XBOX){ 
						static CFontData _fdNumbersFont; 
						DECLARE_CTFILENAME( fnFont, "Fonts\\Display3-caps.fnt"); 
						_fdNumbersFont.Load_t( fnFont); 
						pdp->SetFont( &_fdNumbersFont); 
						}else if(pen->m_eft==EFT_CONSOLE){ 
						pdp->SetFont( _pfdConsoleFont); 
						} 
						pdp->SetTextScaling( pen->m_fScale); 
						pdp->SetTextAspect( 1.0f); 
						pdp->PutTextCXY( pen->m_strParsed, pixDPWidth*pen->m_fPosX, pixDPHeight*pen->m_fPosY, C_WHITE|0xDD); 
						
					}else if(pen->m_eDT==EDT_BAR){ 
						DrawBars(this,pdp,pen,TRUE);                           
					} 
  ...
  void RenderGameView(CDrawPort *pdp, void *pvUserData)
  {
    ...
    //CPrintF("%s: render\n", GetPredictName());
		/*CPrintF("SupportedExtensions: %s\n", _pGfx->go_strSupportedExtensions);
		SupportedExtensions: WGL_ARB_extensions_string, GL_ARB_multitexture, GL_EXT_texture_env_combine,
		GL_ARB_texture_compression, GL_EXT_texture_compression_s3tc, GL_S3_s3tc, GL_EXT_texture_lod_bias,
		GL_EXT_texture_filter_anisotropic, GL_EXT_compiled_vertex_array, WGL_EXT_swap_control, GL_EXT_texture_edge_clamp 
		*/
    ...
    if (bDualHead||m_penCamera==NULL) {
      ...
      if(dpView.Lock()) {
        ...
        dpView.Unlock(); //*** flying crash line
      }
    }
...
//*/
    ...
    if (GetSP()->sp_gdGameDifficulty==CSessionProperties::GD_TOURIST && GetFlags()&ENF_ALIVE) {
      ...
      FLOAT fTopHealth = TopHealth();
      if (fHealth<fTopHealth) {
        SetHealth(ClampUp(fHealth+_pTimer->TickQuantum*2, fTopHealth));  // 2 units per second
			}
		}

		if (GetSP()->sp_bUsePEHealth && GetFlags()&ENF_ALIVE && GetSP()->sp_gdGameDifficulty!=CSessionProperties::GD_TOURIST) {
			if (((CPlayerWeapons&)*m_penWeapons).m_iCurrentWeapon==WEAPON_KNIFE
				|| ((CPlayerWeapons&)*m_penWeapons).m_iCurrentWeapon==WEAPON_CHAINSAW) {
				// slowly increase health with time
				FLOAT fHealth = GetHealth();
				FLOAT fTopHealth = TopHealth();
				if (fHealth<fTopHealth) {
					SetHealth(ClampUp(fHealth+_pTimer->TickQuantum*4, fTopHealth));  // 4 units per second
				}
			}
		}
 ...
 ************************************************************/
                  ...
                  FLOAT fDamageAmmount, const FLOAT3D &vHitPoint, const FLOAT3D &vDirection)
  {
    ...
    if (dmtType == DMT_ABYSS || dmtType == DMT_SPIKESTAB || dmtType == DMT_ACID) {
      return;
    }
                      ...
                      FLOAT fDamageAmmount, const FLOAT3D &vHitPoint, const FLOAT3D &vDirection)
  {
    ...
    if (tmSpawnInvulnerability>0 && _pTimer->CurrentTick()-m_tmSpawned<tmSpawnInvulnerability && 
        dmtType!=DMT_ABYSS && dmtType!=DMT_TELEPORT) {
      // ignore damage
      return;
    }
    ...
    if (IsOfClass(penInflictor, "PlayerBot")) {
      return;
    }
    ...
    // don't harm yourself Self Damage is turned off
    if( penInflictor == this && !GetSP()->sp_bDamageSelf ) {
      return;
    }
    ...
    // don't hurt yourself swashing, or with rockets if self damage is off 
    if( dmtType==DMT_IMPACT || dmtType==DMT_BRUSH || dmtType==DMT_ACID) {
      return;
    }

		// no telefrags unless it's a player and FF is on
    ...
    if( fDifficultyDamage>=0 && penInflictor!=this) {
      fDamageAmmount /= fDifficultyDamage;
    }*/

		// adjust for session properties player damage
		FLOAT fPlayerDamageAdjuster = GetSP()->sp_fPlayerDamageAdjuster;
    if(penInflictor!=this) {
      fDamageAmmount *= fPlayerDamageAdjuster;
    }

		// keep self damage consistent
    FLOAT fWeaponsDamage = GetSP()->sp_fWeaponsDamage;
    if (fWeaponsDamage>0 && penInflictor==this) {
      fDamageAmmount = (fDamageAmmount/fWeaponsDamage)*0.15f;
    }
    ...
    if( fDamageAmmount>1.0f) {
      ...
      /*if( dmtType==DMT_EXPLOSION || dmtType==DMT_PROJECTILE || dmtType==DMT_BULLET
       || dmtType==DMT_IMPACT    || dmtType==DMT_CANNONBALL || dmtType==DMT_CANNONBALL_EXPLOSION) {
        GiveImpulseTranslationAbsolute( vDirection*(fDamageAmmount/7.5f)-en_vGravityDir*(fDamageAmmount/15.0f));
      }*/
      if( GetFlags()&ENF_ALIVE) {
 ...
 ************************************************************/
  ...
  BOOL ReceiveItem(const CEntityEvent &ee)
  {
    ...
    /*else if (ee.ee_slEvent == EVENTCODE_EAmmoPackItem) {
      ...
      EAmmoPackItem &eAmmoPackItem = (EAmmoPackItem&)ee;
			if(eAmmoPackItem.aptPackType==APT_ARMOR) {
				// determine old and new health values
				FLOAT fArmorOld = m_fArmor;
				FLOAT fArmorNew = fArmorOld + 200.0f;
				fArmorNew = ClampUp( fArmorNew, MaxArmor());
				// if value can be changed
				if( ceil(fArmorNew) > ceil(fArmorOld)) {
					// receive it
					m_fArmor = fArmorNew;
					ItemPicked( TRANS("Armor"), 200.0f);
				}
				return TRUE;
			} else {
				return ((CPlayerWeapons&)*m_penWeapons).ReceivePackAmmo(ee);
    ...
    // *********** KEYS ***********
      ...
      /*if (GetSP()->sp_bCooperative && !GetSP()->sp_bSinglePlayer) {
        CPrintF(TRANSV("^cFFFFFF%s - %s^r\n"), (const char *) GetPlayerName(), strKey);
      }*/
      return TRUE;
    }
    ...
    // *********** POWERUPS ***********
      ...
      case PUIT_INVISIB :  m_tmInvisibility    = tmNow + 60.0f;
        ItemPicked(TRANS("^cABE3FFInvisibility"), 0);
        return TRUE;
      //case PUIT_INVULNER:  m_tmInvulnerability = tmNow + m_tmInvulnerabilityMax;
      case PUIT_INVULNER:  
        if (m_tmInvulnerability>tmNow) {
          m_tmInvulnerability += 120.0f;
        } else {
          m_tmInvulnerability = tmNow + 120.0f;
        }
      ...
      case PUIT_DAMAGE  : 
        if (m_tmSeriousDamage>tmNow) {
          ...
          if (m_penBot1!=NULL) {
            m_penBot1->SendEvent(EStartAttack());
          }
          if (m_penBot2!=NULL) {
            m_penBot2->SendEvent(EStartAttack());
          }
          if (m_penBot3!=NULL) {
            m_penBot3->SendEvent(EStartAttack());
          }
          if (m_penBot4!=NULL) {
            m_penBot4->SendEvent(EStartAttack());
          }
          if (m_penBot5!=NULL) {
            m_penBot5->SendEvent(EStartAttack());
          }
          if (m_penBot6!=NULL) {
            m_penBot6->SendEvent(EStartAttack());
          }
        } else {
          m_tmSeriousDamage = tmNow + 120.0f;
          if (m_penBot1!=NULL) {
            m_penBot1->SendEvent(EStopAttack());
          }
          if (m_penBot2!=NULL) {
            m_penBot2->SendEvent(EStopAttack());
          }
          if (m_penBot3!=NULL) {
            m_penBot3->SendEvent(EStopAttack());
          }
          if (m_penBot4!=NULL) {
            m_penBot4->SendEvent(EStopAttack());
          }
          if (m_penBot5!=NULL) {
            m_penBot5->SendEvent(EStopAttack());
          }
          if (m_penBot6!=NULL) {
            m_penBot6->SendEvent(EStopAttack());
          }
        }
      ...
      case PUIT_SPEED   :  
        if (m_tmSeriousSpeed>tmNow) {
          ...
          if (m_penBot1!=NULL) {
            m_penBot1->SendEvent(EStart());
          }
          if (m_penBot2!=NULL) {
            m_penBot2->SendEvent(EStart());
          }
          if (m_penBot3!=NULL) {
            m_penBot3->SendEvent(EStart());
          }
          if (m_penBot4!=NULL) {
            m_penBot4->SendEvent(EStart());
          }
          if (m_penBot5!=NULL) {
            m_penBot5->SendEvent(EStart());
          }
          if (m_penBot6!=NULL) {
            m_penBot6->SendEvent(EStart());
          }
        } else {
					m_bDoSeriousSpeedWarp = TRUE;
          m_tmSeriousSpeed = tmNow + 120.0f;
          if (m_penBot1!=NULL) {
            m_penBot1->SendEvent(EStop());
          }
          if (m_penBot2!=NULL) {
            m_penBot2->SendEvent(EStop());
          }
          if (m_penBot3!=NULL) {
            m_penBot3->SendEvent(EStop());
          }
          if (m_penBot4!=NULL) {
            m_penBot4->SendEvent(EStop());
          }
          if (m_penBot5!=NULL) {
            m_penBot5->SendEvent(EStop());
          }
          if (m_penBot6!=NULL) {
            m_penBot6->SendEvent(EStop());
          }
        }
...
//    }
  ...
  void UsePressed(BOOL bOrComputer)
  {
    ...
    if (pen!=NULL) {
      ...
      if (IsOfClass( pen, "Switch") && penWeapons->m_fRayHitDistance < 6.5f) {
        CSwitch &enSwitch = (CSwitch&)*pen;
        // if switch is useable
        if (enSwitch.m_bUseable /*&& enSwitch.m_bCanUse*/) {
    ...
    else if (!bSomethingToUse)
    {
      ...
      if (penWeapon->m_iCurrentWeapon!=WEAPON_CHAINSAW) {
...
//    CPrintF("---APPLY: %g\n", paOriginal.pa_aRotation(1));
    ...
    if (Abs(paAction.pa_vTranslation(3))< plr_fSpeedForward/1.99f
      && Abs(paAction.pa_vTranslation(1))< plr_fSpeedSide/1.99f
			&& Abs(paAction.pa_vTranslation(2))< plr_fSpeedSide/1.44f) {
      // don't allow falling
...
//          (const char *) pcNew.GetNameForPrinting(), (const char*)ppsNew->GetModelFilename());
  ...
  void ActiveActions(const CPlayerAction &paAction)
  {
    ...
    if (pbsc==NULL && GetSP()->sp_bControlFlying) {
      //CPrintF( "NO Sector....\n");
      // teleport 
      this->Teleport(m_plFlySpawn, FALSE);
    }
    ...
    vTranslation(1) *= GetGameSpeedAdjuster();
    vTranslation(3) *= GetGameSpeedAdjuster(); 
    en_fDeceleration *= GetGameSpeedAdjuster()*1.5f;
    ...
    if (!(GetPhysicsFlags()&EPF_TRANSLATEDBYGRAVITY)) {
      SetDesiredRotation(ANGLE3D(paAction.pa_aRotation(1),0,0));
    ...
    // normal mode
      ...
      if (en_tmJumped+_pTimer->TickQuantum>=_pTimer->CurrentTick() &&
        ...
        m_ulFlags&=~PLF_JUMPALLOWED;
				/*if (m_penBot1!=NULL) {
					m_penBot1->SendEvent(EJump());
				}
				if (m_penBot2!=NULL) {
					m_penBot2->SendEvent(EJump());
				}
				if (m_penBot3!=NULL) {
					m_penBot3->SendEvent(EJump());
				}
				if (m_penBot4!=NULL) {
					m_penBot4->SendEvent(EJump());
				}
				if (m_penBot5!=NULL) {
					m_penBot5->SendEvent(EJump());
				}
				if (m_penBot6!=NULL) {
					m_penBot6->SendEvent(EJump());
				}*/
      }
...
//            CPrintF(TRANSV("%s is riding the gun again\n"), (const char *) GetPlayerName());
  ...
  void ButtonsActions( CPlayerAction &paAction)
  {
    ...
    if (ulNewButtons&PLACT_FIRE_SECONDARY) {
      bFiring = TRUE;
      ((CPlayerWeapons&)*m_penWeapons).SendEvent(ESecondaryFire());
    }
...
//      }
    ...
    } else if (ulNewButtons&PLACT_SNIPER_USE && ((CPlayerWeapons&)*m_penWeapons).m_iCurrentWeapon!=WEAPON_CHAINSAW) {
      UsePressed(FALSE);
    // if computer is pressed
  ...
  void ApplySniperZoom( BOOL bZoomIn )
  {
    ...
    if (((CPlayerWeapons&)*m_penWeapons).m_iCurrentWeapon==WEAPON_CHAINSAW ||
 ...
 ************************************************************/
  ...
  void GetLerpedAbsoluteViewPlacement(CPlacement3D &plView) {
    ...
    if (m_iViewState == PVT_PLAYEREYES || _bDiscard3rdView || m_ulFlags&PLF_ISZOOMING) {
  ...
  /*CEntity *GetViewEntity(void) {
    if (dwk_bBotView && GetFlags()&ENF_ALIVE) {
			if (dwk_iBotView>6 || dwk_iBotView<1
				|| (m_penBot2==NULL && dwk_iBotView>1)
				|| (m_penBot3==NULL && dwk_iBotView>2) 
				|| (m_penBot4==NULL && dwk_iBotView>3) 
				|| (m_penBot5==NULL && dwk_iBotView>4) 
				|| (m_penBot6==NULL && dwk_iBotView>5) 	) {
				dwk_iBotView = 1;
			}
			if (m_penBot1!=NULL && dwk_iBotView==1) {
				return ((CPlayerBot *) m_penBot1.ep_pen)->m_penView;
			} else if (m_penBot2!=NULL && dwk_iBotView==2) {
				return ((CPlayerBot *) m_penBot2.ep_pen)->m_penView;
			} else if (m_penBot3!=NULL && dwk_iBotView==3) {
				return ((CPlayerBot *) m_penBot3.ep_pen)->m_penView;
			} else if (m_penBot4!=NULL && dwk_iBotView==4) {
				return ((CPlayerBot *) m_penBot4).ep_pen->m_penView;
			} else if (m_penBot5!=NULL && dwk_iBotView==5) {
				return ((CPlayerBot *) m_penBot5.ep_pen)->m_penView;
			} else if (m_penBot6!=NULL && dwk_iBotView==6) {
				return ((CPlayerBot *) m_penBot6.ep_pen)->m_penView;
			} else {
        // player eyes
        if (m_iViewState == PVT_PLAYEREYES) {
          return this;
                  ...
                  BOOL bRenderWeapon, INDEX iEye)
  {
    ...
    if( tmSinceWounding<4.0f) {
      ...
      if( tmSinceWounding<0.001f) { ulA = (ulA+64)/2; }
		...
		//CPrintF("ubR: %d, ubG: %d, ubB: %d, ubA: %d\n", ulR, ulG, ulB, ulA);
    ...
    // add world glaring
		...
		UBYTE ubR, ubG, ubB, ubA;
		//UBYTE ubH, ubS, ubV;
		//ColorToHSV(colGlare, ubH, ubS, ubV);
		ColorToRGBA(colGlare, ubR, ubG, ubB, ubA);
		...
		pdp->dp_ulBlendingBA -= ULONG(42);
		pdp->dp_ulBlendingGA += ULONG(5);
		pdp->dp_ulBlendingBA += ULONG(42);
    ...
    if( hud_bShowInfo) { 
      ...
      if( tmDelta<plr_tmSnoopingTime) {
        ...
        if (IsDerivedFromClass(pen->m_penTargeting, "Player")) {
          penHUDPlayer = (CPlayer*) pen->m_penTargeting.ep_pen; // bot snooping crash
        }
        bSnooping = TRUE;
      }
 ...
 ************************************************************/
  ...
  void InitializePlayer()
  {
    ...
    Particles_RunAfterBurner_Prepare(this);
  ...
  void TeleportPlayer(enum WorldLinkType EwltType) 
  {
    ...
    // if start marker is found
			...
			CTString strMarkerName = pen->GetName();
			//CPrintF("Marker Name ,%s,\n", strMarkerName);
			//CPrintF("Level Name ,%s,\n", strLevelName);
			if (strLevelName == "1_3_Teotihuacan" && !GetSP()->sp_bSinglePlayer) {
				if (strMarkerName == "Player Start - " 
					|| strMarkerName == "Player Start - Valley of the Jaguar - The City of the Gods") {
					m_iMoveMarker = 1;
				} else {
					m_iMoveMarker = 0;
				}
			} else if (strLevelName == "1_1_Palenque" && !GetSP()->sp_bSinglePlayer) {
				if (strMarkerName == "Player Start - ") {
					m_iMoveMarker = 2;
				} else {
					m_iMoveMarker = 0;
      ...
      if (bSetHealth) {
				FLOAT fHealth = GetHealth();
				if (fHealth<CpmStart.m_fHealth/100.0f*TopHealth()) {
					SetHealth(CpmStart.m_fHealth/100.0f*TopHealth());
				}
        m_iMana  = GetSP()->sp_iInitialMana;
				if (CpmStart.m_fShield>m_fArmor) {
      ...
      ((CPlayerWeapons&)*m_penWeapons).InitializeWeapons(CpmStart.m_iGiveWeapons, 0, 0,CpmStart.m_fMaxAmmoRatio);
       ...
       //m_iMoveMarker = 3;
      // start position relative to link, or offset for lagamuffin areas
			if (m_iMoveMarker==1) { 
        //Teleport(CPlacement3D(FLOAT3D(-339.25f, -96.0f, -701.5f)+vOffsetRel, ANGLE3D(90, 0, 0)));
				m_iSeriousBombCount += 2;
        Teleport(CPlacement3D(FLOAT3D(-216.0f, 0, 107.0f)+vOffsetRel, ANGLE3D(0, 0, 0)));
			} else if (m_iMoveMarker==2) {
        Teleport(CPlacement3D(FLOAT3D(480.0f, 42.0f, -42.0f)+vOffsetRel, ANGLE3D(0, 0, 0)));
			} else if (m_iMoveMarker==3) {
				m_iSeriousBombCount += 20;
				m_tmInvulnerability = 1820.0f; 
				m_tmSeriousDamage   = 1820.0f;
        //Teleport(CPlacement3D(FLOAT3D(476, -126, 255), ANGLE3D(270, 0, 0))); // end of the pit
        //Teleport(CPlacement3D(FLOAT3D(0, 8, 255), ANGLE3D(0, 0, 0))); // babel test spot, first outside area
        //Teleport(CPlacement3D(FLOAT3D(0, 20, 75), ANGLE3D(0, 0, 0))); // babel test spot, exotech larva
        //Teleport(CPlacement3D(FLOAT3D(431, 110, 34), ANGLE3D(270, 0, 0))); // valley of the jag, right jag area 
        //Teleport(CPlacement3D(FLOAT3D(0, 1, 385), ANGLE3D(0, 0, 0))); // atrium first outside area
        //Teleport(CPlacement3D(FLOAT3D(-850, 1, -239), ANGLE3D(90, 0, 0))); // summoner
        //Teleport(CPlacement3D(FLOAT3D(-244, 163, -513), ANGLE3D(90, 0, 0))); // zzcanyon bouncer
        //Teleport(CPlacement3D(FLOAT3D(294, 84, -448), ANGLE3D(30, 0, 0))); // zzcanyon bouncer
				//Teleport(CPlacement3D(FLOAT3D(0, 3, -238), ANGLE3D(0, 0, 0))); // TGP, area before pyramid
				//Teleport(CPlacement3D(FLOAT3D(89, 64, -296), ANGLE3D(270, 0, 0))); // Tomb of Ramses end area 
				Teleport(CPlacement3D(FLOAT3D(145, 81, -503), ANGLE3D(0, 0, 0))); // Tomb of Ramses end
				//Teleport(CPlacement3D(FLOAT3D(1000, 1, -32), ANGLE3D(270, 0, 0))); // end of Metro
				//Teleport(CPlacement3D(FLOAT3D(-4, 1, 2048), ANGLE3D(90, 0, 0))); // end battle of Karnak
				//Teleport(CPlacement3D(FLOAT3D(-128, 1, 2048), ANGLE3D(90, 0, 0))); // end of Karnak, statue placement
				//Teleport(CPlacement3D(FLOAT3D(1040, 1, 2048), ANGLE3D(90, 0, 0))); // end of the Alley
				//Teleport(CPlacement3D(FLOAT3D(-26, -94, -537), ANGLE3D(0, 0, 0))); // Valley Mesh floor room 
				//Teleport(CPlacement3D(FLOAT3D(-47, -12, 90), ANGLE3D(270, 0, 0))); // end of sand canyon, element pickup
      } else if (EwltType == WLT_RELATIVE) { 
  ...
  {
    FLOAT tmNow = _pTimer->GetLerpedCurrentTick();
    if (tmNow < m_tmSpawned+5.0f) {
      return;
    }
    
    // render empty shells
		if (GetPlayerWeapons()->m_iCurrentWeapon==WEAPON_COLT || GetPlayerWeapons()->m_iCurrentWeapon==WEAPON_DOUBLECOLT) {
			Particles_EmptyShells( this, m_asldData, 1);
		} else {
			Particles_EmptyShells( this, m_asldData, 0);
		}
    ...
    else
    {
      ...
      if (GetFlags()&ENF_ALIVE){
        if (m_tmSeriousDamage>tmNow && m_tmInvulnerability>tmNow) {
          Particles_ModelGlow(this, Max(m_tmSeriousDamage,m_tmInvulnerability),PT_GLOW3, 0.3f, 2, 0.03f, 0x2E183400);
        } else if (m_tmInvulnerability>tmNow) {
          Particles_ModelGlow(this, m_tmInvulnerability, PT_GLOW2, 0.3f, 2, 0.03f, 0x1F254F00);
        } else if (m_tmSeriousDamage>tmNow) {
          Particles_ModelGlow(this, m_tmSeriousDamage, PT_GLOW, 0.3f, 2, 0.03f, 0x3E1E2000);
        }
        if (m_tmSeriousSpeed>tmNow) {
 ...
 ************************************************************/
  ...
  DoAutoActions(EVoid)
  {
    ...
    while (m_penActionMarker!=NULL && IsOfClass(m_penActionMarker, "PlayerActionMarker")) {
			//CPrintF("Player has a marker: %s\n", GetActionMarker()->m_strDescription );
      ...
      // if should run to the marker
        ...
        // go to it
        m_fAutoSpeed = 10.0f*GetActionMarker()->m_fSpeed;                                             
 ...
 ************************************************************/
  Main(EVoid evoid)
  {
    ...
    /*m_strBotName1 = bot_strName1;
    m_strBotName2 = bot_strName2;
    m_strBotName3 = bot_strName3;*/
    ...
    wait(0.4f) { // this is 4 ticks, it has to be at least more than musicchanger for enemy counting
      on (EBegin) : { resume; }
      on (ETimer) : { stop; }
      on (EDisconnected) : { 
        Destroy(); 
    ...
    // set speeds
    /*FLOAT fSpeedAdjustment = GetSP()->sp_fSpeedAdjuster;
    plr_fDeceleration *= fSpeedAdjustment*1.5f;
    plr_fSpeedForward *= fSpeedAdjustment; 
    plr_fSpeedBackward *= fSpeedAdjustment;
    plr_fSpeedSide *= fSpeedAdjustment;*/ 
...
*/
      ...
      on (ETouch eTouch) : {
        ...
        /*if (eTouch.penOther->GetRenderType()==CEntity::RT_BRUSH) {
          ...
          // if the touched plane is more or less orthogonal to the current velocity
					CPrintF("(eTouch.plCollision%vDir) = %.3f\n", (eTouch.plCollision%vDir));
    ...
    if (m_penBot1!=NULL) {
      m_penBot1->SendEvent(EDeath());
    }
    if (m_penBot2!=NULL) {
      m_penBot2->SendEvent(EDeath());
    }
    if (m_penBot3!=NULL) {
      m_penBot3->SendEvent(EDeath());
    }
    if (m_penBot4!=NULL) {
      m_penBot4->SendEvent(EDeath());
    }
    if (m_penBot5!=NULL) {
      m_penBot5->SendEvent(EDeath());
    }
    if (m_penBot6!=NULL) {
      m_penBot6->SendEvent(EDeath());
    }
    /*if (m_penStar!=NULL) {
