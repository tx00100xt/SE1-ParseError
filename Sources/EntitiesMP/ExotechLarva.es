...
#define PERCENT_RIGHTBLOW   0.2f
#define PERCENT_LEFTBLOW    0.1f
...
properties:
 ...
 17 FLOAT m_fLarvaHealth = 2000000.0f,
...
functions:                                        
    ...
    FLOAT fDamageAmmount, const FLOAT3D &vHitPoint, const FLOAT3D &vDirection) 
  {
    ...
    fDamageAmmount *= 0.15f/(_pNetwork->ga_sesSessionState.GetPlayersCount()+0.001f);
		fDamageAmmount = fDamageAmmount/(GetGameDamageMultiplier()+0.001f);

		FLOAT fPlayers = _pNetwork->ga_sesSessionState.GetPlayersCount();
		if (fPlayers<=1) {
			fPlayers = 1.0f;
		} else {
			fPlayers = 1.0f+(fPlayers*0.2f);
		}*/
  ...
  BOOL RechargerActive() {
    if (((CExotechLarvaCharger *) m_penRecharger.ep_pen)->m_bActive) { 
      //CPrintF("Larva - Charger Active\n");
      return TRUE;
    }
    //CPrintF("Larva - Charger Inactive\n");
    return FALSE;
  }
  ...
  void RenderParticles(void)
  {
    ...
    if (m_bRechargePose && ((CExotechLarvaCharger *) m_penRecharger.ep_pen)->m_bBeamActive)
    {
      Particles_LarvaEnergy(this, FLOAT3D(0.0f, LARVA_HANDLE_TRANSLATE, 0.0f)*m_fStretch);
		...
		if (!m_bExploded && GetSP()->sp_bMental) {
      Particles_ModelGlow3(this, PT_STAR07, 0.15f+m_fParticleSize, 1, m_colStar);
		}
  }
...
*/
  ...
  Fire(EVoid) : CEnemyBase::Fire { 
    ...
    if (m_bLeftArmActive || m_bRightArmActive) {
      m_iRnd = IRnd()%9;
      if (m_iRnd>3 && !m_bRechargePose && GetHealth()>0.1f*m_fMaxHealth) {
    ...
    // if no wings left fire laser
      ...
      if (m_iRnd<7 && _pTimer->CurrentTick()>m_tmDontFireLaserBefore) {
    ...
    wait () {
      ...
      on (EReminder er) :
      {
        ...
        if (er.iValue==128) {
          ...
          if (m_bActive && m_bRecharging && DistanceXZ(this, m_penRecharger)<50.0f) {
            if (m_bRechargePose) {
        ...
        // check to see if guided missile firing is needed
          ...
          FindNewTarget();        
          if (AnyPlayerCloserThen(30.0f) && GetHealth()>0.1f*m_fMaxHealth) {
            UpdateFiringPos();
            PlaySound(m_soFire3, SOUND_FIRE_TAIL, SOF_3D);
				    ShootTailProjectile();
          }
  ...
  Main(EVoid) {
    ...
    wait() {
      on (EBegin) : { 
        //CPrintF("Larva Main\n");
        //m_fLarvaHealth = 2.0f*1E10f*_pNetwork->ga_sesSessionState.GetPlayersCount();
        resume; }
      on (ETrigger) : { stop; }
      otherwise (): { resume; }
    }
