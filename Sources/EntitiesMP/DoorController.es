...
properties:
  ...
  // New
  20 BOOL m_bTeleport = FALSE,
  21 BOOL m_bJumpDummy = FALSE,
  22 BOOL m_bSetMods = TRUE,
...
functions:
  ...
  BOOL CanReactOnEntity(CEntity *pen)
  {
    ...
    if (m_bPlayersOnly && !(IsDerivedFromClass(pen, "Player") || IsOfClass(pen, "PlayerBot"))) {
      return FALSE;
    }
    ...
    // if not single player
		if (GetSP()->sp_bCooperative) {
			...
			if (strLevelName=="1_1_Palenque") {
				FLOAT DoorControllerX = GetPlacement().pl_PositionVector(1);
				if (DoorControllerX==456.0f) {
          m_bJumpDummy = TRUE;
        }
			} else if (strLevelName=="1_5_Teotihuacan") {
				FLOAT DoorControllerX = GetPlacement().pl_PositionVector(1);
				if (DoorControllerX==-35.0f) {
          m_bTeleport = TRUE;
				}
      }
		} 
...
procedures:
      ...
      wait() {
        ...
        on (EPass ePass) : {
          ...
          if (CanReactOnEntity(ePass.penOther)) {
            ...
            if (m_bTeleport && (IsOfClass(ePass.penOther, "Player") || IsOfClass(ePass.penOther, "PlayerBot"))) {
							m_penCaused = ePass.penOther;
							TeleportEntity(ePass.penOther, CPlacement3D(FLOAT3D(-26.0f, -97.0f, 320.0f), ANGLE3D(0, 0, 0)));
	          } else {
							// do it
							m_penCaused = ePass.penOther;
							TriggerDoor();
  ...
  DoorLocked()
  {
    while (TRUE) {
      ...
      wait() {
        on (EPass ePass) : {
          if (IsDerivedFromClass(ePass.penOther, "Player") || IsOfClass(ePass.penOther, "PlayerBot")) {
            // open the door
            TriggerDoor();
