...
properties:
 ...
 // Dk's shit
 //73 BOOL  m_bBotView = FALSE,
 //74 CEntityPointer m_penOwner,    // entity which owns it
 //75 FLOAT3D m_vZLast = FLOAT3D(0,0,0),
...
functions:
    ...
    if( tmCurrent > (m_tmAtMarker+m_tmDelta - _pTimer->TickQuantum*3/2)) 
    {
      ...
      FOREVER {
        ...
        if (pcmNp0->m_penTrigger!=NULL) {
          // trigger it
					//CPrintF("CameraMarker Trigger event sent by: Camera\n");
 ...
 /*class CPlayerBot *GetOwner(void)
  {
    ASSERT(m_penOwner!=NULL);
    return (CPlayerBot*) m_penOwner.ep_pen;
  }

  void SetCameraPosition() 
  {
    // 3rd person view
    FLOAT fDistance = 4.5f;
		CPlacement3D pl = GetOwner()->m_plView;
    ...
    FLOAT3D vDest = vBase+vFront*fDistance*2.0f;
    FLOAT fBack = 0;
    // cast a ray to find if any brush is hit
    CCastRay crRay( m_penOwner, vBase, vDest);
    ...
    if (crRay.cr_penHit!=NULL) {
      ...
      if (crRay.cr_pbpoBrushPolygon!=NULL) {
        // back off
        FLOAT3D vDir = (vDest-vBase).Normalize();
    ...
    while(TRUE)
    {
      wait() {
        ...
        on (ETrigger eTrigger) : {
					//CPrintF("Camera Triggered\n");
					//if (GetSP()->sp_bSinglePlayer && !m_bBotView) {
