...
#define SUMMONER_HEALTH 1500000000.0f
// info structure
...
properties:
 ...
 200 INDEX m_ctSpawnerProjectiles = 0,
 201 BOOL  m_bVisible = FALSE,
...
functions:
    ...
    FLOAT fDamageAmmount, const FLOAT3D &vHitPoint, const FLOAT3D &vDirection) 
  {
    ...
    /*if(dmtType==DMT_CANNONBALL || dmtType==DMT_BULLET || dmtType==DMT_ACID || dmtType==DMT_EXPLOSION)
    {
      fDamageAmmount *= 0.1f;
    }
		FLOAT fWeaponsDamage = GetSP()->sp_fWeaponsDamage;
		if (fWeaponsDamage<=0.25f) {
			fWeaponsDamage = 0.25f;
		}*/

		FLOAT fPlayers = _pNetwork->ga_sesSessionState.GetPlayersCount();
		if (fPlayers<=1) {
			fPlayers = 1.0f;
		} else {
			fPlayers = 1.0f+(fPlayers*0.1f);
		}

    fDamageAmmount = 15.0f/fPlayers;
		//fDamageAmmount = fDamageAmmount/fWeaponsDamage;

		//CPrintF("fDamageAmmount: %.0f, Players: %d, fWeaponsDamage: %.3f\n", fDamageAmmount, iPlayers, fWeaponsDamage);
...
//CPrintF("health: %f, scheme: %i\n", GetHealth(), m_iSpawnScheme);
  ...
  INDEX GetProjectileCount(void)
  {
    INDEX iCountP = 0;
    // for each entity in the world
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
			CEntity *pen = iten;
			if (IsDerivedFromClass(pen, "SpawnerProjectile")) {
        iCountP++;
      }
    }}
    return iCountP;
    ...
    for (INDEX i=0; i<apenNearEntities.Count(); i++)
    {
      if (IsDerivedFromClass(apenNearEntities[i], "Enemy Base") &&
        ...
        if (!((CEnemyBase &)*apenNearEntities[i]).m_bTemplate &&
          ...
          apenNearEntities[i]->SendEvent(eDeath2);
        }
      }*/
  ...
  void RenderParticles(void)
  {
    FLOAT tmNow = _pTimer->CurrentTick();
		BOOL bParticlesDisappear = FALSE;
    if( tmNow>m_tmParticlesDisappearStart && tmNow<m_tmParticlesDisappearStart+4.0f)
    {
      Particles_SummonerDisappear(this, m_tmParticlesDisappearStart);
			bParticlesDisappear = TRUE;
    }
		if (!m_bInvulnerable && !bParticlesDisappear && !m_bExploded && GetSP()->sp_bMental) {
      Particles_ModelGlow3(this, PT_STAR07, 0.15f+m_fParticleSize, 2, m_colStar);
		}
    ...
    if( tmNow>m_tmLastAnimation)
    {
      ...
      if(!m_bInvulnerable && !m_bExploded && GetHealth()>0)
      {
        for( INDEX iInter=0; iInter<ctInterpolations; iInter++)
        {
          ...
          //m_emEmiter.AddParticle(vPos, vSpeed*mRot*mEn, RAND_05*360.0f, fRotSpeed, tmBirth, fLife, fStretch, col, TRUE);
  ...
  void SendEndShowEvent(void)
  {
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
			CEntity *pen = iten;
      if (IsDerivedFromClass(pen, "EndShow")) {
        //CPrintF("End Show Event Sent\n");
  ...
  Fire(EVoid) : CEnemyBase::Fire {
    ...
    if (!m_bFiredThisTurn) {
      ...
      // if too much fuss, just laugh and initiate teleport
        ...
        INDEX iProjectileCount = GetProjectileCount();
        INDEX iToSpawn = 0;
        if (iProjectileCount>8) {
          iToSpawn = 0;
        } else if (iProjectileCount>6) {
          iToSpawn = 2;
        } else if (iProjectileCount>4) {
          iToSpawn = 4;
        } else if (iProjectileCount>2) {
          iToSpawn = 6;
        } else {
          iToSpawn = 8;
        }
        //CPrintF("iProjectileCount: %d, iToSpawn: %d\n", iProjectileCount, iToSpawn);
...
//    ShakeItBaby(_pTimer->CurrentTick(), 0.25f, TRUE);
    ...
    plStars = GetPlacement();
		plStars.pl_PositionVector(2) += 9.0f;
    ESpawnEffect eSpawnEffect;
...
//    ShakeItBaby(_pTimer->CurrentTick(), 5.0f, FALSE);
    ...
    if (GetSP()->sp_bSinglePlayer) {
      ...
      for (INDEX i=0; i<15; i++) {
        Debris_Spawn(this, this, MODEL_DEBRIS_FLESH, TEXTURE_SUMMONER, 0, 0, 1, 0, FRnd()*2.0f+1.0f, 
          FLOAT3D(FRnd()*10.0f-5.0f, FRnd()*1.0f+1.0f, FRnd()*10.0f-5.0f));  
        Debris_Spawn(this, this, MODEL_DEBRIS_FLESH, TEXTURE_DEBRIS_FLESH , 0, 0, 1, 0, FRnd()*2.0f+1.0f, 
          FLOAT3D(FRnd()*10.0f-5.0f, FRnd()*1.0f+1.0f, FRnd()*10.0f-5.0f));  
        ...
        CEntityPointer Debris_Spawn(
          ...
          Debris_Spawn(this, this, MODEL_DEBRIS01, TEXTURE_SUMMONER, 0, 0, 1, 0, m_fStretch, 
            FLOAT3D(FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f));
          Debris_Spawn(this, this, MODEL_DEBRIS_FLESH, TEXTURE_DEBRIS_FLESH , 0, 0, 1, 0, m_fStretch*0.33f, 
            FLOAT3D(FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f));        
          break;
        case 1:
          Debris_Spawn(this, this, MODEL_DEBRIS02, TEXTURE_SUMMONER, 0, 0, 1, 0, m_fStretch, 
            FLOAT3D(FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f));
          Debris_Spawn(this, this, MODEL_DEBRIS_FLESH, TEXTURE_DEBRIS_FLESH , 0, 0, 1, 0, m_fStretch*0.33f, 
            FLOAT3D(FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f));        
          break;
        case 2:
          Debris_Spawn(this, this, MODEL_DEBRIS03, TEXTURE_SUMMONER, 0, 0, 1, 0, m_fStretch, 
            FLOAT3D(FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f));
          Debris_Spawn(this, this, MODEL_DEBRIS_FLESH, TEXTURE_DEBRIS_FLESH , 0, 0, 1, 0, m_fStretch*0.33f, 
            FLOAT3D(FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f, FRnd()*0.6f+0.2f));                
          break;
 ...
 ************************************************************/
  Main(EVoid) {
    ...
    m_sptType = SPT_BLOOD;
    m_bBoss = TRUE;
		//FLOAT fHealth = 15000000.0f*5*_pNetwork->ga_sesSessionState.GetPlayersCount();
    //SetHealth(fHealth);
