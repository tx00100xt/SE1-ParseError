...
#ifdef PLATFORM_UNIX
extern "C" __attribute__ ((visibility("default"))) INDEX gam_bUZActive = FALSE;
extern "C" __attribute__ ((visibility("default"))) INDEX gam_iUZLives = 6;
#else
DECL_DLL extern INDEX gam_bUZActive = FALSE;
DECL_DLL extern INDEX gam_iUZLives = 6;
#endif
...
uses "EntitiesMP/Walker";
uses "EntitiesMP/Beast";
...
extern INDEX cht_bDFB;
extern INDEX cht_bDFBD;
extern INDEX cht_bDFBA;
...
#define ATT_PROJECTILE_GUN    (FLOAT3D(-0.703544f, 0.6f, -0.029834f)*SIZE)
#define ATT_LASER             (FLOAT3D(0.63626f, 0.6f, 0.033062f)*SIZE)
...
#define MAGIC_PROJECTILE_EXIT (FLOAT3D(0.035866f, 1.38f, -0.82264f)*SIZE)
...
#define BOSS_HEALTH (40000.0f)
#define HEALTH_IMPULSE (10000.0f)
#define HEALTH_CLASS_1 (5000)
#define HEALTH_CLASS_2 (7500)
#define HEALTH_CLASS_3 (10000)
#define HEALTH_CLASS_4 (15000)
#define CLASS_2_CANNON_FACTOR 0.5f
#define CLASS_3_ROCKETLAUNCHER_FACTOR 0.5f
...
properties:
  ...
  1000 INDEX m_iThing = 0,
	1001 INDEX m_ctEnemiesInWorld = 0,
	1002 INDEX m_ctEnemyMax = 0,
	1003 INDEX m_ctSpawnCounter = 0,
	1004 INDEX m_ctCritterCounter = 0,
	1005 INDEX m_ctEventCount = 0,
	1006 INDEX m_iRndCounter = 0,
	1007 FLOAT m_fFireStrength = 0.0f,
	1008 INDEX m_iPlayerSwitchCounter = 0,
  1009 FLOAT m_fDamageAdjustment = 0.0f,
  1010 BOOL  m_bRegenerating = FALSE,
...
components:
  ...
  2 class   CLASS_EFFECTOR    "Classes\\Effector.ecl",
	3 class   CLASS_WALKER      "Classes\\Walker.ecl",
	4 class   CLASS_BONEMAN     "Classes\\Boneman.ecl",
	5 class   CLASS_WEREBULL    "Classes\\Werebull.ecl",
	6 class   CLASS_HEADMAN     "Classes\\Headman.ecl",
	7 class   CLASS_WOMAN       "Classes\\Woman.ecl",
	8 class   CLASS_EYEMAN      "Classes\\Eyeman.ecl",
...
// ************** SOUNDS **************
 ...
 90 model   MODEL_EYEMAN							"Models\\Enemies\\Eyeman\\Eyeman.mdl",
 91 texture TEXTURE_EYEMAN_SERGEANT   "Models\\Enemies\\Eyeman\\Eyeman5.tex",
 93 model   MODEL_WOMAN								"Models\\Enemies\\Woman\\Woman.mdl",
 94 texture TEXTURE_WOMAN							"Models\\Enemies\\Woman\\Woman.tex",
...
// ************** SOUNDS **************
  ...
  void SetSpeedsToDesiredPosition(const FLOAT3D &vPosDelta, FLOAT fPosDist, BOOL bGoingToPlayer)
  {
    if(m_penEnemy!=NULL)
    {
      ...
      FLOAT fSpeedRadius1 = 1.0f;
      FLOAT fSpeedRadius2 = 1.0f;
  ...
  void InflictHoofDamage( FLOAT3D vOffset)
  {
    ...
    InflictRangeDamage(this, DMT_EXPLOSION, 1000.0f, vFootAbs, DEVIL_HOOF_RADIUS, DEVIL_HOOF_RADIUS);
  }
...
//    ApplyFootQuake();
    ...
    if ( _pTimer->CurrentTick() < m_tmTemp+TM_HEALTH_IMPULSE && GetHealth()<=HEALTH_CLASS_4){
  ...
  {
    // inflict less damage then the default
	  /*if(dmtType==DMT_CANNONBALL || dmtType==DMT_BULLET || dmtType==DMT_PROJECTILE) { 
			fDamageAmmount *= 0.1f; 
		}
    if (dmtType==DMT_ACID) { 
			fDamageAmmount *= 0.1f; 
    ...
    //fDamageAmmount *= 2.0f;
    fDamageAmmount = fDamageAmmount/(_pNetwork->ga_sesSessionState.GetPlayersCount()+0.001f);
    fDamageAmmount = fDamageAmmount/m_fDamageAdjustment;

		INDEX iPlayers = _pNetwork->ga_sesSessionState.GetPlayersCount();
		if (iPlayers<=1) {
			iPlayers = 1;
		}*/

    fDamageAmmount = 25.0f; //iPlayers;
		//CPrintF("fDamageAmmount: %.0f, Players: %d\n", fDamageAmmount, iPlayers);

    if(_pTimer->CurrentTick()<m_tmHitBySpaceShipBeam+1.5f) {
      fDamageAmmount *= 5.0f;
    }
...
//*/
  ...
  void FireOneLargeWalker(void)
  {
    ...
    ANGLE3D aWpnAng = GetPlacement().pl_OrientationAngle;

		CPlacement3D pl;
    CEntityPointer pen = NULL;
    pl = CPlacement3D(vWpnPipeAbs+FLOAT3D(0,-8,-10), aWpnAng + ANGLE3D(-45+(FRnd()*90), -30+(FRnd()*60), 0));
    pen = CreateEntity(pl, CLASS_WALKER);

    // start properties
		((CWalker&)*pen).m_bStretchModel = TRUE;
    ((CWalker&)*pen).m_EwcChar = WLC_SERGEANT;
    ((CWalker&)*pen).m_penEnemy = m_penEnemy;
		((CWalker&)*pen).m_bHasBeenSpawned = TRUE;
		((CWalker&)*pen).m_bTemplate = FALSE;
    ((CWalker&)*pen).m_bPESpawned = TRUE;
    ((CWalker&)*pen).m_tmSpawned = _pTimer->CurrentTick()+0.3f;
		((CWalker&)*pen).Initialize();
		// fire it
		((CWalker&)*pen).LaunchAsFreeProjectile(FLOAT3D(0, 0, -60/m_fFireStrength), this);
    ((CWalker&)*pen).SetDesiredRotation(ANGLE3D(0, 0, 0));
    // increase the number of spawned enemies
		m_ctCritterCounter++;
    if (m_ctCritterCounter==1) {
      m_ctEnemyMax += 20;
    }
		if (m_ctCritterCounter==m_ctEventCount) {
			OpenPyramidDoors();
		}
  ...
  void FireOneSmallWalker(void) 
  {
    ...
    ANGLE3D aWpnAng = GetPlacement().pl_OrientationAngle;
		...
		CPlacement3D pl;
    CEntityPointer pen = NULL;
    pl = CPlacement3D(vWpnPipeAbs, aWpnAng + ANGLE3D(-45+(FRnd()*90), -30+(FRnd()*60), 0));
    pen = CreateEntity(pl, CLASS_WALKER);
    ...
    ((CWalker&)*pen).m_tmSpawned = _pTimer->CurrentTick()+0.3f;
		((CWalker&)*pen).Initialize();
		// fire it
		((CWalker&)*pen).LaunchAsFreeProjectile(FLOAT3D(0, 0, -65/m_fFireStrength), this);
    ((CWalker&)*pen).SetDesiredRotation(ANGLE3D(0, 0, 0));
    // increase the number of spawned enemies
		m_ctCritterCounter++;
    if (m_ctCritterCounter==1) {
  ...
  void FireOneKleer(void)
  {
    ...
    // start properties
		((CBoneman&)*pen).m_bStretchModel = TRUE;
    ((CBoneman&)*pen).m_penEnemy = m_penEnemy;
		((CBoneman&)*pen).m_bHasBeenSpawned = TRUE;
		((CBoneman&)*pen).m_bTemplate = FALSE;
    ((CBoneman&)*pen).m_bPESpawned = TRUE;
    ((CBoneman&)*pen).m_tmSpawned = _pTimer->CurrentTick()+0.3f;
		((CBoneman&)*pen).Initialize();
		// fire it
		((CBoneman&)*pen).LaunchAsFreeProjectile(FLOAT3D(0, 0, -60/m_fFireStrength), this);
    ((CBoneman&)*pen).SetDesiredRotation(ANGLE3D(0, 0, 0));
    // increase the number of spawned enemies
		m_ctCritterCounter++;
    if (m_ctCritterCounter==1) {
      m_ctEnemyMax += 20;
  ...
  void FireOneEyemanWoman(void)
  {
		const FLOATmatrix3D &m = GetRotationMatrix();
    FLOAT3D vWpnPipeAbs = GetPlacement().pl_PositionVector+MAGIC_PROJECTILE_EXIT*m;
    ANGLE3D aWpnAng = GetPlacement().pl_OrientationAngle;
    ...
    pl = CPlacement3D(vWpnPipeAbs, aWpnAng + ANGLE3D(-45+(FRnd()*90), -30+(FRnd()*60), 0));
		...
		((CEyeman&)*pen).m_bStretchModel = TRUE;
		((CEyeman&)*pen).m_penEnemy = m_penEnemy;
		((CEyeman&)*pen).m_bHasBeenSpawned = TRUE;
		((CEyeman&)*pen).m_bTemplate = FALSE;
    ((CEyeman&)*pen).m_bPESpawned = TRUE;
    ((CEyeman&)*pen).m_tmSpawned = _pTimer->CurrentTick()+0.3f;
		((CEyeman&)*pen).Initialize();
		// fire it
		((CEyeman&)*pen).LaunchAsFreeProjectile(FLOAT3D(0, 0, -80/m_fFireStrength), this);
		((CEyeman&)*pen).SetDesiredRotation(ANGLE3D(0, 0, 0));
    // increase the number of spawned enemies
		m_ctCritterCounter++;
    if (m_ctCritterCounter==1) {
      m_ctEnemyMax += 20;
    ...
    pl = CPlacement3D(vWpnPipeAbs, aWpnAng + ANGLE3D(-45+(FRnd()*90), -30+(FRnd()*60), 0));
		...
		// start properties
		((CWoman&)*pen).m_bStretchModel = TRUE;
		((CWoman&)*pen).m_penEnemy = m_penEnemy;
		((CWoman&)*pen).m_bHasBeenSpawned = TRUE;
		((CWoman&)*pen).m_bTemplate = FALSE;
    ((CWoman&)*pen).m_bPESpawned = TRUE;
    ((CWoman&)*pen).m_tmSpawned = _pTimer->CurrentTick()+0.3f;
		((CWoman&)*pen).Initialize();
		// fire it
		((CWoman&)*pen).LaunchAsFreeProjectile(FLOAT3D(0, 0, -80/m_fFireStrength), this);
		((CWoman&)*pen).SetDesiredRotation(ANGLE3D(0, 0, 0));
    // increase the number of spawned enemies
		m_ctCritterCounter++;
		if (m_ctCritterCounter==m_ctEventCount) {
			OpenPyramidDoors();
  ...
  void FireOneWerebull(void)
  {
    ...
    // start properties
		((CWerebull&)*pen).m_bStretchModel = TRUE;
    ((CWerebull&)*pen).m_penEnemy = m_penEnemy;
		((CWerebull&)*pen).m_bHasBeenSpawned = TRUE;
		((CWerebull&)*pen).m_bTemplate = FALSE;
    ((CWerebull&)*pen).m_bPESpawned = TRUE;
    ((CWerebull&)*pen).m_tmSpawned = _pTimer->CurrentTick()+0.3f;
		((CWerebull&)*pen).Initialize();
		// fire it
		((CWerebull&)*pen).LaunchAsFreeProjectile(FLOAT3D(0, 0, -60/m_fFireStrength), this);
    ((CWerebull&)*pen).SetDesiredRotation(ANGLE3D(0, 0, 0));
    // increase the number of spawned enemies
		m_ctCritterCounter++;
    if (m_ctCritterCounter==1) {
      m_ctEnemyMax += 20;
  ...
  void OpenPyramidDoors(void)
  {
		CTString strDoors;
		strDoors.PrintF(TRANS("Pyramid Doors Have Opened"));
    INDEX iMaxPlayers = 16;
      ...
      CPlayer *penPlayer = (CPlayer*)&*_penPlayer->GetPlayerEntity(i);
		  if (penPlayer==NULL) { continue; }
			PrintCenterMessage(this, penPlayer, strDoors, 10.0f, MSS_INFO);
    }
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
			CEntity *pen = iten;
      if (IsDerivedFromClass(pen, "Trigger")) {
        if (((CTrigger *)&*pen)->m_strName=="Open pyramid entrance") {
 ...
 ************************************************************/
  ...
  WalkTo(EVoid) 
  {
    ...
    //m_fMoveSpeed = 20.0f/GetGameEnemyMultiplier();
		m_fMoveSpeed = 15.0f;
 ...
 ************************************************************/
  ...
  Fire(EVoid) : CEnemyBase::Fire
  {
    ...
    if( m_dapAttackPower==DAP_MEDIUM_POWER_ATTACK &&
        (_pTimer->CurrentTick()-m_fLastWalkTime) > 3.0f)
    {
      m_fAttackFireTime = 3.0f;
      m_fLastWalkTime = _pTimer->CurrentTick()+3.0f;
      return;
    }
    ...
    switch( m_dapAttackPower)
    {
      case DAP_PLAYER_HUNT:
        if( _pTimer->CurrentTick()-m_tmLastAngry > 5.0f)
        {
          m_fAttackFireTime = 2.5f+FRnd()*2.5f;
  ...
  FireLaser(EVoid)
  {
    ...
    //m_iAttID = DEVIL_ATTACHMENT_LASER;
    m_fAdjustWeaponTime = 2.0f;
    autocall AdjustWeaponForFire() EReturn;

    // fire lasers
    StartFireLaser();
    m_iFiredProjectiles = 0;
    while (m_iFiredProjectiles<m_iToFireProjectiles*20 && CountEnemies())
    {
    ...
    //autowait(0.25f*m_fPauseStretcher);
		...
		m_iPlayerSwitchCounter++;
		if (m_dsDevilState == DS_PYRAMID_FIGHT && m_iPlayerSwitchCounter>=2) {
			m_iPlayerSwitchCounter = 0;
  ...
  FirePredictedProjectile(EVoid)
  {
    ...
    m_iFiredProjectiles = 0;
    while (m_iFiredProjectiles<m_iToFireProjectiles*10 && CountEnemies())
    {
      //m_fAdjustWeaponTime = 0.2f;
      //autocall AdjustWeaponForFire() EReturn;
      autowait( 0.1f);
      FireOneWerebull();
      FireOneSmallWalker();
  ...
  FireElectricityGun(EVoid)
  {
    ...
    while (m_iFiredProjectiles<m_iToFireProjectiles*10 && CountEnemies())
    {
      ...
      while( _pTimer->CurrentTick() < m_tmTemp+0.1f)
      {
        wait(_pTimer->TickQuantum) {
          on (EBegin): {
            resume;
...
//*/
  FireGuidedProjectile(EVoid)
  {
    ...
    while (m_iFiredProjectiles<m_iToFireProjectiles*6 && CountEnemies())
    {
      m_tmLastPause = 0.1f;
...
//    ShakeItFarBaby(_pTimer->CurrentTick(), 1.5f);
    ...
    m_dapAttackPower = DAP_FULL_POWER_ATTACK;
  ...
  TeleportIntoPyramid(EVoid)
  {
    ...
    TurnOnPhysics();
    autocall GrabBothWeapons() EReturn;
//    autocall GrabUpperWeapons() EReturn;
    m_dsDevilState = DS_PYRAMID_FIGHT;
    m_dapAttackPower = DAP_FULL_POWER_ATTACK;
    m_fAttackRadius = 1e6f;
    m_fAttackRunSpeed = 8.0f;
 ...
 ************************************************************/
  Main(EVoid) {
    ...
    // this one is boss!
    m_bBoss = TRUE;
    //SetHealth(40000.0f*_pNetwork->ga_sesSessionState.GetPlayersCount());
    //m_fMaxHealth = 40000.0f*_pNetwork->ga_sesSessionState.GetPlayersCount();
    m_fMaxHealth = 40000.0f;
    SetHealth(40000.0f);
    ...
    */
    ...
    /*m_fDamageAdjustment = GetGameDamageMultiplier();
    if (m_fDamageAdjustment<=0) {
      m_fDamageAdjustment = 0.1f;
    }*/
    ...
    wait()
    {
      ...
      on (EDevilCommand eDevilCommand) :
      {
        ...
        {
					m_ctEnemyMax = GetSP()->sp_iEnemyMax*1.2f;
          m_ctEnemyMax = Clamp(INDEX(m_ctEnemyMax), INDEX(1), INDEX(200));
      ...
      on (EReturn) :
      {
        ...
        {
					m_ctEnemyMax = GetSP()->sp_iEnemyMax-20;
					m_ctEnemyMax -= _pNetwork->ga_sesSessionState.GetPlayersCount()*2;
					m_ctEventCount = 20*GetGameEnemyMultiplier();
          m_soSound.Set3DParameters(1000.0f, 500.0f, 2.0f, 1.0f);
					m_dapAttackPower = DAP_FULL_POWER_ATTACK;
          m_dsDevilState = DS_ENEMY;
					m_fFireStrength = 0.6f;
