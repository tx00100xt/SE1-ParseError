...
#define IRON_LIFE_TIME  3.0f // 10

// damages
#define IRON_DAMAGE_MIN 600.0f
#define IRON_DAMAGE_MAX 800.0f
#define IRON_RANGE_DAMAGE (25.0f)   // because we have 4 explosions //50
#define IRON_RANGE_HOTSPOT 1.0f  //2
#define IRON_RANGE_FALLOFF 8.0f //8
#define SOUND_RANGE 250.0f

#define STRETCH_2  FLOAT3D(1.5f,1.5f,1.5f)

void CCannonBall_OnPrecache(CDLLEntityClass *pdec, INDEX iUser) 
{
  pdec->PrecacheClass(CLASS_BASIC_EFFECT, BET_EXPLOSION04);
  pdec->PrecacheClass(CLASS_BASIC_EFFECT, BET_EXPLOSION03);
  pdec->PrecacheClass(CLASS_BASIC_EFFECT, BET_EXPLOSION01);
  pdec->PrecacheModel(MODEL_BALL);
  pdec->PrecacheTexture(TEXTURE_IRON_BALL);
  pdec->PrecacheTexture(TEXTURE_NUKE_BALL);
  pdec->PrecacheTexture(TEXTURE_BLUE_BALL);
  pdec->PrecacheTexture(TEX_REFL_CHROME1);
...
// ********* PLAYER ROCKET *********
 ...
 14 texture TEXTURE_BLUE_BALL     "Models\\Weapons\\Cannon\\Projectile\\BlueBall.tex",
 12 sound   SOUND_BALL_BOUNCE     "Models\\Weapons\\Cannon\\Sounds\\Bounce.wav",
200 texture TEX_REFL_CHROME1      "Models\\ReflectionTextures\\Chrome1.tex",
211 texture TEX_SPEC_STRONG       "Models\\SpecularTextures\\Strong.tex",
  ...
  void RenderParticles(void) {
    ...
    if (GetRenderType()!=CEntity::RT_MODEL) {
      return;
        ...
        Particles_CannonBall(this, 140.0f);
    }
  }
...
void Initialize(void) {
  ...
  } else if( m_cbtType == CBT_BLUE) {
    SetModelMainTexture(TEXTURE_BLUE_BALL);
  }
  ...
  GetModelObject()->mo_toReflection.SetData(GetTextureDataForComponent(TEX_REFL_CHROME1));
  // specular texture data
  GetModelObject()->mo_toSpecular.SetData(GetTextureDataForComponent(TEX_SPEC_STRONG));
  // start moving
  LaunchAsFreeProjectile(FLOAT3D(0.0f, 0.0f, -m_fLaunchPower), (CMovableEntity*)(CEntity*)m_penLauncher);
  en_fBounceDampNormal   = 0.125f; // 0.5
  en_fBounceDampParallel = 0.75f;
  en_fAcceleration = 0.0f;
  en_fDeceleration = 5.0f;
  en_fCollisionSpeedLimit = 40.0f;
  en_fCollisionDamageFactor = 100.0f; // 50
  SetHealth(5000.0f);
...
void Explosion(FLOAT3D vCenter, const FLOAT3D &vStretchExplosion, BOOL bHasExplosion, INDEX iHasLight)
{
  ESpawnEffect ese;
  FLOAT3D vOnPlane;
  ...
  if( bHasExplosion)
  {
    ...
    if( iHasLight == 1) {
      ese.betType = BET_EXPLOSION03;
    } else if (iHasLight == 2) {
      ese.betType = BET_EXPLOSION04;
		} else {
      ese.betType = BET_EXPLOSION01;
			//ese.betType = BET_NUKE_EXPLOSION;
    }
...
//    m_fCannonBallSize = 2.25f;
    ...
    if (IsOfClass(m_penLauncher, "Player")) {
      FLOAT fSpeed = GetSP()->sp_fProjectileSpeed/2;
      if (fSpeed>1) {
        m_fLaunchPower *= fSpeed;
      }
 ...
 //   crRay.cr_fTestR = 0.75f/2.0f*CANNONBALL_STRETCH;
    ...
    if(m_cbtType == CBT_IRON) {
      Explosion( FLOAT3D(0.0f,0.0f,0.0f),   STRETCH_2, TRUE, 1);
    } else if( m_cbtType == CBT_NUKE) {
      Explosion( FLOAT3D(0.0f,0.0f,0.0f),   STRETCH_2, TRUE, 2);
    } else if( m_cbtType == CBT_BLUE) {
      Explosion( FLOAT3D(0.0f,0.0f,0.0f),   STRETCH_2, TRUE, 3);
	}
