...
void CGame::ConsoleRender(CDrawPort *pdp)
{
  ...
  dpConsole.Fill(LCDFadedColor(C_BLACK|180));
  ...
  COLOR colMed  = LCDFadedColor(C_BLACK|255);
  COLOR colDark  = LCDFadedColor(C_vlGRAY|255);  
  ...
  //PIX pixLineSpacing = _pfdDisplayFont->fd_pixCharHeight + _pfdDisplayFont->fd_pixLineSpacing;
  ...
  dpConsole.DrawLine( 0, pixSizeJ-1, pixSizeI, pixSizeJ-1, LCDFadedColor(C_BLACK|255));  
  #else // Second Encounter
  ...
  const  ULONG colFill = (colMed & ~CT_AMASK) | 0x2F;
  ...
  dpConsole.SetFont( _pfdDisplayFont);

  //PIX pixTextX = (PIX)(dpConsole.GetWidth()*0.01f);
  //PIX pixYLine = dpConsole.GetHeight()-14;
  //dpConsole.SetFont( _pfdConsoleFont);
	//dpConsole.SetTextCharSpacing(-3.0f);
	//dpConsole.SetTextScaling(0.97f);
  ...
  dpConsole.PutText( strLineOnScreen, pixTextX, pixYLine-4, C_WHITE|255);
  dpConsole.SetTextMode(+1);

  // add blinking cursor
  if( ((ULONG)(_pTimer->GetRealTimeTick()*2)) & 1) {
    CTString strCursor="_";
    FLOAT fTextScalingX = 0.52f;
    PIX pixCellSize = _pfdDisplayFont->fd_pixCharWidth * fTextScalingX + dpConsole.dp_pixTextCharSpacing;
    ...
    dpConsole.PutText( strCursor, pixCursorX, pixYLine-1, C_RED|255);
  /*if( ((ULONG)(_pTimer->GetRealTimeTick()*2)) & 1) {
    CTString strCursor="_";
    ...
    dpConsole.PutText( strCursor, pixCursorX, pixYLine-2, C_RED|255);*/
  }
  ...
  while( pixYLine >= 0) {
    ...
    dpConsole.PutText( strLineOnScreen, pixTextX, pixYLine, C_WHITE|255);
    iBackwardLine++;
...
/*void CGame::ConsolePrintLastLines(CDrawPort *pdp)
{
  ...
  con_tmLastLines = Clamp( con_tmLastLines, 1.0f, 120.0f);
  INDEX ctLines   = CON_NumberOfLinesAfter( _pTimer->GetRealTimeTick() - con_tmLastLines*2);
  // if no lines left to print, just skip it
  if( ctLines==0) return;

  // setup font
  const FLOAT fTextScale  = (_fResolutionScaling+1);
  //_pfdDisplayFont->SetFixedWidth();
  pdp->SetFont( _pfdDisplayFont);
  pdp->SetTextScaling( fTextScale);
  PIX pixCharHeight = _pfdDisplayFont->GetHeight();//*1.05;
	//INDEX iLine2 = 0;
  // for each line
  for( INDEX iLine=0; iLine<ctLines; iLine++) {
    CTString strLine = CON_GetLastLine(iLine+1);
		CTString strLine1 = strLine;
		CTString strLine2 = "";
		CTString strLine3 = "";
		CTString strLine4 = "";
		CTString strTemp1 = "";
		CTString strTemp2 = "";
		strLine1.TrimSpacesRight();
		strTemp1 = strTemp2 = strLine1;
		INDEX iLineLength = strLine1.Length();
		//CPrintF("String Length: %d\n", iLineLength);
		if (iLineLength>135) {
			strTemp1.TrimRight(90);
			iLineLength -= 90;
			strTemp2.TrimLeft(iLineLength);

			strLine1 = strLine2 = strTemp1;
			strLine1.TrimRight(45);
			strLine1  += "\n";
			strLine2.TrimLeft(45);

			strLine3 = strLine4 = strTemp2;
			strLine3.TrimRight(45);
			strLine3  += "\n";
			iLineLength -= 45;
			strLine4.TrimLeft(iLineLength);
			strLine4  += "\n";

			strLine = strLine1 + strLine2 + strLine3 + strLine4;

			pdp->PutText( strLine, 5, pixCharHeight*(ctLines-iLine2-1), C_WHITE|255);
			iLine2 += 2;
		} else if (iLineLength>90) {
			strTemp1.TrimRight(90);
			iLineLength -= 90;
			strTemp2.TrimLeft(iLineLength);
			...
			strLine = strLine1 + strLine2 + strLine3;

			pdp->PutText( strLine, 5, pixCharHeight*(ctLines-iLine2-1), C_WHITE|255);
			iLine2 += 3;
		} else 
		if (iLineLength>45) {
			strTemp1.TrimRight(45);
			strLine1 = strTemp1 + "\n";
			iLineLength -= 45;
			strTemp2.TrimLeft(iLineLength);
			strLine2 = strTemp2 + "\n";
			strLine = strLine1 + strLine2;
			pdp->PutText( strLine, 5, pixCharHeight*(ctLines-iLine-1), C_WHITE|255);
			//iLine2 += 1;
		} else {
			pdp->PutText( strLine, 5, pixCharHeight*(ctLines-iLine-1), C_WHITE|255);
		}
