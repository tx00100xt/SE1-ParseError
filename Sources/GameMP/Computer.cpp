...
static INDEX _ctTextLinesOnScreen = 50;
static INDEX _ctTextCharsPerRow = 50;
...
static void SetFont1(CDrawPort *pdp)
{
  ...
  //pdp->SetTextAspect(0.7f);
	//pdp->SetTextCharSpacing(-2.0f);
	pdp->SetTextScaling(0.8f);
...
void PrintTitle(CDrawPort *pdp)
{
  ...
  CTString strTitle;
	...
	INDEX iNameOffset;
	strPlayerName = (const char *) _ppenPlayer->GetPlayerName();
  strTitle.PrintF(TRANSV("STATS for: %s"), (const char *) strPlayerName);
	iNameOffset = INDEX(strPlayerName.Length()/2+5);
  pdp->PutText( strTitle, _boxTitle.Size()(1)/2-iNameOffset*2, _boxTitle.Size()(2)/4, _colMedium);
}
...
{
  SetFont2(pdp);
  pdp->PutTextR( TRANS("Exit"), _boxExit.Size()(1)/2-3, _boxExit.Size()(2)/4, 
  ...
  for (INDEX iLine = _iTextLineOnScreen; 
    ...
    if (iPrintLine>ctLineToPrint) {
      continue;
		...
		//CPrintF("_boxMsgText.Size()(1): %d, _boxMsgText.Size()(2): %d\n", _boxMsgText.Size()(1), _boxMsgText.Size()(2));
    COLOR col = LerpColor( _colLight, _colMedium, Clamp( FLOAT(ctLineToPrint-iPrintLine)/3, 0.0f, 1.0f));
    pdp->PutTextC(_acmMessages[_iActiveMessage].GetLine(iLine), _boxMsgText.Size()(1)/2, pixJ, col);
    pixJ+=_pixCharSizeJ;
  }
...
void RenderMessageStats(CDrawPort *pdp)
{
  ...
  ULONG ulLevelMask = psp->sp_ulLevelsMask;
	...
	// Ck to see if this is a FE level so we can render the proper map
	BOOL bFELevel = FALSE;	
	CTString strLevelName = (const char *) _pNetwork->ga_fnmWorld.FileName();
    ...
    if (pdp->Lock()) {
      ...
      if( (colFade&255) == 255) {
        RenderMap( pdp, ulLevelMask, NULL);
				if (bFELevel) {
					// map hook
					RenderMap_FE( pdp, ulLevelMask, NULL);
				} else {
					// map hook
					RenderMap( pdp, ulLevelMask, NULL);
				}
...
void CGame::ComputerKeyDown(MSG msg)
{
  ...
  if (bRDown && msg.message==WM_RBUTTONUP) {
    ...
    BOOL bHasUnread = FALSE; //FindGroupWithUnread();
    // if some
    if (bHasUnread) {
...
void CGame::ComputerRender(CDrawPort *pdp)
{
  ...
  if (cmp_ppenPlayer!=NULL) {
    ...
    if (_pGame->gm_csComputerState==CS_ONINBACKGROUND) {
      ...
      // force reinit
      //_cmtCurrentType = (enum CompMsgType)-1;
      _cmtCurrentType = CMT_STATISTICS;
    }
  ...
  _colMedium = LCDFadedColor(C_GRAY|255);
  _colDark   = LCDFadedColor(LerpColor(C_dGRAY, C_vlGRAY, 0.5f)|255);
  _colBoxes  = LCDFadedColor(LerpColor(C_dGRAY, C_vlGRAY, 0.5f)|255);   
